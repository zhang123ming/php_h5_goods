define(["biz"], function(biz) {
    ...
    model.open = function(callback, type, merchid, multi, api_url, selected_ids) {
    ...
    model.init = function() {
        ...
        $(document).on("click", ".goods-selector-op", function() {
            ...
            console.log(model.option_switch )
    ...
    model.put = function(id) {
        ...
        var nooption = 1
        var url = "./index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=util.goods_selector.op&id=" + id + "&nooption=" + nooption + "&column=" + encodeURI($("#goods-selected-list-" + model.name).attr("data-column"));
        if (model.option_switch == 0) {
            url += "&nooption=1"
        }
        var set_color = "btn-danger";
        var htm = '<tr id="goods-selected-goods' + id + '">                <td><img src="' + model.selectedPool[id].thumb + '" style="width: 40px;height: 40px;border: solid #ccc 1px"></td>                <td><p class="title">' + model.selectedPool[id].title + '</p>                <p class="text text-danger">¥' + model.selectedPool[id].marketprice + "</p></td>                <td>" + '' + '</td>                <td>                <a class="btn btn-default btn-sm goods-selector-cancel" data-id="' + id + '">取消</a></td>                </tr>';
