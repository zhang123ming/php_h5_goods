define(['core', 'tpl'], function(core, tpl) {
    ...
    modal.init = function(fromDetail) {
        ...
        $('.order-server').unbind('click').click(function() {
            var orderid = $(this).data('orderid');
            var stauts = $(this).data('serverstatus');
            modal.server(orderid,stauts)
        })
    };
    ...
    modal.server = function(orderid, status) {
        container = new FoxUIModal({
            content: $(".order-server-hidden").html(),
            extraClass: "popup-modal",
            maskClick: function() {
                container.close()
            }
        ...
        container.show();
        if ($(".code_box")) {
            $('.server-pop').find('.code_box').unbind('click').click(function() {
                container.close()
            })
        } else {
            $('.server-pop').find('.close').unbind('click').click(function() {
                container.close()
            })
        }
        core.json('verify/serverqrcode', {
            id: orderid
        }, function(ret) {
            if (ret.status == 0) {
                FoxUI.alert('生成出错，请刷新重试!');
            ...
            console.log(ret);
            $('.server-pop').find('.qrimg').attr('src', ret.result.url).show();
        }, false, true)
    };
    return modal
});