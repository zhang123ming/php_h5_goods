	...
	modal.initList = function() {
		...
		$(document).on('click', '[data-toggle=setdefault]', function() {
			...
			core.json('member/address/setdefault', {
				...
				if (ret.status == 1) {
                    $(item).find('.fui-list-media img').attr('src','/addons/ewei_shopv2/static/images/member/default.png')
                    $(item).siblings().find('.fui-list-media img').attr('src','/addons/ewei_shopv2/static/images/member/default2.png')
                    $(item).find('.default').show();

                    $(item).siblings().find('.default').hide();
