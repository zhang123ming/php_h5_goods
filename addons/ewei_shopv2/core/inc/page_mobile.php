...
class MobilePage extends Page
{
	...
	public function __construct()
	{
		...
		$_W['share_close']=array('commission');
		$merch_data = m('common')->getPluginset('merch');
		if($_W['shopset']['merch']['is_member']==1 && !empty($_GPC['merchid'])){
			$merch_member = pdo_fetch('select count(*) as count from '.tablename('ewei_shop_merch_member').'where uniacid = :uniacid and merchid = :merchid and openid = :openid',array(':uniacid'=>$_W['uniacid'],':merchid'=>$_GPC['merchid'],':openid'=>$_W['openid']))['count'];
		...
		if(!empty($member)){
			m("member")->sendcommission($member['openid']);
		}
	...
	public function followBar($diypage = false, $merch = false,$shopname='')
	{
		global $_W;
		global $_GPC;
		...
		if(!empty($shopname)){
			$_W['shopset']['shop']['name'] = $shopname;
		}

		if ($diypage) {
			$diyfollowbar['params']['isopen'] = 1;
		}
		if(!$diyfollowbar['params']['isopen']) return 0;
		@session_start();
	...
	public function diyDanmu($diy = false)
	{
		...
		elseif ($danmu['params']['datatype'] == 2){
            ...
            if ($danmu['params']['datatype'] == 3){
            	$memberSql = " SELECT avatar AS imgurl,CONCAT('恭喜',LEFT(RPAD(LEFT(nickname,1),LENGTH(nickname),'*'),4),'成为了我们的新会员')AS content FROM ims_ewei_shop_member WHERE nickname <> '' AND avatar <> '' ORDER BY id DESC LIMIT 5 ";
                $danmuM['data'] = pdo_fetchall($memberSql);

                $orderSql = " SELECT b.avatar AS imgurl,CONCAT('最新的订单,来自会员【',LEFT(RPAD(LEFT(b.nickname,1),LENGTH(b.nickname),'*'),4),'】')AS content FROM ims_ewei_shop_order AS a,ims_ewei_shop_member AS b WHERE a.openid = b.openid ORDER BY a.id DESC LIMIT 5 ";
                $danmuO['data'] = pdo_fetchall($orderSql);

                $rechargeSql = " SELECT b.avatar AS imgurl,CONCAT('会员【',LEFT(RPAD(LEFT(b.nickname,1),LENGTH(b.nickname),'*'),4),'】充值了【',a.money,'元】')AS content FROM ims_ewei_shop_member_log AS a,ims_ewei_shop_member AS b WHERE a.openid=b.openid AND a.money>0 AND a.rechargetype='wechat' ORDER BY a.id DESC LIMIT 5 ";
                $danmuR['data'] = pdo_fetchall($rechargeSql);

                $danmu['data'] = array_merge($danmuM['data'],$danmuO['data'],$danmuR['data']);
                shuffle($danmu['data']);
			}
		}
		...
		foreach ($danmu['data'] as $index => $item) {
			...
			elseif ($danmu['params']['datatype'] == 2) {
				...
				if ($danmu['params']['datatype'] == 3){
					$danmu['data'][$index]['imgurl'] = tomedia($danmu['data'][$index]['imgurl']);
				}
			}
		}
