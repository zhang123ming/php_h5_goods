...
if (!function_exists('p')) {
	function p($name = '')
	{
		...
		if (com_run('perm::check_plugin', $name) || ($name == 'grant') || ($name == 'qpay')||($name == 'commission')) {
			if ($name == 'seckill') {
				if (!function_exists('redis') || is_error(redis())) {
					return false;
				}
...
if (!function_exists('com')) {
	...
	function com($name = '')
	{
		...
		//Perm_EweiShopV2ComModel perm
		$_coms[$name] = new $class_name($name);

		if ($name == 'perm') {
			// if($_SERVER['REMOTE_ADDR'] ==  '219.136.95.49'){
			// 	var_dump($_coms[$name]);die();
...
if (!function_exists('tpl_selector_new')) {
	function tpl_selector_new($name, $options = array())
	{
		...
		if ($options['type'] == 'image') {
			...
			$html .= '<div class=\'input-group multi-audio-details container\' ' . $show . ">\r\n<table class='table' style='width:600px;'>\r\n                    <thead>\r\n                        <tr>\r\n                            <th style='width:150px;'>分类缩略图</th>\r\n               <th style='width:150px;'>分类名称</th>\r\n               <th style='width:150px;'>操作</th>\r\n                        </tr>\r\n                    </thead>\r\n                    <tbody id='param-items_category" . $options['selectorid'] . '\' class=\'ui-sortable\'>';
		}
		...
		foreach ($options['items'] as $item) {
			if ($options['type'] == 'image') {
				...
				$html .= "\r\n                    <tr class='multi-product-item' data-" . $options['key'] . '=\'' . $item['categoryid'] . "' >\r\n                        <input type='hidden' class='form-control img-textname' readonly='' value='" . $item[$options['text']] . "'>\r\n                       <input type='hidden'  value='" . $item['categoryid'] . '\' name=\'' . $id . "'>\r\n                        <td style='width:80px;'>\r\n                            <img src='" . tomedia($item[$options['thumb']]) . "' style='width:70px;border:1px solid #ccc;padding:1px' onerror=\"this.src='../addons/ewei_shopv2/static/images/nopic.png'\">\r\n                        </td>\r\n                        <td style='width:220px;'>" . $item[$options['text']] . "</td>";
				// if ($item['optiontitle']) {
				// 	$html .= "<input type='hidden' id='" . $options['selectorid'] . 'packagegoods' . $item['categoryid'] . '\' value=\'' . $item['option'] . '\' name=\'' . $options['selectorid'] . 'packagegoods[' . $item['categoryid'] . ']\'>';
				...
				// else {
					...
					$maxbuy = (isset($item['maxbuy']) ? ',' . $item['maxbuy'] : '');
					$totalmaxbuy = (isset($item['totalmaxbuy']) ? ',' . $item['totalmaxbuy'] : '');
					$html .= "<input type='hidden' id='" . $options['selectorid'] . 'packagegoods' . $item['categoryid'] . '\' value=\'\' name=\'' . $options['selectorid'] . 'packagegoods[' . $item['categoryid'] . "]'>\r\n                    <input type='hidden' value='" . $item['packageprice'] . ',' . $item['commission1'] . ',' . $item['commission2'] . ',' . $item['commission3'] . $total . $maxbuy . $totalmaxbuy . '\' name=\'' . $options['selectorid'] . 'packgoods' . $item['categoryid'] . '\' >';
				// }
...
if (!function_exists('mobileUrl')) {
	function mobileUrl($do = '', $query = NULL, $full = false)
	{
		...
		if (!empty($commissionSet['showmid'])) {
			unset($query['mid']);
		}
...
if (!function_exists('redis')) {
	function redis()
	{
		...
		if (is_null($redis)) {
			...
			$redis_temp->select(intval($config['database']));
...
function checkPlugin($pluginname){
	...
	if (in_array($pluginname, $item_plugins)) {
		return true;
	}
}
function getdiycount(){

	global $_W,$_GPC;
	$diycount=explode("/",$_W['shopset']['shop']['diycount']);
	if(sizeof($diycount)){
		$diyArr=array(
			'order'=>$diycount[0],
			'member'=>$diycount[1],
...
/*type 来源类型*/
function setDanmu($openid,$type=0){
	global $_W,$_GPC;
