	...
	foreach ($orders as $o) {
		if ($o['status'] == 0) {
			...
			if ($o['isparent'] == 0) {
				...
				if (0 < $o['deductprice']) {
					m('member')->setCredit($o['openid'], 'credit1', $o['deductcredit'], array('0', $_W['shopset']['shop']['name'] . '自动关闭订单返还抵扣'.$_W['shopset']['trade']['credittext'].' '.$_W['shopset']['trade']['credittext'].': ' . $o['deductcredit'] . ' 抵扣金额: ' . $o['deductprice'] . ' 订单号: ' . $o['ordersn']));
				}
			}
