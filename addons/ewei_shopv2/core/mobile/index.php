...
class Index_EweiShopV2Page extends MobilePage
{
	...
	public function getpage()
	{
		...
		if (!(empty($_W['shopset']['shop']['indexrecommands']))) {
			...
			if (!(empty($goodids))) {
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice,ispresell,presellprice, productprice, minprice, total from ' . tablename('ewei_shop_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and deleted = 0 and status=1 order by instr(\'' . $goodids . '\',id),displayorder desc', array(':uniacid' => $uniacid));
	...
	public function syncAccessToken()
	{
		global $_W;
		global $_GPC;
