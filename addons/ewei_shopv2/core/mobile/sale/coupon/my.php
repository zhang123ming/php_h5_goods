...
class My_EweiShopV2Page extends MobileLoginPage
{
	...
	public function detail()
	{
		...
		if ($coupon['backtype'] == 0) {
			...
			$coupon['deduct'] = (double) $coupon['amount']>0?$coupon['amount'] : $coupon['deduct'];
		...
		if ($coupon['backtype'] == 2) {
			...
			if (!empty($coupon['backcredit']) && (0 < $coupon['backcredit'])) {
				$title3 = $title3 . '送' . $coupon['backcredit'] . ''.$_W['shopset']['trade']['credittext'].' ';
			}
		...
		if ($coupon['backtype']==3) {
			$title3 = '减' . (double) $coupon['amount'] . '元';
			$coupon['color'] = 'blue';
		}
		...
		if ($coupon['coupontype'] == 0) {
			...
			if(!empty($coupon['limitgoodcateids'])&(stripos($coupon['limitgoodcateids'],',')===false)){
				
				$useurl =mobileUrl('goods',array('cate'=>$coupon['limitgoodcateids']));	
			}elseif (!empty($coupon['limitgoodids'])&&stripos($coupon['limitgoodids'],',')===false) {
				$useurl =mobileUrl('goods/detail',array('id'=>$coupon['limitgoodids']));	
			}else{
	...
	public function getlist()
	{
		...
		foreach ($coupons as $i => &$row) {
			...
			if ($row['backtype'] == 0) {
				$row['deduct'] = (double) $row['amount']>0? $row['amount']:$row['deduct'];
			...
			if ($row['backtype'] == 2) {
				...
				if (!empty($row['backcredit']) && (0 < $row['backcredit'])) {
					$title3 = '立返';
					$title5 = '立返'.$_W['shopset']['trade']['credittext'].'';
				}
			...
			if ($row['backtype']==3) {
				$title3 = '<span class="subtitle">￥</span>' . (double) $row['amount'];
				$title5 = $title5 . '立减' . (double) $row['amount'];
			}
	...
	public function showcoupons()
	{
		...
		foreach ($coupons as $i => &$row) {
			...
			if ($row['backtype'] == 2) {
				...
				if (!empty($row['backcredit']) && (0 < $row['backcredit'])) {
					$title2 = $title2 . '送' . $row['discount'] . ''.$_W['shopset']['trade']['credittext'].'';
				}
		...
		if ($data['backtype'] == 0) {
			...
			if ($data['backtype'] == 2) {
				...
				if (!empty($data['backcredit']) && (0 < $data['backcredit'])) {
					$title1 .= '送' . $data['backcredit'] . ''.$_W['shopset']['trade']['credittext'].'';
				}
			...
			if (($row['coupontype'] == 0) || ($row['coupontype'] == 2)) {
				...
				if ($row['backtype'] == 0) {
					...
					if ($row['backtype'] == 2) {
						...
						if (!empty($row['backcredit']) && (0 < $row['backcredit'])) {
							$row['title1'] .= '送' . $row['backcredit'] . ''.$_W['shopset']['trade']['credittext'].'';
						}
