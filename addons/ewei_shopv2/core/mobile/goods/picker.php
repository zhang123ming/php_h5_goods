...
class Picker_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		...
		$goods = pdo_fetch("select id,thumb,protocolshow,title,merchid,marketprice,total,maxbuy,minbuy,unit,hasoption,showtotal,diyformid,diyformtype,diyfields,isdiscount,presellprice,\r\n                isdiscount_time,isdiscount_discounts, needfollow, followtip, followurl, `type`, isverify,limitbuy,maxprice, minprice, merchsale,ispresell,preselltimeend,unite_total,\r\n                threen,preselltimestart,presellovertime,presellover,islive,liveprice,minliveprice,maxliveprice\r\n                from " . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if (empty($openid)||(!empty($member['isnotlogin'])&&!empty($shopset['isopenlogin']))) {
			show_json(4);
		}
		...
		if ($goods['isdiscount'] && (time() <= $goods['isdiscount_time'])) {
			...
			if($merch_set['is_member']==1&&$goods['merchid']>0){
				if($goods['merchid']){
					$merchlevel = pdo_fetch('select ml.id from'.tablename('ewei_shop_merch_member').' as m,'.tablename('ewei_shop_merch_member_level').' as ml where m.level = ml.id and ml.merchid = m.merchid and m.openid = :openid and m.merchid = :merchid',array(':openid'=>$_W['openid'],':merchid'=>$goods['merchid']));
					if(empty($merchlevel)){
						$levelid = $member['level'];
					}else{
						$levelid = $merchlevel['id'];
						$merchlevels = true;
					}
				}else{
					$levelid = $member['level'];
		...
		if($openidlevel['level']>=91 && !empty($openidlevel)){
			$canbuy = 0;
		} else {
			$canbuy = 1;
		}
		
		show_json(1, array('goods' => $goods, 'seckillinfo' => $seckillinfo, 'specs' => $specs, 'options' => $options, 'diyformhtml' => $diyformhtml, 'cremind' => $cremind, 'cremindformhtml' => $cremindformhtml,'canbuy'=>$canbuy));
	}
}
