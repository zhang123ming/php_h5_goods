...
class Detail_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$courier = empty($_W['shopset']['trade']['courier'])?'快递':$_W['shopset']['trade']['courier'];
		$courier2 = empty($_W['shopset']['trade']['courier2'])?'快递':$_W['shopset']['trade']['courier2'];
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
		$id = intval($_GPC['id']);
		...
		$goods['usercommission'] = round($goods['minprice']*$goods['commission1_rate']/100,2);
		...
		// 判断商品组的购买限制
		$goods_group = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_goods_group') . ' WHERE uniacid=:uniacid and find_in_set('.$goods['id'].',goodsids)', array(':uniacid'=>$_W['uniacid']));

		if(!empty($goods_group['isbuylimit'])){
			$buylimit = unserialize($goods_group['buylimit']);
			$condition = '';
			$count = $buylimit[$member['level']]['count'];
			$month = $buylimit[$member['level']]['month'];

			if(!empty($month)){
				$begin = mktime(0,0,0,date('m')-$month+1,1,date('Y'));
				$end = mktime(23,59,59,date('m'),date('t'),date('Y'));

				$condition = 'and o.paytime>'.$begin.' and o.paytime<'.$end;
			}

			$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid in('.$goods_group['goodsids'].') and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid '.$condition, array(':goodsid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));

			if ($count <= $order_goodscount) {
				$goods['userbuy'] = 0;
				$goods['canbuy'] = false;
			}
		}
		...
		if (0 < $goods['usermaxbuy']) {
			...
			$begin = mktime(0,0,0,date('m'),date('Y'));
			$end = mktime(23,59,59,date('m'),date('t'),date('Y'));

			if(!empty($goods['usermonthbuy'])){
				$condition = 'and o.paytime>'.$begin.' and o.paytime<'.$end;
			}

			$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid '.$condition, array(':goodsid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));
		...
		$_W['shopset']['shop']['name']=$goods['title'];
		$canAddCart = true;
		if ( ($goods['type'] == 2) || ($goods['type'] == 3) || ($goods['type'] == 20) ||  !empty($is_task_goods) || !empty($gifts)) {
			$canAddCart = false;
		}
		...
		if($goods['merchid']){
			$merchlevel = pdo_fetch('select ml.* from'.tablename('ewei_shop_merch_member').' as m,'.tablename('ewei_shop_merch_member_level').' as ml where m.level = ml.id and ml.merchid = m.merchid and m.openid = :openid and m.merchid = :merchid',array(':openid'=>$_W['openid'],':merchid'=>$goods['merchid']));
			if(empty($merchlevel)){
		...
		if ($goods['isdiscount'] && (time() <= $goods['isdiscount_time'])) {
			...
			if (!isset($isdiscount_discounts['type']) || empty($isdiscount_discounts['type'])) {
				...
				$goods_discounts = m('order')->getGoodsDiscounts($goods, $isdiscount_discounts, $levelid,$merchlevels);
		...
		if (com('goodscode')) {
			...
			if ($commission_data['codeShare'] == 1) {
				...
				$title = '    ' . $title[0] . "\r\n    " . $title[1];
				$codedata = array(
					'portrait' => array('thumb' =>  tomedia($member['avatar']), 'left' => 40, 'top' => 40, 'width' => 100, 'height' => 100),
			...
			if ($commission_data['codeShare'] == 1) {
				...
				$title = m('common')->qrcode_title_substr($goods['title'],28,14);
				$codedata = array(
					'portrait' => array('thumb' => tomedia($member['avatar']), 'left' => 40, 'top' => 40, 'width' => 100, 'height' => 100),
		...
		// dump($codedata['portrait']['thumb'],$codedata['thumb']['thumb']);die;
		if ($_W['shopset']['shop']['shopmode']=='show') {
			$goods['canbuy']=0;
		}
		...
		if(!empty($_GPC['status'])){
			if($_GPC['status'] == 1){
				...
				$mem = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_member') . " where openid = '".$_W['openid']."' ");

				$mids = $mem['id'];
				$goodsids = $_GPC['goodsid'];
				$listss = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_counts') . ' where goods_id = '.$goodsids .' and mid = '.$mids);
				
				if($listss){
					if(date("Y-m-d",strtotime($listss['create_time'])) == date("Y-m-d")){
						
						pdo_update('ewei_shop_counts',array('count'=>$listss['count']+1),array('id' => $listss['id']));
					}else{
						pdo_insert('ewei_shop_counts', array('goods_id' => $goodsids,'mid'=>$mids,'count' => 1,'create_time'=>date("Y-m-d H:i:s")));
					}
				}else{
					pdo_insert('ewei_shop_counts', array('goods_id' => $goodsids,'mid'=>$mids,'count' => 1,'create_time'=>date("Y-m-d H:i:s")));
				}
	...
	public function get_comments()
	{
		...
		if (0 < $count['all']) {
			...
			$list = pdo_fetchall('select nickname,level,content,images,createtime,headimgurl from ' . tablename('ewei_shop_order_comment') . ' where goodsid=:goodsid and deleted=0 and checked=0 and uniacid=:uniacid order by istop desc, createtime desc, id desc limit 2', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
