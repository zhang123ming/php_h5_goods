...
class Cart_EweiShopV2Page extends MobileLoginPage
{
	...
	public function get_list()
	{
		...
		$sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock,g.preselltimeend,g.presellprice as gpprice,g.hasoption, o.stock as optionstock,g.presellprice,g.ispresell, g.maxbuy,g.limitbuy,g.title,g.thumb,g.isverify,ifnull(o.marketprice, g.marketprice) as marketprice,' . ' g.productprice,o.title as optiontitle,o.presellprice,f.optionid,o.specs,g.minbuy,g.maxbuy,g.unit,f.merchid,g.checked,g.isdiscount_discounts,g.isdiscount,g.isdiscount_time,g.isnodiscount,g.discounts,g.merchsale' . ' ,f.selected,g.type,g.intervalfloor,g.intervalprice  FROM ' . tablename('ewei_shop_member_cart') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
		...
		foreach ($list as &$g) {
			...
			//会员单次购买限制
			$g = m('goods')->getGoodsLimit($g,$openidlevel['id']);
	...
	public function update()
	{
		...
		$goods = pdo_fetch('select id,maxbuy,minbuy,total,unit,limitbuy from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid and status=1 and deleted=0', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
	...
	public function add()
	{
		...
		($total <= 0) && ($total = 1);
		$optionid = intval($_GPC['optionid']);
		$goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund,usermaxbuy from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$cartnum = pdo_fetchcolumn('select sum(total) from '.tablename('ewei_shop_member_cart').' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid and deleted=0',array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid'],':goodsid'=>$id));
		if ($cartnum>=$goods['usermaxbuy']&&$goods['usermaxbuy']>0) {
			show_json(0, '商品限购'.$goods['usermaxbuy'].'件，请检查购物车数量');
		}
		if (empty($goods)) {
			show_json(0, '商品未找到');
		}
		...
		if (($goods['type'] == 2) || ($goods['type'] == 3) || !empty($goods['cannotrefund'])) {
	...
	public function submit()
	{
		...
		if(count($list) > 1){
			for ($i = 0; $i < count($list); $i++) {
				$list[$i]['storeids'] = explode(",",$list[$i]['storeids']);
				if($list[$i]['isverify'] == 2){
					$res[$i] = true;
				} else {
					$res[$i] = false;
				}
			...
			if(count($unique_arr) > 1 ){
				show_json(0, "订单包含核销和非核销商品，不能结算！");
			}
			$storeids = array_column($list,'storeids');
			...
			if(!empty($storeids)){
				...
				foreach ($storeids as $key => $value) {
					...
					foreach ($storeids as $k => $v) {
						$result[$k] = array_intersect($storeids[$key],$storeids[$k]);
						if(empty($result[$k])){
							show_json(0, "商品没有共同的核销门店，不能结算！");
						}
					}
					// $arrcount = array();
					foreach ($result as $keys => $values) {
						foreach ($result as $ke => $val) {
							$stoids[$ke] = array_intersect($result[$keys],$result[$ke]);
							if(empty($stoids[$ke])){
								show_json(0, "商品没有共同的核销门店，不能结算！");
							}
						}
					}
					$res[$key] = array_unique($stoids);
				}
