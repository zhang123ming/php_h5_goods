...
class Index_EweiShopV2Page extends MobileLoginPage
{
	...
	public function detail()
	{
		...
		if($order['server_status']==1){
			if(!empty($order['server_detail'])){
				$server_detail = json_decode($order['server_detail'],true);
				$order['server_time'] = $server_detail['time'];
				$order['server_name'] = pdo_fetchcolumn('select nickname from '.tablename('ewei_shop_member').' where openid=:openid and uniacid=:uniacid',array(':openid'=>$server_detail['openid'],':uniacid'=>$_W['uniacid']));
				empty($order['server_name'])?$order['server_name']='未更新':'';
			}
