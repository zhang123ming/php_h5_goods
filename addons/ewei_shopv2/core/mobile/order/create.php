...
class Create_EweiShopV2Page extends MobileLoginPage
{
	...
	public function main()
	{
		...
		$courier = empty($_W['shopset']['trade']['courier'])?'运费':$_W['shopset']['trade']['courier'];
		$couponSet = $_W['shopset']['coupon'];
		// if(!empty($_GPC['storeid'])&&!empty($_GPC['storename'])){
		// 	$_SESSION['store_item']['id'] = $_GPC['storeid'];
		// 	$_SESSION['store_item']['name'] = $_GPC['storename'];
		// 	$store_id = $_SESSION['store_item']['id'];
		// 	$store_name = $_SESSION['store_item']['name'];
		// 	unset($_SESSION['store_item']);
		// }
		$member = m('member')->getMember($_W['openid']);
		$level = m('member')->getLevel($member['openid']);
		if($level['level']>=91){
			$this->message('您暂无购买权限！');
			exit();
		}
		if (p('exchange')) {
		...
		if (!$packageid) {
			...
			if (empty($id)) {
				...
				if (!empty($id) && !empty($iswholesale)) {
					...
					$sql = 'SELECT id as goodsid,subtitle,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,isverify,notselfverify,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,coupondeduct,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy,limitbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . $threensql . $ishotelsql . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
					$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
			...
			if ($isverify) {
				...
				foreach ($goods as $key => $value) {
					$goods[$key]['storeids'] = explode(',',$value['storeids']);
				}

				if(count($goods) > 1){
					$stoids = array_column($goods,'storeids');

					if(!empty($stoids)){
						$res = array();
						foreach ($stoids as $key => $value) {
							$result = array();
							foreach ($stoids as $k => $v) {
								$result[$k] = array_intersect($stoids[$key],$stoids[$k]);
								if(empty($result[$k])){
									$this->message('商品没有共同的核销门店，不能结算1！', '', 'error');
								}
							}

							foreach ($result as $keys => $values) {
								foreach ($result as $ke => $val) {
									$stoid[$ke] = array_intersect($result[$keys],$result[$ke]);
									if(empty($stoid[$ke])){
										$this->message('商品没有共同的核销门店，不能结算2！', '', 'error');
									}
								}
							}
							$res[$key] = array_unique($stoid);
						}

						if(!empty($res)){
							$uniarr = array_unique($res);
							$storeids = $uniarr['0']['0'];
						}	
					}
				}
				...
				if (empty($storeids)) {
					if (0 < $merchid) {
						...
						$stores = m('util')->multi_array_sort($stores, 'distance');
					}
					else {						
			...
			foreach ($g as $key => $value) {
				...
				if($package['ispackage']==1){
					$categorygoods = pdo_fetch('select title,packageprice,marketprice from ' . tablename('ewei_shop_package_category_goods') . "\r\n                            where goodsid=" . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' and `option` = '.$value['optionid'].' and pid = ' . $packageid . ' ');	
					$goods[$key]['marketprice'] = $categorygoods['marketprice'];
					if($package['packageway']==1){	
						$goods[$key]['packageprice'] = $package['static'];
						
					}else{
						$n = $categorygoods['packageprice']*($package['ratio']/100);
						$goods[$key]['packageprice'] += sprintf("%.2f", $n);
					}
					
					$goods[$key]['optiontitle'] = !empty($categorygoods['title']) ? $categorygoods['title'] : '';
				...
				$goods[$key]['total'] = 1;
				$goods[$key]['ispackage'] = $package['ispackage'];
			...
			$createInfo = array('id' => 0, 'gdid' => intval($_GPC['gdid']), 'fromcart' => 0, 
				'packagegoodsdiyform' => $packagegoodsdiyform, 'packageid' => $packageid, 'addressid' => $address['id'], 'storeid' => 0, 'couponcount' => 0, 'isvirtual' => 0, 'isverify' => 0, 'isonlyverifygoods' => 0, 'goods' => $goods, 'merchs' => 0, 'orderdiyformid' => 0, 'mustbind' => 0, 'fromquick' => intval($quickid),'backurl'=>$backurl,'commoninvoice'=>intval($invoiceset['commoninvoice'])/100,'addinvoice'=>intval($invoiceset['addinvoice'])/100);
		}
		...
		if (!empty($thisPerm)) {
			...
			if (in_array('task2',$item_plugins)) {

				if ($isvirtual) {
					$headtitle="会员升级";
					$hastask2= true;
					$changenum=0;
					$virtualGoods = $goods[0];
	...
	public function caculate()
	{
		...
		// if (!empty($_GPC['invoiceType'])) {

		// 	if ($_GPC['invoiceType']==1) {
		// 		$realprice=$realprice+$realprice*0.5;
		// 	}
		// 	if ($_GPC['invoiceType']==2) {
		// 		$realprice=$realprice+$realprice*0.6;
		// 	}
		// }
	...
	public function submit()
	{
		...
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
		if($_W['shopset']['shop']['is_sign_goods']==1){
			$sign = p('sign')->getSign();
			$signSet = p('sign')->getSet();
			if(!empty($signSet['reword_order'])){
				$signSet['reword_order'] = iunserializer($signSet['reword_order']);
				foreach ($signSet['reword_order'] as $value) {
					if(!empty($value['goods'])){
						$is_sign_goods = pdo_fetchcolumn('select goodsids from '.tablename('ewei_shop_goods_group').' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$value['goods']));
						$is_sign_goods;
						$is_sign_goods = explode(',',$is_sign_goods);
						if(in_array($_GPC['goods'][0]['goodsid'],$is_sign_goods)){
							if($sign['orderday']<$value['day']){
								show_json(0,'签到天数不足!');
							}
						}
					}
		...
		$packageprice = 0;
		if (!empty($packageid)) {
			$package = pdo_fetch('SELECT id,title,price,freight,cash,starttime,endtime,ispackage,ratio,static,packageway FROM ' . tablename('ewei_shop_package') . "\r\n                    WHERE uniacid = " . $uniacid . ' and id = ' . $packageid . ' and deleted = 0 and status = 1  ORDER BY id DESC');
			...
			$packgoods = pdo_fetchall('SELECT id,title,thumb,0,`option`,goodsid FROM ' . tablename('ewei_shop_package_goods') . "\r\n                    WHERE uniacid = " . $uniacid . ' and pid = ' . $packageid . '  ORDER BY id DESC');
		...
		foreach ($goods as $g) {
			...
			if (!empty($optionid)) {
				...
				if (!empty($option)) {
					...
					if ($data['type'] != 4) {
						...
						if ($packageid) {
							if($package['ispackage']==1){
								$packageoption = pdo_fetch('select packageprice from ' . tablename('ewei_shop_package_category_goods') . "\r\n                                where uniacid = " . $uniacid . ' and goodsid = ' . $goodsid . ' and `option` = ' . $optionid . ' and pid = ' . $packageid . ' ');
								$data['marketprice'] = $packageoption['packageprice'];
								if($package['packageway']==1){	
									$packageprice += $package['static'];

								}else{
									$n = $packageoption['packageprice']*($package['ratio']/100);
									$packageprice += sprintf("%.2f", $n);
								}
							}else{
				...
				if ($packageid) {
					if($package['ispackage']==1){
						$pg = pdo_fetch('select packageprice from ' . tablename('ewei_shop_package_category_goods') . "\r\n                                where uniacid = " . $uniacid . ' and goodsid = ' . $goodsid . ' and pid = ' . $packageid . ' ');	
						$data['marketprice'] = $pg['packageprice'];
								if($package['packageway']==1){	
									$packageprice += $package['static'];

								}else{
									$n = $pg['packageprice']*($package['ratio']/100);
									$packageprice += sprintf("%.2f", $n);
								}
					}else{
			...
			}else if($_GPC['packagegoodsdiyform']==1){
				if ($diyform_plugin) {
					$pachagegoodsdata = pdo_fetch('select id,diyformid,diyformfields,diyformdata,goodsid from ' . tablename('ewei_shop_package_goodsgroup') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1', array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));

					if (!empty($pachagegoodsdata)) {
						$data['diyformdataid'] = $pachagegoodsdata['diyformdataid'];
						$data['diyformdata'] = $pachagegoodsdata['diyformdata'];
						$data['diyformfields'] = $pachagegoodsdata['diyformfields'];
					}
				}
			}
			else {
			...
			if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
				...
				if ($data['coupondeduct']) {
					$coupondeduct +=$data['coupondeduct'];
				}
			}
	    ...
	    if (!empty($_GPC['freemachine'])) {
			$order['isfree'] = 1;
		}
		if ($_GPC['experienceTime']) {
			$order['runningtime'] = $_GPC['experienceTime'];
		}
		$order['parentid'] = 0;
		...
		if (!empty($_GPC['isinvoice'])) {
			$invoiceData['orderID']=$order['ordersn'];
			$invoiceData['openid']=$_W['openid'];
			$invoiceData['uniacid']=$_W['uniacid'];
			$invoiceData['amount']=$order['price'];
			$invoiceData['raisedType']=$_GPC['raisedType'];
			if (empty($_GPC['raised']) ){
					show_json(0, '请填写发票抬头!');
				}
			if ($_GPC['raisedType']==1) {
				$invoiceData['number']=$_GPC['number'];
				if (empty($_GPC['number']) ){
					show_json(0, '请填写识别号!');
				}
			}			
			$invoiceData['raised']=$_GPC['raised'];
			$invoiceData['type']=$_GPC['invoiceType'];
			$invoiceData['content']=$_GPC['invoiceContent'];
		...
		if ($multiple_order == 0) {
			...
			foreach ($allgoods as $goods) {
				...
				if (!empty($_GPC['experienceFee'])&&!empty($_GPC['machineMode'])) {//体验费
				...
				$order_goods['storeid'] = trim($_GPC['storeid']);
		...
		if (empty($virtualid)) {
			$res = m('order')->checkGoodsStock($orderid,0);
			if( $data["seckillinfo"] && $data["seckillinfo"]["status"] == 0 ) 
			{
			}
			else 
		...
		if ($pluginc) {
			if ($multiple_order == 0) {
				foreach($merch_array as $key=>$value){
					$pluginc->checkOrderConfirm($orderid,$key);	
				}
			}
			else {
				if (!empty($merch_array)) {
					foreach ($merch_array as $key => $value) {
						$pluginc->checkOrderConfirm($value['orderid'],$key);
					}
				}
			}
		}
	...
	public function categorydiyform()
	{
		global $_W;
		global $_GPC;
		$goodsid = intval($_GPC['goodsid']);
		$openid = $_W['openid'];
		$optionid = empty($_GPC['optionid']) ? 0:$_GPC['optionid'];
		$_SESSION['optionid'] = 'option'.$optionid;
		$data = $this->singleDiyformData($goodsid);
		extract($data);
		if ($diyformtype == 2) {
			$diyformid = 0;
		...
		$fields = $formInfo['fields'];
		$insert_data = $diyform_plugin->getInsertData($fields, json_decode(htmlspecialchars_decode($_GPC['diyformdata']),true));		
		$idata = $insert_data['data'];
		$corder_plugin = p('corder');		
		if ($corder_plugin) {
			$corder_plugin->check_data($idata);
		...
		$data = pdo_fetch('select id,diyformid from ' . tablename('ewei_shop_package_goodsgroup') . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':optionid' => $optionid, ':id' => $goodsid));

		if (empty($data)) {
			$data = array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $goodsid, 'optionid' => $optionid, 'selected' => 1, 'diyformid' => $diyformid, 'diyformdata' =>$idata, 'diyformfields' => iserializer($fields), 'createtime' => time());			
			pdo_insert('ewei_shop_package_goodsgroup', $data);
		}else {
			$data['diyformid'] = $diyformid;
			$data['diyformdata'] = $idata;
			$data['diyformfields'] = iserializer($fields);
			pdo_update('ewei_shop_package_goodsgroup', $data, array('id' => $data['id']));
		}
		show_json(1, array('pid'=>$_GPC['pid'],'goods_data_id' => $gdid));
	}
