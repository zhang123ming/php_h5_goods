...
class Index_EweiShopV2Page extends MobilePage
{
	...
	public function getlist()
	{
		...
		foreach ($verifygoods as $i => &$row) {
			...
			$verifynum = 0;
			if($row['storeid']>0){
				$row['store_name'] = pdo_fetch('select storename from '.tablename('ewei_shop_store').' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$row['storeid']))['storename'];
				$row['store_name'] = '(仅限'.$row['store_name'].'门店使用)';
			}
