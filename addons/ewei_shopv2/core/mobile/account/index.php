...
// 		if ((is_weixin()&&empty($shopset['isopenlogin'])) || !empty($_GPC['__ewei_shopv2_member_session_' . $_W['uniacid']])&&empty($member['isnotlogin'])&&!empty($shopset['isopenlogin'])) {
...
// 		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($shopset['isopenlogin'])&&is_weixin()) {
				$member2=m('member')->getMember($_W['openid']);
				if ($member['id']!=$member2['id']) {
					if ($member['openid']=='wap_user_'.$_W['uniacid'].'_'.$mobile&&empty($member2['mobile'])) {
						$arr['openid'] = $member2['openid'];
						$arr['uid'] = $member2['uid'];
						m('bind')->update($member['id'],$arr);
						pdo_delete('ewei_shop_member',array('id'=>$member2['id']));
					}else{
						show_json(0,'当前绑定的手机号不是'.$mobile);
					}
					
				}
				pdo_update('ewei_shop_member',array('isnotlogin'=>0),array('id'=>$member[id]));
			}
	...
	protected function rf($type)
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($shopset['isopenlogin'])&&is_weixin()) {
				$member2 = m('member')->getMember($_W['openid']);
				if (!empty($member)&&$member['id']!=$member2['id']) {
					show_json(0,'绑定的手机号码与微信账号不匹配,请输入正确的手机号');
				}
				if (empty($member)) {
					if (empty($member2['mobileverify'])) {
						$salt = m('account')->getSalt();
						m('bind')->update($member2['id'], array('mobile' => $mobile, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'mobileverify' => 1,'isnotlogin'=>0));
						unset($_SESSION[$key]);
						m('account')->setLogin($member2['id']);

						show_json(1, empty($type) ? '注册成功' : '密码重置成功');exit();
					}else{
						show_json(0, '您当前的微信账号已绑定了手机号'.$member2[mobile]);
					}
