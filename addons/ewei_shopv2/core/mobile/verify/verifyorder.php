...
class Verifyorder_EweiShopV2Page extends MobilePage
{
	public function orderData()
	{
		...
		/*开启强制模式*/
		$mobileverify=$verifyset['store']['mobileverify']?true:false;
		/*备货时间*/
		$stocktime = $verifyset['store']['stocktime'];
		...
		if (!empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$condition .= ' AND (o.verifycode=:keyword or locate(:keyword,o.verifycodes)>0)';
			}
				...
				$condition .= ' AND (locate(:keyword,o.ordersn)>0 or locate(:keyword,a.realname)>0 or locate(:keyword,a.mobile)>0 or locate(:keyword,o.carrier)>0 or  locate(:keyword,m.realname)>0 or locate(:keyword,m.mobile)>0 or locate(:keyword,m.nickname)>0)';
			}
		}

		if(isset($_GPC['status'])&&$_GPC['status']!='100'){
			if ($_GPC['status']<1001) {
				$condition .=" AND o.status={$_GPC[status]}";
			}
		}else{
			if (empty($_GPC['keyword'])&&empty($_GPC['searchfield'])) $condition .=" AND o.status>0";
		}

		if ($_GPC['searchtime']) {
			
			$starttime = strtotime($_GPC['searchtime']);
			$endtime  =$starttime+3600*24;
			$condition .=" AND  o.createtime between' {$starttime}' and '{$endtime}'";

		}

		$merchid = 0;
		$merch_plugin = p('merch');
		$saler = pdo_fetch('select * from ' . tablename('ewei_shop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (empty($saler) && $merch_plugin) {
			$saler = pdo_fetch('select * from ' . tablename('ewei_shop_merch_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		}
		...
		if (empty($saler)) {
			...
			$condition  .= " and o.storeid='{$saler['storeid']}'";
		}
		
		if ($merchid) {
			$tablesaler = tablename('ewei_shop_merch_saler');
			$condition  .= " and o.merchid='{$merchid}' ";

		}else{

			$tablesaler = tablename('ewei_shop_saler');
		}
		...
		if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('ewei_shop_order') . ' where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition . ' ORDER BY id DESC  ';
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				$value['verifytime'] =$value['verifytime']?date('Y/m/d H:i:s', $value['verifytime']):"未核销";
				$s = $value['status'];
				$pt = $value['paytype'];
				$value['statusvalue'] = $s;
				...
				if (($value['dispatchtype'] == 1) || !empty($value['isverify']) || !empty($value['virtual']) || !empty($value['isvirtual'])) {
					...
					if (is_array($carrier)) {
						$value['carriername']=$value['addressdata']['realname'] = $value['realname'] = $carrier['carrier_realname'];
						$value['carriermobile']=$value['addressdata']['mobile'] = $value['mobile'] = $carrier['carrier_mobile'];
					}

					if ($stocktime) {
						/*有备货时间并且已经过了备货时间*/
						$value['paytime']=$value['paytime']?date('Y-m-d H:i:s',$value['paytime']):"订单尚未支付";
						$value['stocktime']=$value['paytime']?date('Y-m-d H:i:s',$value['paytime']+$stocktime*3600):false;
						$value['tipforsaler']=$tipforsaler;
					}
				
					if ($mobileverify) {

						$value['mobileverify']=$mobileverify;
					}
				}
				else {
		...
		show_json(1, array('list' => $list, 'total' => $total,'totalmoney'=>$totalmoney,'pagesize' => $psize));
	}
	...
	public function log()
	{
		...
		include $this->template($orders,'verify/verifyorder/log');
	}
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		if ($merchid) {
			$sql = 'SELECT o.id,o.price,o.createtime,s.openid,s.salername FROM' . tablename('ewei_shop_order') . ' o inner JOIN ' . tablename('ewei_shop_merch_saler') . " s ON o.verifyopenid=s.openid   WHERE \r\n        o.uniacid=:uniacid AND o.ismr=0  AND   o.status =3  AND o.deleted=0 AND o.createtime between :createtime1 and :createtime2";
		}else{
	...
	public function detail()
	{
		...
		if (empty($item['addressid'])) {
			...
			$item['addressdata'] = array('realname' => $user['carrier_realname'], 'mobile' => $user['carrier_mobile'], 'address' => $user['address']);
		}
		else {
