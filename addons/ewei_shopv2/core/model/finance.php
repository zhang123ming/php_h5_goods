...
class Finance_EweiShopV2Model
{
	...
	public function pay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '', $return = true)
	{
		...
		if (empty($paytype)) {
			...
			$data = array('openid' => $member['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => '佣金打款', 'money' => $money, 'remark' => $desc, 'rechargetype' => 'system');
	...
	public function AliPay($aliPayAccount, $batch_no, $config, $remark = '现金提现')
	{
		...
		$string .= $config['key'];
		$set['sign'] = md5($string);
		$url = 'https://openapi.alipay.com/gateway.do?' . http_build_query($set, '', '&');
		return $url;
	}
