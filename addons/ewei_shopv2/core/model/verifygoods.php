...
class Verifygoods_EweiShopV2Model
{
	...
	public function createverifygoods($orderid)
	{
		...
		$ordergoods = pdo_fetchall('select o.openid,o.uniacid,o.id as orderid,og.storeid, og.id as ordergoodsid,g.verifygoodsdays,g.verifygoodsnum,g.verifygoodslimittype,g.verifygoodslimitdate,og.total ' . $sql2 . ' from ' . tablename('ewei_shop_order_goods') . '   og inner join ' . tablename('ewei_shop_goods') . " g on og.goodsid = g.id\r\n          inner join " . tablename('ewei_shop_order') . " o on og.orderid = o.id\r\n          where   og.orderid =:orderid and g.type = 5", array(':orderid' => $orderid));
		$time = time();
		foreach ($ordergoods as $ordergood) {
			...
			while ($i < $total) {
				...
				if($ordergood['storeid']){
					$data['storeid'] = intval($ordergood['storeid']);	
				}
