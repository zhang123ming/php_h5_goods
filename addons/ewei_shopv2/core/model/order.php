...
class Order_EweiShopV2Model
{
		...
		foreach ($order_goods as $key => $value) {
			...
			if (0 < $goods['isfullback']) {
				...
				if (!empty($fullbackgoods)) {
					$data = array('uniacid' => $uniacid, 'orderid' => $orderid, 'openid' => $value['openid'], 'day' => $fullbackgoods['day'], 'fullbacktime' => strtotime('+' . $fullbackgoods['startday'] . ' days'), 'goodsid' => $value['goodsid'], 'createtime' => time(),'type'=>$fullbackgoods['credit']);
	...
	public function payResult($params)
	{
		...
		$order = pdo_fetch('select id,ordersn,machineid, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,isprepay,isprepaysuccess,balance from ' . tablename('ewei_shop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':ordersn' => $ordersn));
		...
		if ($params['from'] == 'return') {
			...
			if ($order['istrade'] == 0) {
				...
				if ($order['status'] == 0) {
					...
					if (!empty($verifyset['store']['paynotice'])) {
				...
				}elseif (($order['status']==1)&&!empty($order['isprepay'])&&empty($order['isprepaysuccess'])&&!empty(floatval($order['balance']))) {

					$change_data['isprepaysuccess'] = 1;
					$change_data['price']=$order['price']+$order['balance'];
					pdo_update('ewei_shop_order', $change_data, array('id' => $orderid));
				...
				if (($order['status']==1)&&!empty($order['isprepay'])&&empty($order['isprepaysuccess'])&&floatval($order['balance'])) {
					$change_data['isprepaysuccess'] = 1;
				}
				pdo_update('ewei_shop_order', $change_data, array('id' => $orderid));
				if (($order['status'] == 0) && ($count_ordersn == 1)) {
	...
	/**
     * 计算订单中商品累计赠送的'.$_W['shopset']['trade']['credittext'].'
     * @param type $order
     */
	...
	/**
     * //处理订单库存及用户'.积分.'情况(赠送'.积分.')
     * @param type $orderid
     * @param type $type 0 下单 1 支付 2 取消 3订单完成
     */
	public function setStocksAndCredits($orderid = '', $type = 0)
	{
		global $_W;
		$order = pdo_fetch('select id,agentid,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,g.inviter_credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type,g.merchid from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		$credits = 0;
		...
		foreach ($goods as $g) {
			...
			$gcredit = trim($g['credit']);
			$gcredit_inviter = trim($g['inviter_credit']);
			...
			if (!empty($gcredit_inviter)) {
				if (strexists($gcredit_inviter, '%')) {
					$inviter_credits += intval((floatval(str_replace('%', '', $gcredit_inviter)) / 100) * $g['realprice']);
				}
				else {
					$inviter_credits += intval($g['inviter_credit']) * $g['total'];
				}
			}
		...
		if (empty($credit1['sendtype'])) {
			$credit1_sendtype = 1;
		}else{
			$credit1_sendtype = 3;
		}
		if (0 < $inviter_credits&&!empty($order['agentid'])) {
			$shopset = m('common')->getSysset('shop');
			$member = m('member')->getMember($order['openid']);
			$agentinfo = m('member')->getMember($member['agentid']);
			$commissionSet = p('commission')->getSet();
			if ($type == $credit1_sendtype) {//订单给上级赠送积分送积分
				m('member')->setCredit($agentinfo['openid'], 'credit1', $inviter_credits, array(0, $shopset['name'] . '下级购物积分 订单号: ' . $order['ordersn']));
				m('notice')->sendMemberPointChange($agentinfo['openid'], $inviter_credits, 0);
			}
			else {
				if ($type == 2) {
					if ($credit1_sendtype <= $order['status']) {//订单退款
						m('member')->setCredit($agentinfo['openid'], 'credit1', 0 - $inviter_credits, array(0, $shopset['name'] . '购物取消订单扣除积分 订单号: ' . $order['ordersn']));
						m('notice')->sendMemberPointChange($agentinfo['openid'], $inviter_credits, 1);
					}
				}
			}
		}
		if (0 < $credits) {
			$shopset = m('common')->getSysset('shop');
			$commissionSet = p('commission')->getSet();
			if ($type == $credit1_sendtype) {//订单送积分
				...
				if ($type == 2) {
					if ($credit1_sendtype <= $order['status']) {//订单退款
						m('member')->setCredit($order['openid'], 'credit1', 0 - $credits, array(0, $shopset['name'] . '购物取消订单扣除'.$_W['shopset']['trade']['credittext'].' 订单号: ' . $order['ordersn']));
						m('notice')->sendMemberPointChange($order['openid'], $credits, 1);
					}
				}
			}
		}
		else if ($type == $credit1_sendtype) {
			// 订单送积分 判断积分优惠配置
			$data = m('common')->getPluginset('sale');
			$credit1 = iunserializer($data['credit1']);
			...
			$ordercount = 0;
			if($firstpay == '1'){
				$sql = 'SELECT count(*) FROM'. tablename('ewei_shop_order').' WHERE openid = :openid AND uniacid =:uniacid AND `paytime` > 0 ';
				$data = pdo_fetch($sql,array(':openid'=>$order['openid'],':uniacid'=>$_W['uniacid']));
			...
			if ($type == 2) {
				if ($credit1_sendtype <= $order['status']) {
					$money = com_run('sale::getCredit1', $order['openid'], (double) $order['price'], $order['paytype'], 1, 1,'',$order['id']);
	...
	public function getGoodsDiscounts($goods, $isdiscount_discounts, $levelid, $options = array(),$merchlevels= false)
	{
	...
	public function getGoodsDiscountPrice($g, $level, $type = 0)
	{
		...
		if($g['merchid']&&$merchset['is_member']==1){
			$merchlevel = pdo_fetch('select ml.* from'.tablename('ewei_shop_merch_member').' as m,'.tablename('ewei_shop_merch_member_level').' as ml where m.level = ml.id and ml.merchid = m.merchid and m.openid = :openid and m.merchid = :merchid',array(':openid'=>$_W['openid'],':merchid'=>$g['merchid']));
			if(empty($merchlevel)){
				if(!empty($level['id'])) {
		...
		if ($g['isdiscount'] && (time() <= $g['isdiscount_time']) && $buyagain_sale) {
			if (is_array($isdiscount_discounts)) {
				if($g['merchid']&&$merchset['is_member']==1){
					$key = (!empty($level['id']) ? 'level' . $level['id'] : 'merch');
					if($isdiscount_discounts['merch']&& empty($isdiscount_discounts[$key])){
						$key = 'merch';
					}	
				}else{
		...
		if ($data['birthday']) {
			...
			if ($member['birthmonth']==$currentMonth&&$member['birthday']==$currentDay) {
				$hasbirthdaydiscount  =true;

			}

			if ($hasbirthdaydiscount) {
				if ((0 < $data['birthdayDiscount'][$member[level]]) && ($data['birthdayDiscount'][$member[level]] < 10)) {
					$price2 = round(($data['birthdayDiscount'][$member[level]] / 10) * $gprice, 2);
	...
	public function getOrderCommission($orderid, $agentid = 0,$ismerch)
	{
		global $_W;
		if (empty($agentid)) {
		...
		foreach ($order_goods as &$og) {
			if (!empty($level) && !empty($agentid)) {
				...
				if($ismerch>0){
					foreach($commissions as $k=>$item){
						switch ($k) {
							case 'agent100':
								foreach($commissions['agent100'] as $value){
									$commissionagent += $value;

								}
								break;
							case 'agent99':
								foreach($commissions['agent99'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent98':
								foreach($commissions['agent98'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent97':
								foreach($commissions['agent97'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent96':
								foreach($commissions['agent96'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent95':
								foreach($commissions['agent95'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent94':
								foreach($commissions['agent94'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent93':
								foreach($commissions['agent93'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent92':
								foreach($commissions['agent92'] as $value){
									$commissionagent += $value;									
								}
								break;
							case 'agent91':
								foreach($commissions['agent91'] as $value){
									$commissionagent += $value;									
								}
								break;
						}
		...
		unset($og);
		$commission = $commission1 + $commission2 + $commission3+$commissionagent;
		return $commission;
	}
	...
	public function checkOrderGoods($orderid)
	{
        ...
        $goodsidSql = " SELECT og.goodsid,g.isupgradebag,g.total FROM ".tablename('ewei_shop_order_goods')." AS og, ".tablename('ewei_shop_order')." AS o, ".tablename('ewei_shop_goods')." AS g WHERE og.orderid = o.id AND o.id = ".$orderid." AND og.goodsid = g.id AND g.isupgradebag = 0 AND o.openid = '".$member['openid']."' AND o.uniacid = ".$_W['uniacid']." LIMIT 1 ";
        $goodsid = pdo_fetch($goodsidSql);
		if ((empty($item['isnewstore']) || empty($item['storeid'])) && empty($item['istrade'])) {
				...
				if (($seckillinfo && ($seckillinfo['status'] == 0)) || !empty($ispeerpay)) {
					...
					if ($data['totalcnf'] == 1) {
						if (!empty($data['optionid'])) {
							...
							if (!empty($option)) {
								if ($option['stock'] != -1) {
									if (empty($option['stock'])||($data['buycount']>$option['stock'])) {
										$flag = 1;
							...
							if ($data['stock'] != -1) {
								if (empty($data['stock'])||($data['buycount']>$data['stock'])) {
									$flag = 1;
									$msg .= $data['title'] . '库存不足!';
								}
							}
	...
	*
	*/
	public function checkGoodsStock($orderid = "", $type = 0) 
	{
		global $_W;
	 ...
	 * 根据商品数量,返回各个不同层次的批发价;
	 */
	public function getWholesaleprice($total = 1,$goodsid){
		if (empty($goodsid)){
			return false;
		}
		...
		$goodsSql = " select * from ims_ewei_shop_goods where id= ".$goodsid;
		$goods = pdo_fetch($goodsSql);
		if (empty($goods)){
			return false;
		}
		if ($goods['type']!=4){
		...
		$goods['intervalprice'] = unserialize($goods['intervalprice']);
		if ($goods['intervalfloor']==1){
			if ($total>=$goods['intervalprice'][0]['intervalnum']){
				$price = $goods['intervalprice'][0]['intervalprice'];
			}
		}
		if ($goods['intervalfloor']==2){
			if ($total>=$goods['intervalprice'][0]['intervalnum']&&$total<$goods['intervalprice'][1]['intervalnum']){
				$price = $goods['intervalprice'][0]['intervalprice'];
			}
			if ($total>=$goods['intervalprice'][1]['intervalnum']){
				$price = $goods['intervalprice'][1]['intervalprice'];
			}
		}
		if ($goods['intervalfloor']==3){
			if ($total>=$goods['intervalprice'][0]['intervalnum']&&$total<$goods['intervalprice'][1]['intervalnum']){
				$price = $goods['intervalprice'][0]['intervalprice'];
			}
			if ($total>=$goods['intervalprice'][1]['intervalnum']&&$total<$goods['intervalprice'][2]['intervalnum']){
				$price = $goods['intervalprice'][1]['intervalprice'];
			}
			if ($total>=$goods['intervalprice'][2]['intervalnum']){
				$price = $goods['intervalprice'][2]['intervalprice'];
			}
		}
		return $price;
	}
