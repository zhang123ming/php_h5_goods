...
class Goods_EweiShopV2Model
{
	...
	public function getList($args = array())
	{
		...
		//判断是否获取到is_opensynchro
		$is_opensynchro = (!empty($args['is_opensynchro']) ? $args['is_opensynchro']:'');
		if (!empty($merchid)) {
		...
		if ($args['sourcedata']==1) {
			$condition .= ' and `merchid` = 0';

		}elseif ($args['sourcedata']==2) {

			if (!empty($merchid)) {
				$condition .= ' and merchid=:merchid and checked=0';
				$params[':merchid'] = $merchid;
			}else{
				$condition .= ' and merchid<>0 and checked=0';
			}
		}else{

			if (!empty($merchid)) {
				if ($is_opensynchro) {//获取到is_opensynchro=1就显示总店+自身多商户的全部商品
					$condition .= ' and merchid in (0,'.$merchid.') and checked=0';
					//merchid in (0,'.$merchid.')：总店的merchid=0
				}else{
					$condition .= ' and merchid=:merchid and checked=0';
					$params[':merchid'] = $merchid;
				}
		...
		if (!empty($args['group'])) {
			$ids = pdo_fetchcolumn('select goodsids from '.tablename('ewei_shop_goods_group').' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$args['group']));
		}
		if (!empty($ids)) {
			$condition .= ' and id in ( ' . $ids . ')';
		}
		
		if (!empty($args['bargain_goodsid'])) {
			$condition .= ' and id not in ( ' . $args['bargain_goodsid'] . ')';
		}
		...
		if (!$random) {
			$sql = "SELECT id,title,subtitle,thumb,marketprice,productprice,commission1_rate,minprice,maxprice,isdiscount,isdiscount_time,isstatustime,statustimestart,statustimeend,isdiscount_discounts,sales,salesreal,total,nocommission,description,bargain,`type`,hasoption\r\n            FROM " . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else {
			$sql = "SELECT id,title,thumb,marketprice,productprice,commission1_rate,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,nocommission,description,bargain,`type`,ispresell,preselltimeend,hasoption\r\n            FROM " . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
		...
		foreach ($list as $k => &$v) {
			if(empty($v['nocommission'])){
				if(!empty($v['minprice'])){
					$v['commission'] = round($v['minprice']*$v['commission1_rate']/100,2);
				} else {
					$v['commission'] = round($v['marketprice']*$v['commission1_rate']/100,2);
				}
			} else {
				$v['commission'] = 0;
			}
	...
	public function categorylist($args = array())
	{
		global $_W;
		$openid = $_W['openid'];
		$page = (!empty($args['page']) ? intval($args['page']) : 1);
		...
		if($args['cates']['cid']){			
			...
			foreach ($catearr as $key => $value) {
				...
				if(is_array($son)) {					
					foreach ($son as $skey => $svalue) {
						if ($key==0) {
							if ($skey == 0) {
								$condition .= '( FIND_IN_SET(' . $svalue . ',cates)';
							}else{
								$condition .= ' or FIND_IN_SET(' . $svalue . ',cates)';
							}
						}else{
							if ($skey == 0) {
								$condition .= ' AND ( FIND_IN_SET(' . $svalue . ',cates)';
							}else{
								$condition .= ' or FIND_IN_SET(' . $svalue . ',cates)';
							}
						}
					...
					if ($key==0) {
						...
						$condition .= ' AND FIND_IN_SET(' . $value . ',cates)';
					}
			...
			if (!empty($args['cate'])) {			
			$condition .= ' AND ( ';
			$condition .= 'FIND_IN_SET(' .$args['cate']. ',cates)';
			$condition .= ' <>0 )';
			}
		}	
		...
		if (!$random) {
			$sql = "SELECT id,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isstatustime,statustimestart,statustimeend,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,hasoption\r\n            FROM " . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else {
			$sql = "SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,preselltimeend,hasoption\r\n            FROM " . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
	...
	public function getMemberPrice($goods, $level)
	{
		...
		if($goods['merchid']&&$merchset['is_member']==1){
	...
	//会员单次购买限制
	public function getGoodsLimit($goods,$level)
	{	
		global $_W;
		if(empty($level)){
		...
		if (!isset($goods['limitbuy'])) {

			$goods['limitbuy'] = pdo_getcolumn('ewei_shop_goods',['uniacid'=>$_W['uniacid'],'id'=>$goods['id']],'limitbuy');
		}

		$limitbuy = unserialize($goods['limitbuy']);
		$max = 'maxlimit'.$level;
		$min = 'minlimit'.$level;
		$maxlimit = $limitbuy[$max];//最大限制
		$minlimit = $limitbuy[$min];//最小限制

		if(!empty($maxlimit) && $maxlimit > 0){

			$goods['maxbuy'] = $maxlimit;
		}

		
		if(!empty($minlimit) && $minlimit > 0){
			
			$goods['minbuy'] = $minlimit;
		}
