...
class Bind_EweiShopV2Model
{
	...
	public function merge($a = array(), $b = array())
	{
		...
		if (0 < $a['credit1']) {
			m('member')->setCredit($b['openid'], 'credit1', abs($a['credit1']), '全网通会员数据合并增加'.$_W['shopset']['trade']['credittext'].' +' . $a['credit1'] . $mergeinfo);
		}
	...
	public function sendCredit($member = array()) 
	{
		...
		{
			m('member')->setCredit($member['openid'], 'credit1', abs($data['bindmobilecredit']), '绑定手机号送'.$_W['shopset']['trade']['credittext'].' +' . $data['bindmobilecredit']);
		}
	}
}
