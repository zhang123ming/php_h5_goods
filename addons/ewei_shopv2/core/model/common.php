...
class Common_EweiShopV2Model
{
	...
	public function pc_wechat_native_build($params, $wechat, $type = 0){
		global $_W;
	...
	public function sendredpack($params)
	{
		...
		$extras = array();
		$errmsg = '未上传完整的微信支付证书，请到【系统设置】->【支付方式】中上传!1';
	...
	public function qrcode_title_substr($title='',$maxlength=0,$Linefeed=0)
	{
		if(empty($maxlength)){
			return $title;
		}
		...
		$num = 0;
		while ($num <$max) {
			$item = mb_substr($title,$i,1,'utf-8');
	...
	*$param 	$openid
	*$param 	$type string 
	*					'getall'获取当前openid的所有数据 
	*					'getstatus' 获取status各种状态的数据要查找的数据条件 第3个参数为整形数字 
	*					'getsum' 获取某个状态下的返利金额和集  第三个参数为数组 值为[ok 可提现 total 累计佣金 apply 待审核 check 待打款 pay 已提现 fail]已失效 
	*					'getcount'	获取所有状态下数据总条数
	*					  		
	*$param 	$stauts 	要查寻的数据状态
	*/
	public function record($openid,$type,$status=array())
	{
		global $_W;
		if(empty($openid)){
			return false;
		}
		if(preg_match_all("/^\d+$/",$openid)){
			$openid = m('member')->getmember($openid)['openid'];
		}
		$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>$status);
		switch($type){
			case 'getall':
				$sql = "select * from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and openid = :openid order by createtime desc";
				$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid);
				$list = pdo_fetchall($sql,$params);
				break;
			case 'getstatus':
				$sql = "select * from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and openid = :openid and status = :status order by createtime desc";
				$list = pdo_fetchall($sql,$params);
				break;
			case 'getsum':
				if (is_array($status)) {
					...
					foreach($status as $v){
						if($v=='ok'){
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and status = :status and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>0);
							$ok = pdo_fetch($sql,$params);
							!empty($ok)?$list['ok'] = $ok['total']:'';
						}elseif ($v=='total') {
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid);
							$total = pdo_fetch($sql,$params);
							!empty($total)?$list['total'] = $total['total']:'';
						}elseif($v=='apply'){
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and status = :status and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>1);
							$apply = pdo_fetch($sql,$params);
							!empty($apply)?$list['apply'] = $apply['total']:'';
						}elseif($v=='check'){
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and status = :status and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>2);
							$check = pdo_fetch($sql,$params);
							!empty($check)?$list['check'] = $check['total']:'';
						}elseif($v=='pay'){
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and status = :status and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>3);
							$pay = pdo_fetch($sql,$params);
							!empty($pay)?$list['pay'] = $pay['total']:'';
						}elseif($v=='fail'){
							$sql = "select sum(amount) as total from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and status = :status and openid = :openid";
							$params = array(":uniacid"=>$_W['uniacid'],":openid"=>$openid,":status"=>-1);
							$fail = pdo_fetch($sql,$params);
							!empty($fail)?$list['fail'] = $fail['total']:'';
						}
					}
					return $list;
				}else{
				...
				break;
			case 'getcount':
				$sql = "select count(*) as count from".tablename("ewei_shop_commission_record")."where uniacid = :uniacid and openid = :openid and amount>0";
				$params = array("uniacid"=>$_W['uniacid'],"openid"=>$openid);
				$list = pdo_fetch($sql,$params);
				break;
		}
	...
	/*
	*其它佣金数据插入表
	*$param 	$openid   用户opneid
	*$param     $amount   返利金额
	*$param     $detail   二维数组('备注',array())
	*$param     $teamdetail   一维数组()团队详细分红记录
	*/
	public function setRecord($openid,$amount,$detail=array(),$teamdetail='',$orderid='0')
	{
		global $_W;
		if(empty($openid)&&empty($amount)&&empty($detail)){
			return false;
		}
		if(is_array($detail)&&count($detail)>=2){
			$data['remark'] = $detail[0];
			$data['frominfo'] = '';
			foreach($detail[1] as $k=>$v){
				$data['frominfo'] .= $k.":".$v."/";
			}
		}else{
			return false;
		...
		if(!empty($teamdetail)){
			$data['detail'] = iserializer($teamdetail);
		}
		$data['openid'] = $openid;
		...
		pdo_insert("ewei_shop_commission_record",$data);
		$logid = pdo_insertid();
		return $logid;
	}
	/**
	 * 杉德扫码支付
	 */
	public function sandpay_qrcode($params, $sandpay, $type = 0)
	{
		global $_W;
		// step1: 拼接data
		$url = 'https://cashier.sandpay.com.cn/qr/api';
		$private_key_path = IA_ROOT.$sandpay['private_key'];
		$public_key_path = IA_ROOT.$sandpay['public_key'];
		$data = array(
			'head' => array(
				'version' => '1.0',
				'method' => 'sandpay.trade.precreate',
				'productId' => '00000005',//微信扫码
				'accessType' => '1',
				'mid' => $sandpay['mid'],
				'channelType' => '07',
			...
			'body' => array(
				...
				'limitPay' => '1',
				'totalAmount' => str_pad($params['total_fee'], 12, "0", STR_PAD_LEFT),
				'subject' => $params['title'],
				'body' => $params['body'],
				'txnTimeOut' =>  date('YmdHis', time()+7200),//订单过期时间
				'notifyUrl' => $_W['siteroot'] . 'addons/ewei_shopv2/payment/sandpay/notify.php',
				'extend' => $_W['uniacid'].":".$type
			)
		);
		// step2: 私钥签名
		$prikey = $this->loadPk12Cert($private_key_path, $sandpay['key']);
		if($prikey['error']==1) {
			return array('success'=>false,'msg'=>$prikey['msg']);
		}
		$sign = $this->sign($data, $prikey);
		if($sign['error']==1) {
			return array('success'=>false,'msg'=>$sign['msg']);
		}
		...
		// step4: post请求
		$result = $this->http_post_json($url . '/order/create', $post);
		if($result['error']==1) {
		...
		//step5: 公钥验签
		$pubkey = $this->loadX509Cert($public_key_path);
		if($pubkey['error']==1) {
			return array('success'=>false,'msg'=>$pubkey['msg']);
		}
		...
		if($data['head']['respCode']!='000000'){
			$payinfo = array(
				'success'=>false,
				'msg'=>$data['head']['respMsg'],
				'code' => $data['head']['respCode']
			);
		}else{
			$payinfo = array(
				'success'=>true,
	 ...
	 * 杉德微信公众号支付
	 */
	public function sandpay_wxpay($params, $sandpay, $type = 0)
	{
		global $_W;
		// step1: 拼接data
		...
		$data = array(
			...
			'body' => array(
				...
				'subject' =>  $params['title'],
				'body' => $params['body'],
				'txnTimeOut' => date('YmdHis', time()+7200),
				'payMode' => "sand_wx",//微信公众号支付
				'payExtra' => json_encode(array('subAppid' =>$sandpay['appid'], 'userId' => $_W['openid'])),
				// 'payExtra' => json_encode(array('subAppid' =>$sandpay['appid'], 'userId' => 'oxGUk1ZEQlzJ7cPuWeq5VPPGw7hM')),
				'clientIp' => $_SERVER['REMOTE_ADDR'],//客户端ip
				'notifyUrl' =>  $_W['siteroot'] . 'addons/ewei_shopv2/payment/sandpay/notify.php', // 填写异步通知地址(需要群里报备)
				'frontUrl' =>  $_W['siteroot'],//前台通知地址
			)
		);
		// step2: 私钥签名
		$prikey = $this->loadPk12Cert($private_key_path, $sandpay['key']);
		...
		// step4: post请求
		$result = $this->http_post_json($url . '/order/pay', $post);
		if($result['error']==1) {
			return array('success'=>false,'msg'=>$result['msg']);
		}
		$arr = $this->parse_result($result);
	...
	public function sandpay_refund($params,$sandpay,$type=0)
	{
		global $_W;
		$url = 'https://cashier.sandpay.com.cn/gateway/api';
		$private_key_path = IA_ROOT.$sandpay['private_key'];
		...
		$data = array(
			...
			'body' => array(
				'orderCode' => $params['refundsn'],
				'oriOrderCode' => $params['ordersn'],
				'refundAmount' => str_pad($params['total_fee'], 12, "0", STR_PAD_LEFT),
				'notifyUrl' =>   $_W['siteroot'] . 'addons/ewei_shopv2/payment/sandpay/notify.php',
				'refundReason' => '订单退款'
			)
		);
		...
		// step4: post请求
		$result = $this->http_post_json($url . '/order/refund', $post);
		if($result['error']==1) {
			return array('success'=>false,'msg'=>$result['msg']);
		}
		$arr = $this->parse_result($result);
		...
		$res = $this->verify($arr['data'], $arr['sign'], $pubkey);
		if($res!=1){
			return array('success'=>false,'msg'=>$res['msg']);
		}
		$data = json_decode($arr['data'],true);
		if($data['head']['respCode']!='000000'){
			return array('success'=>false,'msg'=>$data['head']['respMsg']);
		}
		return array('success'=>true,'msg'=>'退款成功');
	...
	function loadX509Cert($path)
	{
		...
		if (!$file) {
			return array('error'=>1,'msg'=>'loadx509Cert::file_get_contents ERROR');
		}
		...
		if (!$detail) {
			return array('error'=>1,'msg'=>'loadX509Cert::openssl_pkey_get_details ERROR');
		}
	...
	function loadPk12Cert($path, $pwd)
	{
		$file = file_get_contents($path);
		if (!$file) {
			return array('error'=>1,'msg'=>'loadPk12Cert::file
			_get_contents');
		}

		if (!openssl_pkcs12_read($file, $cert, $pwd)) {
			return array('error'=>1,'msg'=>'loadPk12Cert::openssl_pkcs12_read ERROR');
		}
		return $cert['pkey'];
	}
	...
	function sign($plainText, $path)
	{
		...
		if (!$result) {
			return array('error'=>1,'msg'=>'签名出错' . $plainText);
		}
	...
	function verify($plainText, $sign, $path)
	{
		...
		if (!$result) {
			return array('error'=>1,'msg'=>'签名验证未通过,plainText:' . $plainText . '。sign:' . $sign, '02002');
		}
