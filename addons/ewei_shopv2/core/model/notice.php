...
class Notice_EweiShopV2Model
{
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false)
	{
		...
		$orderpricestr = ' ¥' . $price . '元 (包含运费: ¥' . $order['freight'] . '元，'.$_W['shopset']['trade']['credittext'].'抵扣: ¥' . $order['creditmoney'] . '元)';
	...
	public function sendOrderWillCancelMessage($orderid = '0', $closedaytimes)
	{
		...
		$datas = array(
			...
			array('name' => '卖家备注信息', 'value' => $order['remarksaler']),
				...
				if (($order['status'] == 1) && !empty($order['istrade'])) {
					...
					if (($order['status'] == 1) && empty($order['sendtype'])) {
						...
						if (!empty($is_send)) {
							...
							if($carrier["carrier_address"]){
								$text = "您有新的已付款订单！！\n请及时安排发货。\n\n订单号：\n[订单号]\n订单金额：[订单金额]\n支付时间：[支付时间]\n---------------------\n购买商品信息：[商品详情]\n备注信息：[备注信息]\n---------------------\n收货人：[购买者姓名]\n收货人电话:[购买者电话]\n收货地址:{$carrier["carrier_address"]}\n\n请及时安排发货";
							}else{
								$text = "您有新的已付款订单！！\n请及时安排发货。\n\n订单号：\n[订单号]\n订单金额：[订单金额]\n支付时间：[支付时间]\n---------------------\n购买商品信息：[商品详情]\n备注信息：[备注信息]\n---------------------\n收货人：[购买者姓名]\n收货人电话:[购买者电话]\n收货地址:[收货地址]\n\n请及时安排发货";
							}
								...
								if ($pv && !empty($order['virtual'])) {
									if (empty($usernotice['virtualsend'])) {
										...
										$msg = array(
											...
											'keyword4' => array('title' => '信息', 'value' => '点击查看详情', 'color' => '#ff0000')
											);
	...
	public function sendMemberUpgradeMessage($openid = '', $oldlevel = NULL, $level = NULL)
	{
		...
		$lname = $_W['shopset']['shop']['levelname'];
		if(empty($lname)){
			$lname = '普通会员';
		}
		$oldlevelname = (empty($oldlevel['levelname']) ? $lname : $oldlevel['levelname']);
		$message = array(
			...
			if ($log_info['status'] == 1) {
				if ($isback) {
					...
					$message = array(
						...
						'remark'   => array('value' => "充值方式：".$_W['shopset']['shop']['name']."\n\n" . $_W['shopset']['shop']['name'] . '感谢您的支持，如有疑问请联系在线客服。', 'color' => '#000000')
						);
	 ...
	 * @return type
	 */
	public function sendStoreWithdrawalMessage($log_id = '', $channel = 0, $isback = false)
	{
		global $_W;
		global $_GPC;
		$url = $this->getUrl('member/log');
		$appurl = '/pages/member/log/index';
		$log_info = pdo_fetch('select * from ' . tablename('ewei_shop_store_withdrawal') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
		$member = m('member')->getMember($log_info['openid']);
		// $usernotice = unserialize($member['noticeset']);
		...
		// $log_info['gives'] = floatval($log_info['gives']);
		$log_info['amount'] = floatval($log_info['amount']);
		$type = '后台充值';
		...
		$yuan = (empty($_W['shopset']['commission']['texts']['yuan']) ? '元' : $_W['shopset']['commission']['texts']['yuan']);
		$datas[] = array('name' => '支付方式', 'value' => $type);
		$datas[] = array('name' => '充值金额', 'value' => $log_info['amount'] . $yuan);
		$datas[] = array('name' => '充值时间', 'value' => date('Y-m-d H:i', $log_info['createtime']));
		// $datas[] = array('name' => '赠送金额', 'value' => $log_info['gives']);
		$datas[] = array('name' => '到帐金额', 'value' => $log_info['amount'] + $log_info['gives']);
		$datas[] = array('name' => '实际到账', 'value' => $log_info['amount'] + $log_info['gives']);
		$datas[] = array('name' => '退款金额', 'value' => $log_info['amount'] + $log_info['gives']);
		$datas[] = array('name' => '积分余额', 'value' => $member['credit1'] . $credittext);
		$datas[] = array('name' => '账户余额', 'value' => $member['credit2'] . $yuan);
		$datas[] = array('name' => '提现金额', 'value' => $log_info['amount'] . '元');
		$datas[] = array('name' => '提现实际到账', 'value' => $log_info['amount'] . '元');
		// $datas[] = array('name' => '提现手续费率', 'value' => $log_info['charge']);
		// $datas[] = array('name' => '提现手续费', 'value' => $log_info['deductionmoney'] . '元');
		$datas[] = array('name' => '提现渠道', 'value' => $apply_type[$log_info['paytype']]);
		...
		$message = array(
			'first'    => array('value' => "恭喜您已经成功提现，请检查您的账户余额。\n", 'color' => '#ff0000'),
			'keyword1' => array('title' => '申请提现金额', 'value' => $log_info['amount'], 'color' => '#000000'),
			// 'keyword2' => array('title' => '取提现手续费', 'value' => $log_info['deductionmoney'], 'color' => '#000000'),
			'keyword2' => array('title' => '实际到账金额', 'value' =>	 $log_info['amount'], 'color' => '#000000'),
			'keyword3' => array('title' => '提现渠道', 'value' => $apply_type[$log_info['paytype']], 'color' => '#0a4b9c'),
			'remark'   => array('value' => "\n如果您选择银行卡提现，预计在1-2个工作日内到账，如有疑问请联系在线客服或点击查看详情。", 'color' => '#000000')
			);
		$this->sendNotice(array('openid' => $log_info['openid'], 'tag' => 'withdraw_ok', 'default' => $message, 'cusdefault' => $text, 'url' => $url, 'datas' => $datas));
		com_run('sms::callsms', array('tag' => 'withdraw_ok', 'datas' => $datas, 'mobile' => $member['mobile']));
	...
	public function sendOrderServiceMessage($openid,$params,$type,$diymessage)
	{
		global $_W;
		...
		if ($type == 'servicenotice') {
			$datas = array(
				array('name' => '用户昵称', 'value' => $member['nickname']),
				array('name' => '订单号', 'value' => $params['ordersn']),
				array('name' => '买家手机', 'value' => $params['mobile']),
				array('name' => '买家地址', 'value' => $params['address']),
				array('name' => '商品名称', 'value' => $params['title']),
				array('name' => '下单时间', 'value' => $params['createtime']),
				array('name' => '订单金额', 'value' => $params['price']),
				);
			...
			$text = "亲爱的[用户昵称]，您有新的服务订单，详情如下：\n\n订单编号：\n[订单号]\n订单金额：[订单金额]" ."\n下单时间：[下单时间]" . "\n商品名称 : " . $params['title'] ."\n买家地址 : " . $params['address'] . "\n买家联系方式 : " . $params['mobile'] ;
			if (!empty($diymessage)) {
				$text = $diymessage;
			}
			$this->sendNotice(array('openid' => $openid, 'tag' => '', 'default' => '', 'cusdefault' => $text, 'url' => $url, 'datas' => $datas));
		}
	}
	...
	public function gettemplateinfo()
	{
		...
		$result['virtualsend'] = array('id' => 7, 'name' => '自动发货通知(虚拟物品及卡密)', 'typegroup' => 'sys', 'groupname' => '系统消息通知', 'typecode' => 'virtualsend', 'templatecode' => 'OPENTM207793687', 'templatename' => '自动发货通知', 'content' => '{{first.DATA}}商品名称：{{keyword1.DATA}}订单号：{{keyword2.DATA}}订单金额：{{keyword3.DATA}}信息：{{keyword4.DATA}}{{remark.DATA}}');
	...
	public function sendMessageToSaler($orderid = '0')
	{
		global $_W;
		if (empty($orderid)) {
			return NULL;
		...
		$saler=pdo_fetchall("select * from ".tablename('ewei_shop_saler').' where status=:status and  uniacid=:uniacid and storeid=:storeid ', array(':storeid' =>$order['storeid'] ,':uniacid'=>$_W['uniacid'],':status'=>'1'));
		...
		$content="消费者下单通知！\r\n客户昵称:".$member['nickname']."\r\n商品详情:".$goods."\r\n付款状态：".$status.
		"\r\n".$orderpricestr;

		foreach ($saler as $value) {
			sleep(1);
			$result = m('message')->sendTexts($value['openid'], $content);
			//file_put_contents('mysql20181119.txt',date('Y-m-d H:i:s').":".var_export($result,true)."\r\nopenid:".$value['openid']."\r\n",FILE_APPEND);
		}
	}
