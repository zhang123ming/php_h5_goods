...
class Excel_EweiShopV2Model
{
	...
	public function export($list, $params = array())
	{
		...
		$lastColumn=$this->column_str($key);
		...
		foreach ($list as $row) {
			...
			if($row['merge']){
				...
				$sheet->getStyle("A{$rownum}")->applyFromArray($blodArray);	
			}
			++$rownum;
		}
		if($params['firstInsert']){
			$sheet->insertNewRowBefore(1,1);
			$sheet->mergeCells("A1:{$lastColumn}1");
			$sheet->getStyle("A1")->applyFromArray($blodArray);
			$sheet->setCellValue('A1', $params['firstInsertText']);
			$sheet->getStyle('A1')->getFont()->setName('宋体') //字体
        ->setSize(18) //字体大小
        ->setBold(true); //字体加粗
			//->setBold(true); //字体加粗 
		}
		if(!$params['firstInsert']) $rownum=$rownum-1;		
		$sheet->getStyle("A0:{$lastColumn}{$rownum}")->applyFromArray($styleArray);
		if($params['lastMerge']){
