...
class Coupon_EweiShopV2ComModel extends ComModel
{
	...
	public function getAvailableCoupons($type, $money = 0, $merch_array, $goods_array = array(),$machineid,$mergecoupon=false)
	{
		global $_W;
		global $_GPC;
		$time = time();
		...
		$condition = '';
		if ($mergecoupon) {
			$condition=' and c.backtype in (0,3)';
		}
		$param[':openid'] = $_W['openid'];
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select d.id,d.couponid,d.gettime,d.amount,d.goodscates,c.timelimit,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.limitgoodcatetype,c.limitgoodtype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('ewei_shop_coupon_data') . ' d';
		$sql .= ' left join ' . tablename('ewei_shop_coupon') . ' c on d.couponid = c.id';
		$sql .= ' where d.openid=:openid and d.uniacid=:uniacid and c.merchid=0 and d.merchid=0 and c.coupontype=' . $type . ' and d.used=0 '.$condition;

		if ($type == 1) {
			$sql .= 'and ' . $money . '>=c.enough ';
		}
		if(!empty($machineid)){
			$sql .= ' and d.machineid=:machineid';
			$param[':machineid']=$machineid;
		}
		//检查商家属性 判断优惠券 开始
		$sql.=$this->createSql($merch_array);
		//结束
		...
		if (!empty($merch_array)) {
			foreach ($merch_array as $key => $value) {
				...
				if (0 < $merchid) {
					...
					$sql .= ' left join ' . tablename('ewei_shop_coupon') . ' c on d.couponid = c.id';
					$sql .= ' where d.openid=:openid and d.uniacid=:uniacid and c.merchid=:merchid and d.merchid=:merchid and c.coupontype=' . $type . '  and d.used=0 '.$condition;
					$sql .= ' and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<=' . $time . ' && c.timeend>=' . $time . ')) order by d.gettime desc';
		...
		if ($_W['shopset']['coupon']['goodscate'] == 1) {
			foreach ($goods_array as $key => $value) {
				$cates[$key] = explode(",",$value['cates']);
			}

			if(!empty($cates)){
				$result = [];
				array_walk_recursive($cates, function($value) use (&$result) {
				    array_push($result, $value);
				});
				$goodscate = array_unique($result);
			}

			foreach ($list as $k => $v) {
			$goodscates = array();
				if (!empty($v['goodscates'])) {
					$goodscates = unserialize($v['goodscates']);
					foreach ($goodscates as $keys => $values) {
						$gcates[$keys] = explode(",",$values);
					}
					$results = [];
					array_walk_recursive($gcates, function($value) use (&$results) {
					    array_push($results, $value);
					});
					$coupongoodscate = array_unique($results);
					// foreach ($coupongoodscate as $ckey => $cvalue) {
					// 	$pid = pdo_fetchall('select * from ' . tablename('ewei_shop_category') . ' where parentid=:parentid and uniacid=:uniacid and enabled = 1', array(':parentid' => $cvalue,':uniacid' => $_W['uniacid']));
					// 	if(!empty($pid)){
					// 		unset($coupongoodscate[$ckey]);
					// 	}
					// }
					$res = array_intersect($goodscate,$coupongoodscate);
		...
		if (!empty($list)) {
			foreach ($list as &$row) {
				...
				if ($row['amount']>0) {
					$row['deduct'] = $row['amount'];
				}
				if ($row['backtype'] == 0||$row['backtype']==3) {
					$row['backstr'] = '立减';
					$row['css'] = 'deduct';
					...
					if ($row['backtype'] == 2) {
						...
						if (0 < $row['backredpack']) {
							...
							if (!empty($row['backcredit'])) {
								$row['backstr'] = '返'.$_W['shopset']['trade']['credittext'].'';
								$row['css'] = 'credit';
	...
	public function payResult($logno)
	{
		...
		if (empty($log['status'])) {
			...
			if ((0 < $coupon['credit']) && empty($log['creditstatus'])) {
				m('member')->setCredit($log['openid'], 'credit1', 0 - $coupon['credit'], '购买优惠券扣除'.$_W['shopset']['trade']['credittext'].' ' . $coupon['credit']);
	...
	public function sendBackMessage($openid, $coupon, $gives)
	{
		...
		if (isset($gives['credit'])) {
			$givestr .= ' ' . $gives['credit'] . '个'.$_W['shopset']['trade']['credittext'].'';
		}
	...
	public function useRechargeCoupon($log)
	{
		...
		if (!empty($backcredit)) {
			...
			if (0 < $backcredit) {
				...
				m('member')->setCredit($data['openid'], 'credit1', $backcredit, array(0, '充值优惠券返'.$_W['shopset']['trade']['credittext'].''));
			}
		}
		...
		if (!empty($backcredit) || !empty($backmoney)) {
			...
			if (!empty($backcredit)) {
				$text .= $backcredit . ''.$_W['shopset']['trade']['credittext'].'  ';
			}
	...
	private function createSql($merch_array){
		global $_W,$_GPC;
		$couponSet = $_W['shopset']['coupon'];
		$sql = '';
		if (!empty($merch_array)) {
			$merchs = array_filter(array_keys($merch_array));
			$merch_user = pdo_fetchall('select id,merchtype from '.tablename('ewei_shop_merch_user').' where uniacid=:uniacid and id in ('.implode(',', $merchs).') group by merchtype',array(':uniacid'=>$_W['uniacid']));
			if(!empty($couponSet['useapart'])&&count($merchs)>0){
				//比例发放 券 区分商家使用
				$sql .=' and d.frommerch not in ('.implode(',', $merchs).') ';
			}
			if(!empty($couponSet['fbapart'])){
				if (count($merch_user)==1) {
					$merchtype = $merch_user[0]['merchtype'];
					//B端 fromtype==0  F端 fromtype==1
					if($couponSet['fbapart']==1&&!empty($merchtype)){
						//F->B F端可在B端使用
						$sql .=' and (d.fromtype=1 or d.fromtype=0)';
					}
					if ($couponSet['fbapart']==2&&empty($merchtype)) {
						//B->F B端可在F端使用

						$sql .=' and (d.fromtype=2 or d.fromtype=0))';
					}
				}elseif (count($merch_user)>1) {
					$sql .=' and d.fromtype=0';
				}
	...
	public function consumeCouponCount($openid, $money = 0, $merch_array, $goods_array,$machineid='')
	{
		global $_W;
		global $_GPC;
		$couponSet = $_W['shopset']['coupon'];
		...
		$param[':openid'] = $openid;
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select d.frommerch,d.fromtype,d.amount, d.id,d.couponid,d.goodscates,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('ewei_shop_coupon_data') . ' d';
		$sql .= ' left join ' . tablename('ewei_shop_coupon') . ' c on d.couponid = c.id';
		...
		if (!empty($merch_array)) {
			foreach ($merch_array as $key => $value) {
				...
				if (0 < $merchid) {
					...
					$param[':merchid'] = $merchid;
					$sql = 'select d.id,d.couponid,d.goodscates,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('ewei_shop_coupon_data') . ' d';
					$sql .= ' left join ' . tablename('ewei_shop_coupon') . ' c on d.couponid = c.id';
	...
	public function useConsumeCoupon($orderid = 0)
	{
		...
		$order = pdo_fetch('select ordersn,createtime,couponid,couponprice from ' . tablename('ewei_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		$coupon = false;
		$couponids = explode(',',$order['couponid']);
		if (!empty($couponids)) {
			$count = count($couponids);
			$couponTotal = 0;
			foreach ($couponids as $key => $couponid) {
				if (!empty($couponid)) {
					$coupon = $this->getCouponByDataID($couponid);
				}
				if (empty($coupon)) {
					continue;
				}
				$couponTotal +=$coupon['amount']>0 ? $coupon['amount'] : $coupon['deduct'];
				pdo_update('ewei_shop_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $couponid));
				$this->posterbyusesendtask($coupon['id'], $_W['openid']);
			}
			if ($couponSet['mergecoupon']==2) {
				//优惠券多出部分可退
				$balance = $couponTotal-$order['couponprice'];
				if ($balance>0) {
					$couponid = $couponids[$count-1];
					$coupon = pdo_get('ewei_shop_coupon_data',array('id'=>$couponid));
					$backtype = pdo_fetchcolumn('select backtype from '.tablename('ewei_shop_coupon').' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$coupon['couponid']));
					if ($backtype==0||$backtype==3) {
						if ($coupon['amount']>0) {
							pdo_update('ewei_shop_coupon_data',array('amount -='=>$balance),array('id'=>$couponid));
						}
						
						unset($coupon['id']);
						unset($coupon['used']);
						unset($coupon['usetime']);
						unset($coupon['ordersn']);
						$newcoupon = $coupon;
						$newcoupon['amount'] = $balance;
						pdo_insert('ewei_shop_coupon_data',$newcoupon);
					}
			...
			foreach ($couponids as $couponid) {
				...
				if (empty($coupon['returntype'])) {
					if (!empty($coupon['used'])) {
						pdo_update('ewei_shop_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $couponid));
	...
	public function backConsumeCoupon($orderid)
	{
		...
		if ($canback) {
			...
			if (!empty($backcredit)) {
				...
				if (0 < $backcredit) {
					$gives['credit'] = $backcredit;
					m('member')->setCredit($order['openid'], 'credit1', $backcredit, array(0, '充值优惠券返'.$_W['shopset']['trade']['credittext'].''));
				}
			}
	...
	{
		global $_W;
		$data = pdo_fetch('select id,openid,amount,couponid,used,back,backtime from ' . tablename('ewei_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));
	...
	public function addtaskdata($orderid)
	{
		...
		if($pdata['goodscate'] == 1){
			$goodsid = pdo_fetchall('select goodsid from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid  and uniacid=:uniacid', array(':orderid' => intval($orderid), ':uniacid' => $_W['uniacid']));
			if(!empty($goodsid)){
				foreach ($goodsid as $key => $value) {
					$c = array();
					$c[$key] = pdo_fetchall('select cates from ' . tablename('ewei_shop_goods') . ' where id=:id  and uniacid=:uniacid', array(':id' => intval($value['goodsid']), ':uniacid' => $_W['uniacid']));
					$cates[$key] = $c[$key][0]['cates'];
				}
			}
		}
		if(!empty($cates)){
			$goodscates = serialize($cates);
		} else {
			$goodscates = 0;
		}

		if ($pdata['isopengoodssendtask'] == 1) {
			$goodssendtasks = pdo_fetchall("select  og.id,og.goodsid,og.orderid,og.parentorderid,og.total,gst.id as taskid,gst.couponid,gst.sendnum,gst.sendpoint,gst.num\r\n            from " . tablename('ewei_shop_coupon_goodsendtask') . " gst\r\n            inner join " . tablename('ewei_shop_order_goods') . " og on og.goodsid =gst.goodsid  and (orderid=:orderid or parentorderid=:orderid)\r\n            where  og.uniacid=:uniacid and og.openid=:openid and gst.num>=gst.sendnum and gst.status = 1 and  gst.starttime< :now and gst.endtime>:now", array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':orderid' => $orderid,':now'=>time()));

			foreach ($goodssendtasks as $task) {
				$data = array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'taskid' => intval($task['taskid']), 'couponid' => intval($task['couponid']), 'sendnum' => intval($task['total']) * intval($task['sendnum']), 'tasktype' => 2, 'orderid' => intval($task['orderid']), 'parentorderid' => intval($task['parentorderid']), 'createtime' => time(), 'status' => 0, 'sendpoint' => intval($task['sendpoint']),'goodscates'=>$goodscates);
				pdo_insert('ewei_shop_coupon_taskdata', $data);
			...
			$allgoodssendtasks = pdo_fetchall("select gst.*,cp.backrate from " .tablename('ewei_shop_coupon_goodsendtask')." as gst LEFT JOIN ".tablename('ewei_shop_coupon')." as cp ON cp.id=gst.couponid WHERE gst.uniacid=:uniacid AND gst.num >gst.sendnum and gst.status=1 and gst.allgoods=1 and gst.starttime< :now and gst.endtime>:now",array(':uniacid'=>$_W['uniacid'],':now'=>time()));

			foreach ($allgoodssendtasks as $task) {

				$sql="SELECT *  FROM ".tablename('ewei_shop_order')." WHERE uniacid=:uniacid AND parentid=:orderid";
				$order =pdo_fetchall($sql, array(':uniacid'=>$_W['uniacid'],':orderid'=>$orderid));
				$parentorderid=0;
				if (empty($order)) {
					$sql="SELECT *  FROM ".tablename('ewei_shop_order')." WHERE uniacid=:uniacid AND id=:orderid";
					$order =pdo_fetchall($sql, array(':uniacid'=>$_W['uniacid'],':orderid'=>$orderid));
				...
				foreach ($order as $key => $value) {
					...
					if (!empty($value[merchid])) {
						$merch = pdo_fetch("SELECT * FROM ".tablename('ewei_shop_merch_user')." WHERE uniacid={$_W[uniacid]} AND id={$value[merchid]}");
						if (empty($merch['merchtype'])) {
							$fromtype=2;
						}else{
							$fromtype=1;
						}
					...
					$sendamount = ($value['price']-$value['dispatchprice'])*$task['backrate'];
					if (empty($sendamount)) {
						continue;
					}
					$data = array(
					...
					$num = intval($task['num']) -  intval($task['sendnum']);
					pdo_update('ewei_shop_coupon_goodsendtask', array('num' => $num), array('id' => $task['id']));
				}
	...
	public function posterbylist($list, $openid, $gettype)
	{
		...
		foreach ($list as $taskdata) {
			...
			while ($i <= $couponnum) {
				...
				$data = array('uniacid' => $_W['uniacid'], 'openid' => $openid, 'couponid' => $taskdata['couponid'], 'gettype' => intval($gettype), 'gettime' => time(),'amount'=>$taskdata['sendamount'],'fromtype'=>$taskdata['fromtype'],'frommerch'=>$taskdata['frommerch'],'goodscates'=>$taskdata['goodscates']);
				pdo_insert('ewei_shop_coupon_data', $data);
