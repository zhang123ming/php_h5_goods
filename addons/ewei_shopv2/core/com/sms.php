...
class Sms_EweiShopV2ComModel extends ComModel
{
	...
	public function send($mobile, $tplid, $data, $replace = true)
	{
		...
		if ($template['type'] == 'juhe') {
			...
			require_once EWEI_SHOPV2_VENDOR .'aliyun/SignatureHelper.php';
			//获取默认的阿里云参数
			if ($smsset['aliyun_appkey']=='default') {
				$sms_default = pdo_fetch('SELECT aliyun_appkey,aliyun_appsecret FROM ' . tablename('ewei_shop_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => 4));
				$smsset['aliyun_appsecret'] = $sms_default['aliyun_appsecret'];
				$smsset['aliyun_appkey'] = $sms_default['aliyun_appkey'];
			}
			$result = $this->sendAliMsg($mobile,$params,$template['smstplid'],$template['smssign'],$smsset['aliyun_appkey'],$smsset['aliyun_appsecret']);
			$msg = $result->Code;
			if($msg!='OK'){
	...
	public function sendAliMsg($mobile, $message, $templateCode,$SignName,$appkey,$appSecret) 
	{
	    //global $COM;
	    $params = array ();
	    ...
	    $params['TemplateParam'] =json_decode($message,true);
	...
	protected function sms_verify($tplid, $smsset)
	{
		...
		if ($template['type'] == 'juhe') {
			...
			if (empty($smsset['aliyun_appkey'])||empty($smsset['aliyun_appsecret'])) {
				return array('status' => 0, 'message' => '未填写阿里云短信参数');
			}
