...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function query()
	{
		...
		if (!empty($kwd)) {
			$condition .= ' AND (`realname` LIKE :keyword or `nickname` LIKE :keyword or `mobile` LIKE :keyword or `openid` LIKE :keyword or id LIKE :keyword)';
			$params[':keyword'] = '%' . $kwd . '%';
		}

		$ds = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		$levels = array();
		if(!empty($level))$levels = pdo_fetchall('SELECT id from '.tablename('ewei_shop_member_level').' where level>='.$level,array(),'id');
		foreach ($ds as $k=> &$value) {
			$value['nickname'] = htmlspecialchars($value['nickname'], ENT_QUOTES);
			if (!empty($levels)&&!in_array($value['level'], array_keys($levels))) {
				unset($ds[$k]);
			}
		}
