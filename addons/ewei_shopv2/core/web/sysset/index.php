...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function upload_cert($fileinput)
	{
		...
		if (!empty($filename) && !empty($tmp_name)) {
			...
			if ($ext != '.pem') {
				...
				if($fileinput =='sandpay_public_key_file'&& $ext!= '.cer'){
					$errinput = 'cer文件格式错误';
				}else if($ext == '.cer'){
					$filepath = '/addons/ewei_shopv2/cert/sand'.$_W['uniacid'].'.cer';
					move_uploaded_file($tmp_name,IA_ROOT.$filepath);
					return $filepath;
				}
				if($fileinput =='sandpay_private_key_file'&& $ext != '.pfx'){
					$errinput = 'pfx文件格式错误';
				}else if($ext == '.pfx'){
					$filpath =  '/addons/ewei_shopv2/cert/sand'.$_W['uniacid'].'.pfx';
					move_uploaded_file($tmp_name,IA_ROOT.$filpath);
					return $filpath;
				}
				if($fileType['0']=='sandpay'&& !empty($errinput)||$fileType['0'] !='sandpay'){
	...
	public function payset()
	{
		...
		if ($_W['ispost']) {
			...
			if ($_FILES['sandpay_public_key_file']['name']) {
				$sec['sandpay_pay']['public_key'] = $this->upload_cert('sandpay_public_key_file');
			}
			if ($_FILES['sandpay_private_key_file']['name']) {
				$sec['sandpay_pay']['private_key'] = $this->upload_cert('sandpay_private_key_file');
			}
			$sec['sandpay_pay']['mid']=trim($_GPC['data']['sandpay_mid']);
			$sec['sandpay_pay']['appid']=trim($_GPC['data']['sandpay_appid']);
			$sec['sandpay_pay']['key']=trim($_GPC['data']['sandpay_key']);
			$sec['sandpay_pay']['type']=$_GPC['data']['sandpay_type'];
			...
			$data['qukuailian'] = isset($inputdata['qukuailian'])?$inputdata['qukuailian']:array();
			$data['sandpay'] = intval($inputdata['sandpay']);
		...
		$resp = ihttp_get($url);
		// echo "<pre>";
		// var_dump($sec,$sec['sandpay_pay']['public_key']);die;
		include $this->template();
		exit();
	}
	...
	public function member()
	{
		...
		if ($_W['ispost']) {
			...
			$data['chargetype'] = intval($data['chargetype']);
			$data['isopenconsummate'] =intval($data['isopenconsummate']);
			...
			if ($data['consummatetype']=='coupon') {
				if (is_array($_GPC['couponid'])) {
					...
					$data['consummatereward'] =implode(',', $_GPC['couponid']);
				}
			...
			$shop['chargetype'] = $data['chargetype'];
		...
		if ($data['consummatetype']=='coupon') {
			$cpids = explode(',', $data['consummatereward']);
			$coupon = array();
			$cpname = array();
		...
		if (!isset($data['levelname'])) {
			...
			$data['chargetype'] = $shop['chargetype'];
		}
	...
	public function express()
	{
		...
		if ($_W['ispost']) {
			...
			$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_express') . ' WHERE 1 ORDER BY displayorder desc ,id asc');
		}
	...
	public function expressSave()
	{
		...
		if (!empty($_GPC['cate'])) {
			foreach ($_GPC['cate'] as $id => $cate) {
				$data = array('name' => trim($cate['name']), 'displayorder' => intval($cate['displayorder']), 'status' => intval($cate['status']));
				if (!empty($id) && !empty($data['name'])) {
					pdo_update('ewei_shop_express', $data, array('id' => $id));
					plog('sysset.express.edit', '修改物流公司 ID: ' . $id . ' 名称: ' . $data['name']);
				}
			}
		}
		...
		if (!empty($_GPC['cate_new'])) {
			foreach ($_GPC['cate_new'] as $cate_new) {
				...
				pdo_insert('ewei_shop_express', array('name' => $cate_new));
				$insert_id = pdo_insertid();
				plog('sysset.express.edit', '添加物流公司 ID: ' . $insert_id . ' 名称: ' . $cate_new);
			}
		}

		plog('sysset.express.edit', '批量修改分类');
		show_json(1);
	}
}
