...
class Goods_rank_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		if (!empty($_GPC['paytime'])) {
			$starttime = strtotime($_GPC['paytime']['start']);
			$endtime = strtotime($_GPC['paytime']['end']);

			if (!empty($starttime)) {
				$condition .= ' AND o.paytime >= ' . $starttime;
			}

			if (!empty($endtime)) {
				$condition .= ' AND o.paytime <= ' . $endtime . ' ';
			}
		}
		...
		if ($status) {
			$sql = 'SELECT g.id,g.title,g.thumb,' . '(select ifnull(sum(og.price),0) from  ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id  where o.status='.$status.' and og.goodsid=g.id ' . $condition . ')  as money,' . '(select ifnull(sum(og.total),0) from  ' . tablename('ewei_shop_order_goods') . ' og left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id  where o.status='.$status.' and og.goodsid=g.id ' . $condition . ') as count  ' . 'from ' . tablename('ewei_shop_goods') . ' g  ' . 'where 1 ' . $condition1 . '  order by ' . $orderby . ' desc ';
		...
		if ($_GPC['export'] == 1) {
			...
			$list[] = array('data' => '商品销售排行');
			$saletotal = $moneytotal = 0;
			foreach ($list as &$row) {
				$saletotal +=$row['count'];
				$moneytotal +=$row['money'];
				$row['marketprice']=$row['money']/$row['count'];
				$row['createtime'] = date('Y-m-d H:i', $row['createtime']);
			}

			
			$list[]=array('title'=>'合计','count' =>$saletotal,'money'=>$moneytotal);
			unset($row);
			m('excel')->export($list, array(
			'title'   => '商品销售报告-' . date('Y-m-d-H-i', time()),
			'columns' => array(
				array('title' => '商品名称', 'field' => 'title', 'width' => 36),
				array('title' => '单价', 'field' => 'marketprice', 'width' => 12),
				array('title' => '合计数量', 'field' => 'count', 'width' => 12),
