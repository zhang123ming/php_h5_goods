...
class Set_EweiShopV2Page extends ComWebPage
{
	...
	'verify' => array('keyword' => $keyword, 'type' => $type, 'store' => $_GPC['post'])
	));
	...
	'verify' => array('keyword' => $keyword, 'type' => $type)
		...
		$store = $verify['store'];
		//var_dump($kmdStore);die();
		$storeurl = mobileUrl('store/storelist',NULL, true);
		$storeview = m('qrcode')->createQrcode($storeurl);
