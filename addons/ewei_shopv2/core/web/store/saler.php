...
class Saler_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		if (!empty($_GPC['keyword'])) {
			...
			$condition .= ' and ( s.salername like :keyword or m.realname like :keyword or m.mobile like :keyword or m.nickname like :keyword or store.storename like :keyword)';
	...
	protected function post()
	{
		...
		if (!empty($item)) {
			...
			$wxappsaler=m('member')->getMember($item['wxappopenid']);
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'storeid' => intval($_GPC['storeid']), 'openid' => trim($_GPC['openid']), 'status' => intval($_GPC['status']), 'salername' => trim($_GPC['salername']), 'mobile' => trim($_GPC['mobile']), 'roleid' => intval($_GPC['roleid']),'wxappopenid'=>trim($_GPC['wxappopenid']));
			...
			pdo_update('ewei_shop_member',array('store_id'=>$data['storeid']),array('uniacid'=>$_W['uniacid'],'openid'=>$data['openid']));
			if (!empty($_GPC['wxappopenid'])) {
				pdo_update('ewei_shop_member',array('store_id'=>$data['storeid']),array('uniacid'=>$_W['uniacid'],'openid'=>$data['wxappopenid']));
			}
