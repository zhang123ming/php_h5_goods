...
class Index_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		if(!empty($_GPC['distributor'])){

			$distributor = intval($_GPC['distributor']);

			if ($distributor=='1') {
		
				$condition .=" AND distributor <>''";

			}elseif ($distributor=='2') {
		...
		if (!empty($_GPC['saler'])) {
			...
			$sql= "SELECT storeid FROM ".tablename('ewei_shop_saler')." WHERE uniacid=:uniacid and storeid <> '0'  GROUP BY storeid";

			$salerList =pdo_fetchall($sql,array(":uniacid"=>$_W['uniacid']));

			$salerList = implode(',', array_column($salerList,'storeid'));
			if($saler==1){

				$condition .=" and id in ({$salerList})";

			}elseif($saler==2){

				$condition .=" and id not in ({$salerList})";
			}
		}
		...
		foreach ($list as $key=> &$row) {
			...
			$row['distributor'] = m('member')->getMember($row['distributor']);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data['distributor'] =$_GPC['distributor'];
		...
		$distributor=m('member')->getMember($item['distributor']);
