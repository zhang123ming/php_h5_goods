...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function change()
	{
		...
		$goods = pdo_fetch('select id,hasoption,marketprice,dowpayment,total,status from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if($type=='total' && $_W['uniacid'] && $goods['total']=='0' && $goods['status']=='1' && $value>'0'){
			$this->sendAddGroup($id);
		}
	...
	//小程序购物提醒
	public function sendAddGroup($id=0)
	{
		global $_W,$_GPC;
		$sets = m('common')->getSysset(array('app', 'pay'));
		$appid=$sets['app']['appid'];
		$secret=$sets['app']['secret'];
		// $id = $_GPC['id'];
		// $id = 16073;
		$goods = pdo_fetch('select id,title from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id=:id limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$id));

		$remind = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_remind') . ' where goodsid=:goodsid and uniacid=:uniacid ',array(':uniacid'=>$_W['uniacid'],':goodsid'=>$id));
		// var_dump($remind);die;
		if(!empty($remind)){
			foreach ($remind as $key => $value) {
				$info = pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $value['openid']));
				$openid = $value['openid'];
				if (!empty($info)) 
				{
				...
				$access_token = ihttp_request($tokenUrl);
				$access_token = json_decode($access_token['content'],true)['access_token'];
				$data = array(
					'touser'=>$openid,
					'msgtype'=>'text',
					'text'=>array(
						'content'=>'到货通知'.$goods['title'].'已到货，请进入小程序查看'
					)
				);
				$data = json_encode($data,JSON_UNESCAPED_UNICODE);
				$send_kefu = $this->postCurl($access_token,$data);
			}
		}
	}

	public function postCurl($access_token,$param)
	{
