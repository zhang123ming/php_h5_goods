...
class Group_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			...
			$buylimit = unserialize($item['buylimit']);
		...
		if ($_W['ispost']) {
			...
			$buylimit = serialize($_GPC['buylimit']);
			$isbuylimit = intval($_GPC['isbuylimit']);
			...
			$data = array('name' => $groupname, 'merchid' => 0, 'goodsids' => implode(',', $goodsids), 'enabled' => $enabled, 'buylimit' => $buylimit, 'isbuylimit' => $isbuylimit);

			if (!empty($item)) {
				$count = pdo_update('ewei_shop_goods_group', $data, array('id' => $item['id']));
	...
	public function buylimit(){
		global $_W,$_GPC,$_S;
		$set = $_S['commission'];
		$defaultlevel = array('id' => '0', 'levelname' => empty($set['levelname']) ? '默认等级' : $set['levelname'],'level' => '0');
		$levels = m('member')->getLevels();
		$levels[] = $defaultlevel;

		exit(json_encode(array('level' => $levels)));;
	}
}
