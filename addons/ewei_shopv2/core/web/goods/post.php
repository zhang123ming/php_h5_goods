...
if(!$_GPC['docopy']) $id = intval($_GPC['id']);
//商品升级礼包等级
$levelnumber = pdo_fetchall('select level,levelname from'.tablename('ewei_shop_member_level').'where uniacid = :uniacid',array(':uniacid'=>$_W['uniacid']));
$commissionnumber = pdo_fetchall('select level,levelname from'.tablename('ewei_shop_commission_level').'where uniacid = :uniacid',array(':uniacid'=>$_W['uniacid']));
if (!(empty($id)))
{
...
//解序列化商品升级礼包等级
$item['upgradebag'] = iunserializer($item['upgradebag']);
...
$commission_agent=iunserializer($item['commission_agent']);
if (p('commission'))
{
...
$category = m('shop')->getFullCategory(true);
...
//解序列化会员等级单次购买限制
$item['limitbuy'] = unserialize($item['limitbuy']);

//会员单次购买限制
$levellimit = [];
foreach ($levels as $key => $value) {
	if($value['key']=='default') $value['id'] = 0;
	$levellimit[$key] = $value;//会员等级数据
	$levellimit[$key]['maxlimit'] = 'maxlimit'.$value['id'];//最大
	$levellimit[$key]['minlimit'] = 'minlimit'.$value['id'];//最小
}
...
if ($_W['ispost'])
{
	...
	$data['fixed_pay'] = $fixed_pay;
	$data['fixed_pay'] = $_GPC['fixed_pay'];
	$data['fixed_money'] =$_GPC['fixed_money'];
	$data['isupgradebag'] = intval($_GPC['isupgradebag']);//商品是否为升级礼包
	$data['limitbuy'] = serialize($_GPC['limitbuy']);//会员单次购买限制
	empty($data['isupgradebag'])?$data['upgradebag'] = '0':$data['upgradebag'] = iserializer($_GPC['upgradebag']);//升级礼包对应等级
	...
	$data['protocolcontent'] = m('common')->html_images($_GPC['protocolcontent']);
	if (p('commission'))
	{
		...
		if (!(empty($cset['level'])))
		{
			...
			empty($data['hascommission'])?$data['commissionagent'] = 0:$data['commissionagent'] = intval($_GPC['commissionagent']);//管理奖返利开关
			empty($data['hascommission'])?$data['commissionself'] = 0:$data['commissionself'] = intval($_GPC['commissionself']);//自购返利开关
			...
			$data['level1_rate'] = $_GPC['level1_rate'];
			$data['level2_rate'] = $_GPC['level2_rate'];
			$data['level3_rate'] = $_GPC['level3_rate'];
			$data['commission_thumb'] = save_media($_GPC['commission_thumb']);
			$data['commission_agent'] = serialize($_GPC['commission_agent']);
			$data['has_commission_agent'] = !empty($_GPC['has_commission_agent'])&&!empty($data['commissionagent']) ? 1 :0;
			
		}
	}

	if (p('abonus')) {
		$abonus_set = p('abonus')->getSet();
		$data['abonus_rate'] = serialize($_GPC['abonus']);
	}
	if ($ccard_plugin)
	{
		if ($goodstype == 20)
	...
	while ($k < $len)
	{
		...
		$param = pdo_fetch('select * from ' . tablename('ewei_shop_goods_param') . ' where uniacid=:uniacid and id=:id and goodsid=:goodsid limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$get_param_id,':goodsid'=>$id));
		if (!(is_numeric($get_param_id)) || empty($param))
		{
	...
	while ($k < $len)
	{
		...
		if (is_numeric($get_spec_id) && (!$_GPC['docopy']))
		{
		...
		while ($n < $itemlen)
		{
			...
			if (is_numeric($get_item_id) && (!$_GPC['docopy']))
			{
	...
	while ($k < $len)
	{
		...
		$totalstocks += $a['stock'];
		if (empty($get_option_id) && (!$_GPC['docopy']))
		{
...
if (!(empty($id)))
{
	...
	$abonus = unserialize($item['abonus_rate']);
	...
	if (0 < count($options))
	{
		...
		if ($canedit)
		{
			...
			$html .= '<th><div class=""><div style="padding-bottom:10px;text-align:center;">预付价</div><div class="input-group"><input type="text" class="form-control  input-sm option_preprice_all"  VALUE=""/><span class="input-group-addon"><a href="javascript:;" class="fa fa-angle-double-down" title="批量设置" onclick="setCol(\'option_preprice\');"></a></span></div></div></th>';
		...
		while ($i < $newlen)
		{
			...
			foreach ($options as $o )
			{
				while ($ids === $o['specs'])
				{
					$val = array('id' => $o['id'], 'title' => $o['title'], 'stock' => $o['stock'], 'costprice' => $o['costprice'], 'productprice' => $o['productprice'], 'presell' => $o['presellprice'],'preprice'=>$o['preprice'],'period'=>$o['period'],'marketprice' => $o['marketprice'], 'goodssn' => $o['goodssn'], 'productsn' => $o['productsn'], 'weight' => $o['weight'], 'virtual' => $o['virtual']);
			...
			if ($canedit)
			{
				...
				$hh .= '<td><input data-name="option_preprice_' . $ids . '" type="text" class="form-control option_preprice option_preprice_' . $ids . '" value="' . $val['preprice'] . '"/></td>';
