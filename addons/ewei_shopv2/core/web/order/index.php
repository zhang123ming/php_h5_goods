...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		if($price>10000&&$diycount['order']>1){
			$price=$price*$diycount['order'];
		}
		...
		if (!empty($pdo_fetchall)) {
			...
			foreach ($pdo_fetchall as $key => $value) {
				if (array_key_exists(date('Y-m-d', $value['createtime']), $transaction['price'])) {
					$transaction['price'][date('Y-m-d', $value['createtime'])] += $value['price']*$diycount['order'];
	...
	/**
	 * ajax return 统计各个省的订单总数
	 */
	public function ajaxall()
	{
		$arr = $this->ajaxprovince();
		echo json_encode(array('ajaxprovince' => $arr['ajaxprovince'],'max'=>$arr['max']));
	}


	/**
	 * ajax return 订单数据所属省份
	 */
	protected function ajaxprovince()
	{
		global $_W;
		$province = pdo_fetchall('select province,count(province) as province_num from ' . tablename('ewei_shop_member_address') . ' m left join ' . tablename('ewei_shop_order') . ' o on m.id=o.addressid where o.uniacid=:uniacid GROUP BY m.province',array(':uniacid'=>$_W['uniacid']));
		$result = array();
		$max = 0;
		...
		foreach ($province as $array) {
			...
			if($max < $array['province_num']){
				$max = $array['province_num'];
			}
		...
		return array('ajaxprovince'=>$result,'max'=>$max);
	}
