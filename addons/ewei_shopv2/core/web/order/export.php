...
class Export_EweiShopV2Page extends WebPage
{
	...
	protected function defaultColumns()
	{
		...
		$ordermatchs=$_S['shop']['ordermatchs'];
		if (is_array($ordermatchs)){
			$columnsL=array();
			foreach ($dc as $c) {
				if($ordermatchs[$c['field']]){
					$c['title']=$c['subtitle']=$ordermatchs[$c['field']];
				}
				$columnsL[]=$c;
			}
		}else{
	...
	public function main()
	{
		...
		$ordermatchs=$shop_set['ordermatchs'];
		$columnsL=array();
		foreach ($columns as $c) {
			if($ordermatchs[$c['field']]){
				$c['title']=$c['subtitle']=$ordermatchs[$c['field']];
	...
	array('title' => '商品价格(折扣后)', 'field' => 'goods_rprice2', 'width' => 12)
		...
		if($_GPC['show']){
			//print_r($shop_set['ordercolumns']);
			//var_dump($shop_set['ordermatchs']);
			die();
		}
	...
	public function match()
	{
		...
		$matchs=$_GPC['post'];
		$data['ordermatchs']=$matchs;
		m('common')->updateSysset(array('shop' => $data));
		$url = webUrl('order/export');
		$this->message("操作成功", $url, '');
	}
