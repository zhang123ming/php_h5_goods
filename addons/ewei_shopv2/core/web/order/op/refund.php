...
class Refund_EweiShopV2Page extends WebPage
{
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($refund['refundno'])||$refund['status'] ==0) {
			...
			if ($refundstatus == 0) {
				...
				if (0 < $item['parentid']) {
					...
					// var_dump($item['isCash'],$item['earnest']);die;

					// if ($item['isCash']==1) {
						
					// 	$order_price = $item['earnest'];
					// }else{
				...
				if ($item['paytype'] == 1) {
					...
					$sandrefund = m('common')->sandpay_refund(array('ordersn'=>$ordersn,'refundsn'=>$refund['refundno'],'total_fee'=>$order_price*100),$sec['sandpay_pay']);
					if($sandrefund['success']){
						$result = true;
					}else{
						show_json(0, $sandrefund['msg']);
					}
				}
			...
			if($refundstatus>0){
				m('member')->cancelcommission($item['id']);
			}
