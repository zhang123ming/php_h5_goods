...
class Detail_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		if (empty($item['addressid'])) {
			...
			if($_W['shopset']['shop']['isuserid'] == '1'){
				$userthumb = pdo_fetch('SELECT userid,justthumb,backthumb FROM ' . tablename('ewei_shop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
			}
		...
		/*查找订单返利*/
		$order_log = pdo_fetchall('SELECT l.*,m.nickname,m.avatar,m.id as mid, m.realname,og.optionid FROM ' . tablename('ewei_shop_commission_order_log') . ' l left join '.tablename('ewei_shop_member').' m on l.openid=m.openid and l.uniacid=m.uniacid ' . ' left join ' . tablename('ewei_shop_order_goods') . ' og on l.uniacid=og.uniacid and l.goodsid=og.goodsid and l.orderid=og.orderid ' . ' WHERE l.uniacid =:uniacid and l.orderid=:orderid and l.from_type="shop"', array(':uniacid' => $_W['uniacid'],':orderid'=>$id));
		foreach ($order_log as &$log) {
			$og = pdo_fetch('select id,title from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id=:id limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$log['goodsid']));
			$log['title'] = $og['title'];
			if(!empty($log['optionid'])){
				$op = pdo_fetch('select id,title from ' . tablename('ewei_shop_goods_option') . ' where uniacid=:uniacid and id=:id and goodsid=:goodsid limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$log['optionid'],':goodsid'=>$log['goodsid']));
				$log['optionname'] = $op['title'];
			}
		}
		unset($log);
		/*返利奖励end*/
