...
class List_EweiShopV2Page extends WebPage
{
	protected function orderData($status, $st)
	{
		...
		if ($_GPC['paytype'] != '') {
			...
			}else if($_GPC['paytype'] == '3'){
				$condition .= ' AND isCash=1';
			}
			else {
		...
		if($totalmoney>10000&&$diycount['order']>1){
			$totalmoney=$totalmoney*$diycount['order'];
		}
	...
	public function prepay()
	{
		...
		$orderData = $this->orderData('','prepay');
