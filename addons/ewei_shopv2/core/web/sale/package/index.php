...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($_GPC['goodsid'])&&$_GPC['ispackage']==0) {
				show_json(0, '套餐商品不能为空！');
			}
			else {
				if($_GPC['ispackage']==1){
					$goodsid = $data['categoryid'];
					$data['categoryid'] = is_array($_GPC['categoryid']) ? implode(',', $_GPC['categoryid']) : 0;
				}else{
			...
			foreach ($goodsid as $key => $value) {
				if($_GPC['ispackage']==1){
					$good_data = pdo_fetch("select name,advimg\r\n                            from " . tablename('ewei_shop_category') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
				}else{
				...
				if ($good_data['hasoption'] && empty($good_data['option'] || $_GPC['ispackage']!=1)) {
					show_json(0, '请选择商品规格！');
				}
			}
			...
			if (!empty($id)) {
				...
				foreach ($goodsid as $key => $value) {
					...
					if($_GPC['ispackage']==1){
						$nd = pdo_fetch("select name,advimg,thumb\r\n                            from " . tablename('ewei_shop_category') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
						$good_data = pdo_fetch("select title,thumb,marketprice,goodssn,productsn,hasoption\r\n                            from " . tablename('ewei_shop_goods') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
						$good_data['title'] = $nd['name'];
						if($nd['advimg']){
							$good_data['thumb'] = save_media($nd['advimg']);
						}else{
							$good_data['thumb'] = save_media($nd['thumb']);
						}
						$good_data['categoryid'] = $value;
					}else{
						$good_data = pdo_fetch("select title,thumb,marketprice,goodssn,productsn,hasoption\r\n                            from " . tablename('ewei_shop_goods') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
				...
				foreach ($goodsid as $key => $value) {
					...
					if($_GPC['ispackage']==1){
						$good_data['categoryid'] = $value;
						$category = pdo_fetch("select advimg,name,thumb \r\n                            from " . tablename('ewei_shop_category') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
						if($category['thumb']){							
							$good_data['thumb'] = save_media($category['thumb']);
						}else{
							$good_data['thumb'] = save_media($category['advimg']);
						}
						$good_data['title'] = $category['name'];
		...
		if($item['ispackage']==1){
			if ($item) {
				...
				$category = array();
				$category = pdo_fetchall("select id,pid,title,thumb,packageprice,hasoption,goodsid,`option`,commission1,commission2,commission3,categoryid,categorystatus\r\n                            from " . tablename('ewei_shop_package_goods') . ' where pid = ' . $id . ' and uniacid = ' . $uniacid . ' ');
	...
	public function categoryquery()
	{
		...
		$ds = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_category') . "WHERE 1 " . $condition . ' ORDER BY id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);

		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_category') . ' WHERE 1 ' . $condition . ' ', $params);
		$pager = pagination($total, $pindex, $psize, '', array('before' => 5, 'after' => 4, 'ajaxcallback' => 'select_page', 'callbackfuncname' => 'select_page'));
		$ds = set_medias($ds, array('thumb'));
		include $this->template();
	}
