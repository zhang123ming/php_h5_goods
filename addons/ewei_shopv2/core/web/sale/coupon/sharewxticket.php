...
class Sharewxticket_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			$getmax=$_GPC['getmax']?$_GPC['getmax']:0;
			$data = array('uniacid' => intval($_W['uniacid']), 'cpid' => trim($cpids), 'expiration' => intval($_GPC['expiration']), 'status' => intval($_GPC['status']), 'createtime' => TIMESTAMP,'getmax'=>$getmax);
			...
			if (empty($id)) {
				pdo_insert('ewei_shop_wxapp_sendticket', $data);
				plog('sendticket.set', '设置发券内容');
			}
			else {
				$params = array('id' => $id);
				pdo_update('ewei_shop_wxapp_sendticket', $data, $params);
			}
		...
		$sql = 'SELECT * FROM ' . tablename('ewei_shop_wxapp_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
		$item = pdo_fetch($sql);
