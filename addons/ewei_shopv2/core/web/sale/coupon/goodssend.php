...
class Goodssend_EweiShopV2Page extends ComWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (intval($_GPC['allgoods'])==0&&empty($_GPC['goodsid'])) {
				show_json(0, '没有选择商品');
			}
			

			$data = array('uniacid' => $uniacid, 'goodsid' => floatval($_GPC['goodsid']), 'couponid' => intval($_GPC['couponid']), 'starttime' => strtotime($_GPC['sendtime']['start']), 'endtime' => strtotime($_GPC['sendtime']['end']) + 86399, 'sendnum' => intval($_GPC['sendnum']), 'num' => intval($_GPC['num']), 'sendpoint' => intval($_GPC['sendpoint']), 'status' => intval($_GPC['status']),'allgoods'=>intval($_GPC['allgoods']));
			if ($data['allgoods']) {
				$data['goodsid'] = 0;
			}
			if (!empty($id)) {
