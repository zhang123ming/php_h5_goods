...
class Log_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('ewei_shop_coupon_data') . ' d ' . ' left join ' . tablename('ewei_shop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . ' where  1 and ' . $condition . ' ORDER BY id DESC';
		...
		foreach ($list as &$row) {
			...
			if ($row['gettype'] == 0) {
				...
				$row['gettypestr'] = ''.$_W['shopset']['trade']['credittext'].'商城';
			}
