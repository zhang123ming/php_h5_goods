...
class Index_EweiShopV2Page extends ComWebPage
{
	...
	'trade' => array('credit' => 0)
			...
			$data['firstpay'] = $_GPC['firstpay'];
			$data['sendtype'] = intval($_GPC['sendtype']);
			$data['give_every'] = $_GPC['give_every'];
			$data['enough1'] = $enough1;
	...
	'sale' => array('credit1' => iserializer($data))
			...
			plog('sale.credit1.edit', '修改基本'.$_W['shopset']['trade']['credittext'].'活动配置');
		...
		$give_every = (empty($credit1['give_every']) ? "" : $credit1['give_every']);
		$firstpay = $credit1['give_every'];
		include $this->template();
	}
	...
	public function birthday()
    {
		...
		if ($_W['ispost']) {
			$data = (is_array($_GPC['data']) ? $_GPC['data'] : array());
			$data['birthday'] = intval($data['birthday']);
			$data['birthdayDiscount'] = $data['birthdayDiscount'];
			// $data['goodsids'] = $_GPC['goodsids'];
			plog('sale.birthday', '修改生日优惠');
			m('common')->updatePluginset(array('sale' => $data));
			show_json(1);
		}
