...
class Index_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $uniacid, 'displayorder' => intval($_GPC['displayorder']), 'type' => intval($_GPC['type']), 'titles' => trim($_GPC['titles']), 'thumb' => save_media($_GPC['thumb']), 'marketprice' => floatval($_GPC['marketprice']), 'goodsid' => floatval($_GPC['goodsid']), 'startday' => intval($_GPC['startday']), 'refund' => intval($_GPC['refund']), 'status' => intval($_GPC['status']),'credit'=>intval($_GPC['credit']));
