...
class EweiShopWechatPay
{
	...
	public function recharge()
	{
		...
		if ($OK) {
			...
			m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $shopset['name'] . '会员微信充值余额:' . $log['money']));
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			m('member')->setRechargeTimes($log['openid'], $log['money']);
	...
	public function wxapp_recharge()
	{
		...
		if ($OK) {
			...
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			m('member')->upgradeLevelByRercharge($log['openid'],$log['money']);
			com_run('sale::setRechargeActivity', $log);
			com_run('coupon::useRechargeCoupon', $log);
			m('notice')->sendMemberLogMessage($log['id']);
		}
