...
class Temp_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			...
			$adcontent = iunserializer($item['adcontent']);
		...
		if ($_W['ispost']) {
			...
			$insert = array('uniacid' => $_W['uniacid'], 'cate' => intval($_GPC['cate']), 'title' => trim($_GPC['tp_title']), 'adpic' => trim($_GPC['adpic']),'adurl' => trim($_GPC['adurl']),'adcontent'=>iserializer($_GPC['adcontent']),'fields' => iserializer($data));
