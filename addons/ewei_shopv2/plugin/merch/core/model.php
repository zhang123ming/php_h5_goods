...
class MerchModel extends PluginModel
{
	...
	{
		global $_W;
		$condition = ' WHERE `uniacid` = :uniacid and `logo` != "" and `tel` != "" and `lng` != "" and `merchname` != ""';
		$params = array(':uniacid' => $_W['uniacid']);
		$res = $this->build($condition, $params, $data);
	...
	public function getSet($name = '', $merchid = 0, $refresh = false)
	{
		...
		if (!(empty($name)) && empty($allset[$name])||empty($allset)) {
	...
	public function getMerchPrice($merchid, $flag = 0)
	{
		...
		if (0 < $flag) {
			...
			$sql = 'select o.id,o.agentid,o.merchid from ' . tablename('ewei_shop_merch_user') . ' u ' . ' left join ' . tablename('ewei_shop_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition;
			...
			if (!(empty($order))) {
				...
				foreach ($order as $k => $v ) {
					...
					$orderids[] = $v['id'];
					if($merch_data['is_member']==1 && $v['merchid']>0){
						$commission += m('order')->getOrderCommission($v['id'], $v['agentid'],1);
					}else{
		...
		if ($deduct_commission||($merch_data['is_member']==1 && $list['merchid']>0)) {
			$list['realprice'] -= $list['commission'];
		}
		if($merch_data['is_member']==1 && $list['merchid']>0){
				$list['realprice'] -= $list['discountprice'];
		}
		if (!empty($merch_data['deduct_dispatchprice'])) {
	...
	public function getMerchPriceList($merchid, $orderid = 0, $flag = 0)
	{
		...
		foreach ($order as &$list ) {
			...
			if($merch_data['is_member']==1 && $list['merchid']>0){
				$list['commission'] += m('order')->getOrderCommission($list['id'], $list['agentid'],1);
			}else{
				$list['commission'] += m('order')->getOrderCommission($list['id'], $list['agentid']);
			}
			$list['orderprice'] = $list['goodsprice'] + $list['dispatchprice'] + $list['changeprice'];
			$list['realprice'] = $list['orderprice'] - $list['merchdeductenough'] - $list['merchisdiscountprice'] - $merchcouponprice - $list['seckilldiscountprice']-$list['dispatchprice'];
			if ($deduct_commission || ($merch_data['is_member']==1 && $list['merchid']>0)) {
			...
			$list['realprice'] = round($list['realprice'],2);
			$list['realpricerate'] = ((100 - floatval($list['payrate'])) * $list['realprice']) / 100;
			$list['merchcouponprice'] = $merchcouponprice;
		}
