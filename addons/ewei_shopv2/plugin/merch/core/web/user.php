...
class User_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$url='';
		$accessToken = p('app')->getAccessToken();
		foreach ($list as &$row ) {
			if (!is_error($accessToken)) {
				$url = "pages/changce/merch/detail?id=".$row['id'];
				$row['wxurl']=m('qrcode')->createWxCode($url,$row['logo']);
			}

			$row['merchurl'] = mobileUrl('paymentcode', array('merchid' => $row['id']),true);
			$row['merchpay'] = m('qrcode')->createQrcode($row['merchurl']);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data['diypage']=intval($_GPC['diypage']);
			if ($diyform_flag) {
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,merchname,openid FROM ' . tablename('ewei_shop_merch_user') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
