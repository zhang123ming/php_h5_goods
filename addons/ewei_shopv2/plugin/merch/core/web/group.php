...
class Group_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = array('uniacid' => $_W['uniacid'], 'groupname' => trim($_GPC['groupname']), 'status' => intval($_GPC['status']), 'isdefault' => intval($_GPC['isdefault']), 'goodschecked' => intval($_GPC['goodschecked']), 'commissionchecked' => intval($_GPC['commissionchecked']),'memberdiscount'=>intval($_GPC['memberdiscount']), 'changepricechecked' => intval($_GPC['changepricechecked']), 'finishchecked' => intval($_GPC['finishchecked']));
			if ($data['isdefault'] == 1) 
			{
