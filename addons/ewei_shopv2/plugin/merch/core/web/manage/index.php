...
class Index_EweiShopV2Page extends MerchWebPage
{
	public function main()
	{
		...
		$merchurl = mobileUrl('paymentcode', array('merchid' => $user['id']), true);
		$merchcode = m('qrcode')->createQrcode($merchurl);
		if (p('app')) {
			$wxurl = "pages/changce/merch/detail?id=".$user['id'];
			$xcxcode = m('qrcode')->createWxCode($wxurl,$user['logo']);
		}

		//$xcxcode = $this->get_wxapp_store_qrcode();

		require $this->template('shop/index');
	}
	...
	// 	$url = $_W['grouplevel'].'images/mini_qrcode_'.$merchid.'.png';
	// 	// if($_SERVER['REMOTE_ADDR']=='219.136.94.93'){
	// 	// 	echo "<pre>";
	// 	// var_dump($_W);die;}
	// //var_dump($_W);die;
	//     return $url;
