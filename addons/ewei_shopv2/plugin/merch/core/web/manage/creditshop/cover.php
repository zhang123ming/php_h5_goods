...
class Cover_EweiShopV2Page extends MerchWebPage 
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'ewei_shopv2'.$_W['shopset']['trade']['credittext'].'商城入口设置'));
		if (!(empty($rule))) 
		{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($keyword))) 
			{
				if ($keyword['name'] != 'ewei_shopv2'.$_W['shopset']['trade']['credittext'].'商城入口设置') 
				{
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2'.$_W['shopset']['trade']['credittext'].'商城入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			plog('creditshop.cover.edit', '修改'.$_W['shopset']['trade']['credittext'].'商城入口设置');
			show_json(1);
		}
