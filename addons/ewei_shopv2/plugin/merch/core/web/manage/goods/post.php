...
if($merchset['is_member']==1){
	$merchlevels = m('member')->getmerch_Levels($_W['merchid']);
	if(empty($merchlevels)){
...
unset($l);
$levels = array_merge(array( array('id' => 0, 'key' => 'merch', 'levelname' => (empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])) ), $levels);
if (0 < $maxgoods) {
...
if ($_W['ispost']) {
	...
	if($merch_user['memberdiscount']){
		$data['discounts'] = json_encode($_GPC['discounts']);
	}
	if($merchset['is_member']==1){
		$data['commissions'] = json_encode($_GPC['commissions']);
	}
	...
	if (0 < count($options)) {
		...
		while ($i < $newlen)
		{
			...
			foreach ($options as $o )
			{
				while ($ids === $o['specs'])
				{
					$val = array('id' => $o['id'], 'title' => $o['title'], 'stock' => $o['stock'], 'costprice' => $o['costprice'],'period' => $o['period'], 'productprice' => $o['productprice'], 'presell' => $o['presellprice'],'preprice'=>$o['preprice'],'marketprice' => $o['marketprice'], 'goodssn' => $o['goodssn'], 'productsn' => $o['productsn'], 'weight' => $o['weight'], 'virtual' => $o['virtual']);
