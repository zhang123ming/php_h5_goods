...
class Saler_EweiShopV2Page extends MerchWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = array('uniacid' => $_W['uniacid'], 'storeid' => intval($_GPC['storeid']), 'openid' => trim($_GPC['openid']), 'status' => intval($_GPC['status']), 'salername' => trim($_GPC['salername']), 'merchid' => $_W['merchid'],'wxappopenid' => $_GPC['wxappopenid']);
			...
			pdo_update('ewei_shop_member',array('store_id'=>$data['storeid'],'shop_id'=>$_W['merchid']),array('uniacid'=>$_W['uniacid'],'openid'=>$data['openid']));

			if (!empty($_GPC['wxappopenid'])) {
				pdo_update('ewei_shop_member',array('store_id'=>$data['storeid'],'shop_id'=>$_W['merchid']),array('uniacid'=>$_W['uniacid'],'openid'=>$data['wxappopenid']));
			}
