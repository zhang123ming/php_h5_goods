...
class Login_EweiShopV2Page extends Page 
{
	public function main() 
	{
		...
		if (is_int($_GET['pic']) || empty($_GET['pic']) || $_GET['pic'] > count($set['regpic'])-1 || $_GET['pic'] < 0){ 
            $bgimg = tomedia($set['regpic'][0])?tomedia($set['regpic'][0]):'../addons/ewei_shopv2/plugin/merch/template/mobile/default/static/images/regbg.png';
        }else{
             $bgimg = tomedia($set['regpic'][$_GET['pic']]);
        }
