...
if ($_W['routes'] != 'merch.manage.login') 
{
	...
	{
		isetcookie($session_key, false, -100);
		header('location: ' . merchurl('login').'&pic='.$_GET['pic']);
		exit();
	}
}
...
$merch_user = pdo_fetch('select u.*,g.groupname,g.goodschecked,g.commissionchecked,g.memberdiscount,g.changepricechecked,g.finishchecked from ' . tablename('ewei_shop_merch_user') . ' u left join ' . tablename('ewei_shop_merch_group') . ' g on u.groupid=g.id where u.id=:id limit 1', array(':id' => $session['merchid']));
