...
class User_EweiShopV2Page extends SnsMobilePage
{
	public function main()
	{
		...
		$boardid = pdo_fetchAll('select bid from ' . tablename('ewei_shop_sns_board_follow') . ' where uniacid=:uniacid and openid=:openid', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		$boardcount = 0;
		foreach($boardid as $v){
			$board = $this->model->getboard($v['bid']);
			if($board['status'] == 1){
				$boardcount +=1;
			}
		}
