...
class Post_EweiShopV2Page extends SnsMobilePage
{
	public function main()
	{
		...
		$isManager = $this->model->isManager($board['id'],$post['openid']);
		$self_isManager = $this->model->isManager($board['id']);
	...
	public function getlist()
	{
		...
		$sql = 'select id,bid,pid,rpid,title,createtime,content,images ,openid, nickname,avatar,checked from ' . tablename('ewei_shop_sns_post') . '  where 1 ' . $condition . ' ORDER BY createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		...
		foreach ($list as $key => &$row) {
			...
			$author = $this->model->getPost($row['pid']);
			if($author['openid'] == $row['openid']){
				$row['isAuthor'] =  1;
			}else{
				$row['isAuthor'] = 0;
			};
