...
if (!class_exists('SnsModel')) {
	class SnsModel extends PluginModel
	{
		...
		public function isManager($bid = 0, $openid = '')
		{
			...
			// var_dump('select count(*)  from ' . tablename('ewei_shop_sns_manage') . ' where uniacid=:uniacid and bid=:bid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':bid' => $bid, ':openid' => $openid));die();
