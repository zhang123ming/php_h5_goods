...
class Goods_EweiShopV2Page extends PluginWebPage
{
	...
	array('id' => 0, 'key' => 'default', 'levelname' => empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])
		...
		if (0 < count($options)) {
			...
			if ($canedit) {
				...
				$html .= '<th"><div class=""><div style="padding-bottom:10px;text-align:center;">'.$_W['shopset']['trade']['credittext'].'</div></div></th>';
		...
		if ($_W['ispost']) {
			...
			if ($data['credit'] <= 0) {
				show_json(0, '请正确填写'.$_W['shopset']['trade']['credittext'].'！');
			}
			...
			if (!empty($id)) {
				...
				plog('creditshop.goods.edit', '编辑'.$_W['shopset']['trade']['credittext'].'商城商品 ID: ' . $id . ' <br/>商品名称: ' . $data['title']);
			}
			else {
				...
				$id = pdo_insertid();
				plog('creditshop.goods.add', '添加'.$_W['shopset']['trade']['credittext'].'商城商品 ID: ' . $id . '  <br/>商品名称: ' . $data['title']);
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('creditshop.goods.delete', '删除'.$_W['shopset']['trade']['credittext'].'商城商品 ID: ' . $item['id'] . '  <br/>商品名称: ' . $item['title'] . ' ');
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('creditshop.goods.edit', '修改'.$_W['shopset']['trade']['credittext'].'商城商品 ' . $item['id'] . ' <br /> 状态: ' . ($status == 0 ? '下架' : '上架'));
		}
	...
	public function property()
	{
		...
		if (in_array($type, array('istop', 'isrecommand', 'status', 'displayorder', 'title'))) {
			...
			plog('creditshop.goods.edit', '修改'.$_W['shopset']['trade']['credittext'].'商城商品' . $typestr . '状态   ID: ' . $id . ' ' . $statusstr . ' ');
		}
