		...
		if ($_W['ispost']) {
			if ($goods['goodstype'] == 0) {
				...
				if ($goods['goodstype'] == 3) {
					...
					$params = array('openid' => $log['openid'], 'tid' => $log['logno'], 'send_name' => $set['sendname'] ? $set['sendname'] : $_W['shopset']['shop']['name'], 'money' => $money, 'wishing' => $set['wishing'] ? $set['wishing'] : '红包领到手抽筋，别人加班你加薪!', 'act_name' => ''.$_W['shopset']['trade']['credittext'].'兑换红包', 'remark' => ''.$_W['shopset']['trade']['credittext'].'兑换红包');
			...
			plog('creditshop.log.doexchange', ''.$_W['shopset']['trade']['credittext'].'商城兑换 兑换记录ID: ' . $id);
			show_json(1);
		}
