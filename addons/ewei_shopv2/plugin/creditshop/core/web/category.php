...
class Category_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('creditshop.category.edit', '修改'.$_W['shopset']['trade']['credittext'].'商城分类 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_creditshop_category', $data);
				$id = pdo_insertid();
				plog('creditshop.category.add', '添加'.$_W['shopset']['trade']['credittext'].'商城分类 ID: ' . $id);
			}
	...
	public function delete()
	{
		...
		plog('creditshop.category.delete', '删除'.$_W['shopset']['trade']['credittext'].'商城分类 ID: ' . $id . ' 标题: ' . $item['name'] . ' ');
		show_json(1);
	}
