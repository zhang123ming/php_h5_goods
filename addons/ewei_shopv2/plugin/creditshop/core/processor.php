...
class CreditshopProcessor extends PluginProcessor
{
	...
	public function respond($obj = NULL)
	{
		...
		if (($msgtype == 'text') || ($event == 'click')) {
			...
			if (!$obj->inContext) {
				$obj->beginContext();
				return $obj->respText('请输入8位及以上'.$_W['shopset']['trade']['credittext'].'商城兑换码:');
			}
			...
			if ($obj->inContext) {
				if (is_numeric($content)) {
					if (8 <= strlen($content)) {
						...
						$str = '';
						$str .= "您正在对".$_W['shopset']['trade']['credittext']."商城的订单进行线下兑换\r\n";
						$str .= "\r\n订单号: " . $log['logno'] . "\r\n";
						$str .= '商品: ' . $goods['title'] . "\r\n";

						if ($goods['acttype'] == 0) {
							$str .= '价格: ' . $goods['credit'] . ''.$_W['shopset']['trade']['credittext'].'+' . $goods['money'] . "元\r\n";
						}
						else if ($goods['acttype'] == 1) {
							$str .= '价格: ' . $goods['credit'] . "".$_W['shopset']['trade']['credittext']."\r\n";
						}
