...
class Log_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function Receivepacket()
	{
		...
		$params = array('openid' => $openid, 'tid' => $log['logno'], 'send_name' => $set['sendname'] ? $set['sendname'] : $_W['shopset']['shop']['name'], 'money' => $money, 'wishing' => $set['wishing'] ? $set['wishing'] : '红包领到手抽筋，别人加班你加薪!', 'act_name' => ''.$_W['shopset']['trade']['credittext'].'兑换红包', 'remark' => ''.$_W['shopset']['trade']['credittext'].'兑换红包');
	...
	public function paydispatch()
	{
		...
		if ($paytype == 'wechat') {
			...
			if ($paytype == 'alipay') {
				...
				$params['user'] = $openid;
				$params['fee'] = $goods['dispatch'];
				$params['title'] = $set['shop']['name'] . ''.$_W['shopset']['trade']['credittext'].'兑换运费支付' . ' 单号:' . $log['logno'];
