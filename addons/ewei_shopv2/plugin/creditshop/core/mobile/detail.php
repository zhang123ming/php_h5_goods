	...
	public function lottery()
	{
		...
		if (empty($log['paystatus'])) {
			if ((0 < $goods['credit']) && ($credit < $goods['credit'])) {
				show_json(0, array('status' => '-1', 'message' => ''.$_W['shopset']['trade']['credittext'].'不足!'));
			}
		...
		if ((0 < ($goods['money'] + $goods['dispatch'])) && ($log['paystatus'] < 1)) {
			if ($log['paytype'] == 0) {
				m('member')->setCredit($openid, 'credit2', 0 - ($goods['money'] + $goods['dispatch']), ''.$_W['shopset']['trade']['credittext'].'商城扣除余额度 ' . $goods['money']);
				$update['paystatus'] = 1;
			}
		...
		if ((0 < $goods['credit']) && empty($log['creditpay'])) {
			// var_dump($openid,$goods['credit']);die;
			m('member')->setCredit($openid, 'credit1', 0 - $goods['credit'], ''.$_W['shopset']['trade']['credittext'].'商城扣除'.$_W['shopset']['trade']['credittext'].' ' . $goods['credit']);
			$update['creditpay'] = 1;
		...
		if ($status == 2) {
			if ($goods['goodstype'] == 1) {
				...
				$credittype = 'credit2';
				$creditstr = ''.$_W['shopset']['trade']['credittext'].'商城兑换余额';
				...
				$data = array('openid' => $openid, 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . ''.$_W['shopset']['trade']['credittext'].'商城兑换余额', 'money' => $num, 'remark' => $creditstr, 'rechargetype' => 'creditshop');
