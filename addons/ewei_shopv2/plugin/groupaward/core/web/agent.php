...
class Agent_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if ($set['awardMode']==4||$set['awardMode']==5) {
			...
			if(strlen($ids)>0){
				$condition .= ' and (dm.id in ('.rtrim($ids,',').') or l.level>='.$set['level'].' ) ';
			}else{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_member_level') . ' l on l.id = dm.level' . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1 and dm.status=1' . $condition . ' ORDER BY dm.agenttime desc';
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('ewei_shop_member') . ' dm  ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_member_level') . ' l on l.id = dm.level' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isagent =1 and dm.status = 1 ' . $condition, $params);
		foreach ($list as &$row ) 
		{
