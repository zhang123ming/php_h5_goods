...
class LiveModel extends PluginModel 
{
    ...
    public function sendMessage($roomId){
        global $_W,$_GPC;
        $room = pdo_fetch("SELECT * FROM `ims_ewei_shop_live` WHERE uniacid={$_W['uniacid']} and id = {$roomId}");

        $account = m('common')->getAccount();
        $text = '您关注的主播 ' . $room['title'] . ' 正在直播,快来围观吧!~ ' ;
        $url =  "http://meilihou.guangmaidata.com/app" . substr(mobileUrl('live/room',array('id'=>$roomId)),1);

        $fans = pdo_fetchAll("SELECT * FROM `ims_ewei_shop_live_favorite` WHERE uniacid={$_W['uniacid']} and roomid={$roomId} and deleted=0");

        foreach($fans as $fan){
            m('message')->sendTexts($fan['openid'], $text, $url, $account);
        }
    }

    //生成直播间二维码
    public function setQrcode($roomId){
        $url ="http://meilihou.guangmaidata.com/app"  . substr(mobileUrl('live/room',array('id'=>$roomId)),1);
