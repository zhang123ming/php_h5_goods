...
class LiveSocket 
{
	...
	public function onMessage($server, $data, $fd) 
	{
		...
		if( $data["type"] == "login" ) 
		{
			...
			if( $data["role"] == "manage" && $this->isManage($data["uid"]) ) 
			{
							...
							else 
							{
								...
								else 
								{
									if( $data["type"] == "setstatus" ) 
									{
									    ...
									    if($update['living']==1){
									        //推送直播消息
                                            p('live')->sendMessage($data['roomid']);
                                        }
									}
									else 
									{
										if( $data["type"] == "clicklike" ) 
										{
										...
										else 
										{
											...
											else 
											{
												if( $data["type"] == "redpack" ) 
												{
													...
													$this->sendAll($server, array( "type" => "redpack", "fromUser" => $data["uid"], "toUser" => "all", "nickname" => $data["nickname"], "redpack" => array( "title" => $redpacktitle, "id" => $redpackid, "total" => $redpacktotal, "money" => $redpackmoney, "type" => $redpacktype ), "msgid" => $msgid, "roomid" => $data["roomid"] ));
												}
												else 
												{
													...
													else 
													{
														...
														else 
														{
															if( $data["type"] == "coupon" ) 
															{
																...
																$this->sendAll($server, array( "type" => "coupon", "fromUser" => $data["uid"], "toUser" => "all", "nickname" => $data["nickname"], "coupon" => array( "title" => $couponname, "id" => $pushcouponid, "total" => $coupontotal ), "msgid" => $msgid, "roomid" => $data["roomid"] ));
															}
															else 
															{
