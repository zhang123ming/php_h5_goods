define(['core', 'tpl', 'biz/goods/picker'], function(core, tpl, picker) {
    ...
    modal.initSet = function() {
        ...
        $('.btn-submit').click(function() {
            ...
            var html = btn.html();
            var thumb = $('#imageLogo').find('li:first-child');
            var data = {
                title: $('#title').val(),
                covertype:$('#covertype:checked').val(),
                thumb: thumb.length > 0 ? thumb.data('filename') : '',
                liveidentity: $('#liveidentity').val(),
                url:$('#url').val(),
                introduce:$('#introduce').html()
            };
            btn.attr('stop', 1).html('正在处理...');
            core.json('live/myroom/set', {
                roomdata: data
            }, function(pjson) {
                if (pjson.status == 0) {
                ...
                location.href = core.getUrl('live.myroom');
            }, true, true)
        })
    };
    modal.initSelect = function() {
        ...
        $('.btn-submit').click(function() {
            ...
            $(".goods-selected").each(function() {
                var liveprive =  $(this).data('price');
                goods.push({'id':$(this).data('goodsid'),'liveprice':liveprive});
                goodsids.push($(this).data('goodsid'));
            });
            if (goods.length <= 0 && $('#openselect').prop('checked')) {
            ...
            btn.attr('stop', 1).html('正在处理...');
            core.json('live/myroom/select', data, function(pjson) {
                if (pjson.status == 0) {
                    btn.removeAttr('stop').html(html);
                    FoxUI.toast.show(pjson.result.message);
                    return
    ...
    modal.getIndexList = function() {
        ...
        core.json('live/myroom/get_goods', {
