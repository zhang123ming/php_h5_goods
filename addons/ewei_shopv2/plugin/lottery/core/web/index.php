...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			}elseif ($_GPC['task_type']==5) {
				$payments_num = intval($_GPC['payments_num']);
				$payments_money = floatval($_GPC['payments_money']);
				$data['task_type'] = 5;
				$data['task_data'] = array('payments_num' => $payments_num, 'payments_money' => $payments_money);
			}
			else {
				if ($_GPC['task_type'] == 4) {
