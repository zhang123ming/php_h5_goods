...
class index_EweiShopV2Page extends PluginMobilePage
{
	...
	public function lotteryList()
	{
		...
		$task_sql = 'SELECT * FROM ' .tablename('ewei_shop_lottery').' WHERE uniacid=:uniacid and uniacid=:uniacid AND `is_delete`=:is_delete ';
		$lotteryList = pdo_fetchall($task_sql,array(':uniacid' => $_W['uniacid'], ':is_delete'=>$is_delete ));
	...
	public function lottery_info()
	{
		...
		if ($id) {
			...
			$log = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM ' . tablename('ewei_shop_lottery_log') . ' AS l LEFT JOIN ' . tablename('ewei_shop_member') . ' AS m ON m.openid=l.join_user WHERE l.uniacid=:uniacid AND l.lottery_id=:lottery_id AND l.is_reward=1 order by l.log_id desc LIMIT 10', array(':uniacid' => $_W['uniacid'], ':lottery_id' => $id));
			if (!empty($lottery['lottery_days'])) {
				$effecttime = time() - $lottery['lottery_days'];
				$conditon = ' and `addtime` > ' . $effecttime;
			}
			if ($id==22) {
				# 活动测试code... INSERT INTO `weizan`.`ims_ewei_shop_lottery_join` (`id`, `uniacid`, `join_user`, `lottery_id`, `lottery_num`, `lottery_tag`, `addtime`) VALUES ('404', '4', 'oPiMiw-jv85leYH51hdnr4ahh7wQ', '19', '0', '消费满1元,赠1次', '1539081811');
				$history =  pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_lottery_join') . 'where uniacid=:uniacid AND lottery_id=:lottery_id  AND join_user=:join_user ', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':lottery_id' => $id));
				if (empty($history)) {
					$insertData = array(
						'uniacid'=>$_W['uniacid'],
						'join_user'=>$_W['openid'],
					...
					for ($i=0; $i <3 ; $i++) { 
						pdo_insert('ewei_shop_lottery_join',$insertData);
					}
	...
	public function deduct()
	{
		...
		if ($id) {
			...
			$taskData = unserialize($lottery['task_data']);

		}
		$credittype='credit2';
		$credits=0-$taskData['payments_money'];
		$remark="转盘扣款";

		$member = m('member')->getMember($_W['openid'], true);
		if ($member['credit2']<=0) {
			
			$url = mobileUrl('member/recharge',array(),true);
			$data=[
				'status'=>100,
				'msg'=>"余额不足，请充值",
				'url'=>$url
			...
			m('member')->setCredit($_W['openid'],$credittype,$credits,array($_W['uid'], $typestr . ' ' . $remark));
			$set = m('common')->getSysset('shop');
			$logno = m('common')->createNO('member_log', 'logno', 'RC');
			$data = array('openid' => $_W['openid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '转盘扣款', 'money' => $credits, 'remark' => $remark, 'rechargetype' => 'system');
			...
			if ($logid) {
				...
				if (p('lottery')) {
					$res = p('lottery')->getLottery($member['openid'], 5,'');
