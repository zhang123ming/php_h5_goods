...
if (!(class_exists('AppModel'))) {
	class AppModel extends PluginModel
	{
		 ...
		 * @DateTime 2019-03-14T22:07:42+0800
		 * @param    [type]                   $params      [description]
		 * @param    integer                  $type        [description]
		 * @return   [array]                                [description]
		 */
		public function wxpayhk($params,$type=0)
		{
			global $_W;
			$data = m('common')->getSysset('app');
			$openid = ((empty($params['openid']) ? $_W['openid'] : $params['openid']));
			...
			// 海外支付
			$postdata = array(
				'attach' =>$_W['uniacid'] . ':' . $type,
				'mchid' => $sec['wxapp']['mchid'],
				'appid' =>$data['appid'],
				'description' =>$params['title'],
				'notify_url' =>$_W['siteroot'] . 'addons/ewei_shopv2/payment/wechat/notify.php',
				'out_trade_no' =>$params['tid'],
				'trade_type' =>'JSAPI',
				'merchant_category_code' =>'7399',
				'payer'=>array('openid'=>$openid),
				'amount'=>array('total'=>$params['fee'] * 100,"currency"=>"HKD")
			);
			if (!(empty($params['goods_tag']))) {
				$postdata['goods_tag'] = $params['goods_tag'];
			}
			
			$post_method = "POST";
			$post_url = "/hk/v3/transactions/jsapi";
			$post_timestamp = $signtime;
			$post_nonce_str = $nonce_str;
			$post_data = json_encode($postdata);
			$signString = $post_method."\n".$post_url."\n".$post_timestamp."\n".$post_nonce_str."\n".$post_data."\n";
			//加密开始
			$signature = $this->getRsaSign($signString,$sec['wxapp_key']);
			$serial_no = '776D540CDBE6BAEB6A33D40462D321FDB0FDFAD4';
			$curlVersion = curl_version();
			$ua = " CURL/".$curlVersion['version'];
			$extra = array(
				'User-Agent'=>$ua,
				'Authorization'=>'WECHATPAY2-SHA256-RSA2048 mchid="'.$sec['wxapp']['mchid'].'",nonce_str="'.$nonce_str.'",signature="'.$signature.'",timestamp="'.$signtime.'",serial_no="'.$serial_no.'"',
				'Content-Type'=>'application/json',
				'Accept'=>'application/json',
				'Content-Length'=>strlen($post_data)
			);
			$url = 'https://api.mch.weixin.qq.com/hk/v3/transactions/jsapi';
			$response = ihttp_request($url, $post_data,$extra);
			if (in_array($response['code'], array(400,401,403,404,429,500,502,503))) {
			...
			$wOpt['signType'] = 'RSA';
			$wOpt['paySign'] = $this->getRsaSign($string,$sec['wxapp_key']);
			return $wOpt;

		}
		public static function getRsaSign($signString,$priviteKey)
		{
			$priviteKeyId = openssl_pkey_get_private($priviteKey);
			$signature = '';
			openssl_sign($signString, $signature, $priviteKeyId, OPENSSL_ALGO_SHA256);
			openssl_free_key($priviteKeyId);
		...
		public function isWeixinPay($out_trade_no, $money = 0)
		{
			...
			file_put_contents('mysql201806212018.txt',date('Y-m-d H:i:s').":".var_export($resp,true)."\r\n",FILE_APPEND);
			if (is_error($resp)) {
						...
						if ($mobile) {
							...
							if ($item['id'] == 'richtext') {
								...
								if (!(empty($item['data']))) {
									...
									//var_dump($item['params']['saleout']);die;
									if (($item['params']['saleout'] == 0) && !(empty($_W['shopset']['shop']['saleout']))) {
										
										$item['params']['saleoutimg'] = tomedia($_W['shopset']['shop']['saleout']);

										if (empty($item['params']['saleout'])) {
											$item['params']['saleoutimg'] = tomedia('../addons/ewei_shopv2/plugin/diypage/static/images/default/saleout-2.png');
										}

									}
									 else if (($item['params']['saleout'] == 1) ) {
									 	
										$item['params']['saleoutimg'] = tomedia('../addons/ewei_shopv2/plugin/diypage/static/images/default/saleout-' . $item['style']['saleoutstyle'] . '.png');
									}
		...
		public function judge($type = NULL, $str = NULL)
		{
			...
			if ($type == 'dot') {
				...
				}elseif (strexists($str,'/pages/sale/coupon/index/index')) {
					return 0;
				}else {
					return $order['cartnum'];
					return $order['favorite'];
			...
			if ($item['id'] == 'goods') {
				...
				if (empty($item['params']['goodsdata']) && !(empty($item['data'])) && is_array($item['data'])) {
					...
					if (!(empty($group)) && !(empty($group['goodsids']))) {
						...
						$item['params']['goodsnum'] = ((!(empty($item['params']['goodsnum'])) ? $item['params']['goodsnum'] : 20));
						$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice,preselltimeend,ispresell,video,`type` from ' . tablename('ewei_shop_goods') . ' where id in( ' . $group['goodsids'] . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $item['params']['goodsnum'], array(':uniacid' => $_W['uniacid']));
					}
					...
					if ($item['params']['sourcedata']==1) {
						$args['merchid'] = "0";
						$args['sourcedata']=1;
						
					}elseif ($item['params']['sourcedata']==2) {
						$args['merchid'] = $member['shop_id'];
						$args['sourcedata']=2;
					}
					...
					if ($item['params']['goodsdata'] == 3) {
						...
						$args['istime'] = 1;
					}elseif ($item['params']['goodsdata']  == 100) {
						$args['all'] = 1;
					}
				...
				if (!(empty($goods)) && is_array($goods)) {
					if (empty($item['params']['goodsdata'])) {
						...
						foreach ($item['data'] as $childid => $childgoods ) {
							foreach ($goods as $index => $good ) {
								if ($good['id'] == $childgoods['gid']) {
									...
									if ($good['isstatustime']&&($good['statustimestart']>time()||$good['statustimeend']<time())) {
										
										continue;
									}


									$item['data'][$childid] = array('gid' => $good['id'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'thumb' => $good['thumb'], 'total' => $good['total'], 'productprice' => $good['productprice'], 'ctype' => $good['type'], 'sales' => $good['sales'] + intval($good['salesreal']), 'video' => $good['video'], 'bargain' => $good['bargain'],'isdiscount'=>$goods['isdiscount'],'isdiscount_time'=>$goods['isdiscount_time'],'isstatustime'=>$good['isstatustime'],'statustimestart'=>$good['statustimestart'],'statustimeend'=>$good['statustimeend']);
								}
						...
						if ($good['isstatustime']&&($good['statustimestart']>time()||$good['statustimeend']<time())) {
							...
							$item['data'][$childid] = array('gid' => $good['id'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'thumb' => $good['thumb'], 'total' => $good['total'], 'productprice' => $good['productprice'], 'ctype' => $good['type'], 'sales' => $good['sales'] + intval($good['salesreal']), 'video' => $good['video'], 'bargain' => $good['bargain'],'isdiscount'=>$good['isdiscount'],'isdiscount_time'=>$good['isdiscount_time'],'isstatustime'=>$good['isstatustime'],'statustimestart'=>$good['statustimestart'],'statustimeend'=>$good['statustimeend']);
						}
					}
				}else{
		...
		public function getCodeUnlimit($params = array())
		{
			...
			file_put_contents('mysql2018060911.txt',date('Y-m-d H:i:s').":".var_export($params,true),FILE_APPEND);
		...
		public function defaultMenu($page)
		{
			...
			if ($page == 'shop') {
				$menu = array(
					...
					array('url' => 'commission', 'icon' => 'e647', 'text' => '会员中心'),
					array('url' => 'member.cart', 'icon' => 'e642', 'text' => '购物车', 'needlogin' => 1),
					array('url' => 'member', 'icon' => 'e724', 'text' => '个人中心', 'needlogin' => 1)
					);
				if (p('commission') && (0 < $_W['shopset']['commission']['level'])) {
					$member = m('member')->getMember($_W['openid']);

					if (($member['isagent'] == 1) && ($member['status'] == 1)) {
						$menu[2]['text'] = ((empty($_W['shopset']['commission']['texts']['center']) ? '会员中心' : $_W['shopset']['commission']['texts']['center']));
					}
					 else {
				...
				$menu = array(
					array('url' => 'commission', 'icon' => 'e647', 'text' => '个人中心', 'needlogin' => 1),
	...
	'items'    => $menu
		...
		public function getCodelimit($params = array())
		{
			if (empty($params) || !(is_array($params))) {
				return error(-1, '参数错误(params)');
			}
			...
			load()->func('communication');
			$request = ihttp_post('https://api.weixin.qq.com/wxa/getwxacode?access_token=' . $accessToken, json_encode($params));
			$content = json_decode($request['content'], true);
