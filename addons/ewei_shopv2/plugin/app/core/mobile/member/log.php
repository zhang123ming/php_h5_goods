...
class Log_EweiShopV2Page extends AppMobilePage
{
	public function get_list()
	{
		...
		if($type==1){
			...
			$condition = ' and openid=:openid and uniacid=:uniacid and credittype=:type';
			$params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':type' => 'credit2');
			$list = pdo_fetchall("select * from " . tablename('mc_credits_record') . "
				 where 1 " . $condition.' order by id desc', $params);
			$total = pdo_fetchcolumn('select count(*) from '. tablename('mc_credits_record') . "
				 where 1 " . $condition, $params);

			if (is_array($list) && !empty($list)) {
				foreach ($list as $row) {
					if($row['operator']==$row['uid']){
						if($row['uid']==1){
							$type=0;
						}else{
							$type=3;
					...
					$newList[] = array('id' => $row['id'], 'type' => $type, 'money' => $row['num'], 'status' => 1,'createtime' => date('Y-m-d H:i', $row['createtime']));
				}
			}
		}else if ($type==2) {
			$condition = ' and openid=:openid and uniacid=:uniacid and credittype=:type';
			$params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':type' => 'credit1');
			$list = pdo_fetchall("select * from " . tablename('mc_credits_record') . "
				 where 1 " . $condition, $params);
			$total = pdo_fetchcolumn('select count(*) from '. tablename('mc_credits_record') . "
				 where 1 " . $condition, $params);
			...
			if (is_array($list) && !empty($list)) {
				foreach ($list as $row) {
					$newList[] = array('id' => $row['id'], 'remark' => $row['remark'], 'money' => $row['num'], 'status' => 1,'createtime' => date('Y-m-d H:i:s', $row['createtime']));
				}
			}
		}
