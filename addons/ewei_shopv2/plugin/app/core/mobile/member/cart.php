...
class Cart_EweiShopV2Page extends AppMobilePage
{
	public function get_cart()
	{
		...
		$sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,' . ' g.productprice,o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.limitbuy,g.unit,f.merchid,g.merchsale' . ' ,f.selected,g.intervalfloor,g.intervalprice,g.type FROM ' . tablename('ewei_shop_member_cart') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
		...
		foreach ($list as &$g ) {
			...
			$g['intervalprice'] = unserialize($g['intervalprice']);
			...
			$g['type']==4 && $g['marketprice'] = (double) m('order')->getWholesaleprice(intval($g['total']),$g['goodsid']);
			$totalmaxbuy = $g['stock'];
			...
			if ($totalmaxbuy < $g['total']) {
				...
				/*同时更新数据库中的值*/
				pdo_update('ewei_shop_member_cart', array('total' => $totalmaxbuy, 'optionid' => $g['optionid']), array('id' => $g['id'], 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
			}
	...
	public function add()
	{
		...
		// if($_SERVER['REMOTE_ADDR']=='219.136.74.5'){
		// 	echo "<pre>";
		// 	var_dump($goods);die;
		// }
...
// $goods['isverify'] == 1表示商品状态为支持线下核销和快递，此时的商品不可以加入购物车
// $goods['isverify'] == 2表示商品状态为仅支持线下核销，此时的商品不可以加入购物车

		// if (($goods['isverify'] == 2) || ($goods['type'] == 2) || ($goods['type'] == 3)) {
		// 	app_error(AppError::$NotAddCart);
		// }
		// 此处去掉了$goods['isverify'] == 2，不管商品状态如何均可以加入购物车
		// 若有异常可以回滚上面的那一段代码
