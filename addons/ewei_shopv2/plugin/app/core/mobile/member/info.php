...
class Info_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		$memberArr = array('nickname' => $member['nickname'], 'realname' => $member['realname'], 'mobile' => $member['mobile'], 'weixin' => $member['weixin'], 'birthday' => $member['birthday'], 'city' => $member['city'], 'mobileverify' => $member['mobileverify'], 'avatar' => tomedia($member['avatar']),'userid' => $member['userid']);

		if (!empty($memberSet['openapply'])&&!empty($memberSet['limitgrade'])) {

		
			$mlevels=pdo_fetchall("SELECT id,levelname FROM ".tablename('ewei_shop_member_level')." WHERE uniacid=$_W[uniacid] and id ={$memberSet['limitgrade']}");
		}else{
			$mlevels=pdo_fetchall("SELECT id,levelname FROM ".tablename('ewei_shop_member_level')." WHERE uniacid=$_W[uniacid] order by level asc");
		}

		$sysset = $_W['shopset']['shop'];
		$memberArr['isuserid'] = $sysset['isuserid'];

		$sql="SELECT * FROM ".tablename('ewei_shop_member_apply')." where uniacid=$_W[uniacid] and openid='$_W[openid]' and status=0";

		$item=pdo_fetch($sql);
		if ($item) {
			$memberArr['applyed']=true;
		}else{
			$memberArr['applyed']=false;
		}
			
		$result = array(
			'member'  => $memberArr,
			'level' =>$mlevels?$mlevels:0,
	...
	public function submit()
	{
		...
		if ($template_flag == 1) {
			...
			$arr = array('realname' => trim($memberdata['realname']), 'weixin' => trim($memberdata['weixin']), 'birthyear' => intval($memberdata['birthyear']), 'birthmonth' => intval($memberdata['birthmonth']), 'birthday' => intval($memberdata['birthday']), 'province' => trim($memberdata['province']), 'city' => trim($memberdata['city']), 'datavalue' => trim($memberdata['datavalue']), 'mobile' => $memberdata['mobile']);
	...
	public function apply()
	{
		...
		$result=array(
			'uniacid'=>$_W['uniacid'],
			'openid'=>$_W['openid'],
			'memberdata'=>iserializer($memberdata),
			'level'=>$memberdata['level']['id'],
			'status'=>$status,
			'createtime'=>time()
		);

		if (empty($memberdata['realname'])||empty($memberdata['mobile']||empty($memberdata['birthday']))) {
			app_error('-2000',"请检查信息是否完整");
		}
		...
		$mobile = trim($memberdata['mobile']);
		$verifycode = trim($memberdata['code']);
		...
		if (!empty($memberSet['notapply'])) {
			$arr['level']=$memberdata['level']['id'];
		}

		pdo_update('ewei_shop_member', $arr, array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));

		$item=pdo_fetch("SELECT id FROM ".tablename('ewei_shop_member_apply')." WHERE uniacid={$_W[uniacid]} AND openid='{$_W[openid]}'");


		if ($item) {
			pdo_update('ewei_shop_member_apply', $result,array("id"=>$item[id]));
		}else{

			pdo_insert('ewei_shop_member_apply', $result);
			...
			if ($data['consummatetype']=='coupon') {
				...
				if ($couponArr) {
				 	...
				 	foreach ($couponArr as $value) {
				 		com('coupon')->sendTicket($_W['openid'],$value,0);				 	}

				 }
				 pdo_update('ewei_shop_member', array('informationreward'=>1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
			}
			
		}
		app_json(array('notapply'=>$memberSet['notapply']));
