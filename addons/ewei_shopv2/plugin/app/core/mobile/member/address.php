...
class Address_EweiShopV2Page extends AppMobilePage
{
	public function get_list()
	{
		...
		$isuserid = $_W['shopset']['shop']['isuserid'];
		if(!empty($isuserid) && !empty($list)){
			foreach ($list as $key => $value) {
				if(!empty($value['userid'])){
					if(strlen($value['userid'])==15){
						$list[$key]['userid'] = substr_replace($value['userid'],"*****",6,4);
					}
					if(strlen($value['userid'])==18){
						$list[$key]['userid'] = substr_replace($value['userid'],"*****",6,8);
					}
				}
			}
		}
	...
	public function submit()
	{
		...
		if($set['isuserid']=='1'){
			$data['userid'] = trim($_GPC['userid']);
			$data['justthumb'] = trim($_GPC['justthumb']);
			$data['backthumb'] = trim($_GPC['backthumb']);
		}
		...
		if($set['isuserid']=='1'){
			if (empty($data['userid'])) {
				app_error(AppError::$ParamsError, '身份证号为空');
			}
		}
		...
		pdo_update('ewei_shop_member_address',array('is_default'=>0),array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		pdo_update('ewei_shop_member_address',array('is_default'=>1),array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		$address = pdo_fetch("SELECT * FROM ".tablename('ewei_shop_member_address')." WHERE uniacid=:uniacid and openid=:openid and id=:id",array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		app_json(array('addressid' => $id,'address'=>$address,'addressCount'=>$addresscount));
	}
