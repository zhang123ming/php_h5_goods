...
class Goods_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		if ($merch_plugin && $merch_data['is_openmerch']) {
			if ($_W[uniacid]=='91') {
				...
				if (empty($args['cate'])) {
					//无分类时取关联商户以及平台的产品
					$args['merchid'] = intval($_GPC['merchid'])?intval($_GPC['merchid']):$member['shop_id'];
				}else{
					//有分类时只取平台商品
					$args['sourcedata']=1;
		...
		//小程序不显示砍价商品
		$bargain_list = pdo_fetchall('select goods_id from '.tablename('ewei_shop_bargain_goods').' where account_id=:uniacid',array(':uniacid'=>$_W['uniacid']),'goods_id');
		$bargain_goodsid = array_keys($bargain_list);
		if (!empty($bargain_goodsid)) {
			$args['bargain_goodsid'] = implode(',', $bargain_goodsid);
		}
		//小程序不显示砍价商品 结束
	...
	public function get_detail()
	{
		...
		//会员等级单次购买限制
		$goods = m('goods')->getGoodsLimit($goods,$openidlevel['id']);
		...
		if($goods['total']=='0'){
			$remind = pdo_fetch('select * from ' . tablename('ewei_shop_goods_remind') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid limit 1 ',array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid'],':goodsid'=>$id));
			if(!empty($remind)){
				$goods['goodsremind'] = '1';
			} else {
				$goods['goodsremind'] = '0';
		...
		if ($goods['protocolshow']==1) {

			$goods['protocolcontent'] = m('common')->html_to_images($goods['protocolcontent']);
		...
		if ($goods['isdiscount'] == 1) {
			if (time() > $goods['isdiscount_time']) {
			
				$goods['canbuy'] = 0;
				$goods['cannotbuy'] = '活动已结束';
			}
			
		}


		if ($goods['isstatustime']==1) {
			if ($goods['statustimestart'] > time() || time() > $goods['statustimeend']) {

				$statusdata['status'] = 0;
				$goods['canbuy'] = 0;
				$goods['cannotbuy'] = '活动已结束';

				pdo_update('ewei_shop_goods', array('status' => $statusdata['status']), array('id' => $goods['id']));
		...
		$list = pdo_fetchall('select id,storename,lat,lng from '.tablename('ewei_shop_store').' where uniacid=:uniacid',array(':uniacid'=>$_W['uniacid']));
		$arr = array();
		foreach($list as $k=> &$v){
			if(!empty($_GPC['lat'])&&!empty($_GPC['lng'])){
				$distance = m('store')->getDistance($_GPC['lat'],$_GPC['lng'],$v['lat'],$v['lng']);
				$v['distance'] = round($distance/1000,2);
				$arr[$k] = $v['distance'];	
			}	
		}
		unset($v);
		$goods['intervalprice'] = unserialize($goods['intervalprice']);
		$goods['mobileverify'] = intval($member['mobile']);
		array_multisort($arr,SORT_ASC,SORT_REGULAR,$list);
		$distance = $list[0];

		app_json(array('goods' => $goods,'distance'=>$distance));
	}
	...
	public function get_picker()
	{
		...
		$member = m('member')->getMember($openid);
		if ((!empty($_W['shopset']['app']['openbind'])||!empty($_W['shopset']['wap']['mustbind']))&&empty($member['mobileverify'])) {
			app_error(3,"请先绑定手机");
		}
				...
				foreach ($options as &$option) {
					foreach ($seckillinfo['options'] as $so) {
						if ($option['id'] == $so['optionid']) {
							...
							$option['total'] = $so['total'];
							$option['maxbuy'] = $so['maxbuy'];
						}
					}
				}
		...
		//获取用户等级
		$levelopenid = m('member')->getLevel($openid);
		//会员单次购买限制
		$goods = m('goods')->getGoodsLimit($goods,$openidlevel['id']);
		...
		$goods['mobileverify'] = $member['mobile'];
	...
	public function pay_coupon()
	{
		...
		$openid = $_W['openid'];
		$id = intval($_GPC['id']);
		$couponCode = $_GPC['couponCode'];


		if ($couponCode) {
			$coupon = pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . " where couponcode='{$couponCode}' and uniacid=:uniacid  limit 1", array(':uniacid' => $_W['uniacid']));
			$id = $coupon[id];
		}else{
		...
		app_json(array('dataid' => $result['dataid'],'name'=>$coupon['couponname'],'id'=>$result['dataid'],'coupontype' => $result['coupontype']));
	}
	...
	public function getshoptips()
	{
		...
		$id = $_GPC['id'];
		$shoptips = pdo_fetch('select * from ' . tablename('ewei_shop_shoptips') . ' where uniacid = ' . $_W['uniacid'] . ' and showtype = "2" and status = "1" and istop = "1" and id=:id',array(':id' => $id));
		if(!empty($shoptips)){
			if(!empty($shoptips['thumb'])){
				$shoptips['thumb'] = tomedia($shoptips['thumb']);
			}
		}

		app_json(array('shoptips' =>$shoptips));
	}

	//到货通知
	public function goodsremind()
	{
		global $_GPC;
		global $_W;
		...
		$remind = pdo_fetch('select * from ' . tablename('ewei_shop_goods_remind') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid limit 1 ',array(':uniacid'=>$_W['uniacid'],':openid'=>$_W['openid'],':goodsid'=>$_GPC['id']));

		if(!empty($remind)){
			app_error(1,'无需重复添加到货提醒！');
		} else {
			$data['goodsid'] = $_GPC['id'];
			$data['openid'] = $_W['openid'];
			$data['uniacid'] = $_W['uniacid'];

			$res = pdo_insert('ewei_shop_goods_remind',$data);
			if($res){
				app_json(0);
			} else {
				app_error(1,'添加失败！');
