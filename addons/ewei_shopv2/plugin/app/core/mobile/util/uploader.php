...
class Uploader_EweiShopV2Page extends AppMobilePage
{
	...
	public function uploadmp3()
	{
		...
		if (!empty($_FILES[$field]['name'])) {
			...
			$result = $this->uploadMp3File($_FILES[$field]);
	...
	protected function uploadMp3File($uploadfile)
	{
		global $_W;
		global $_GPC;
		$result['status'] = 'error';
		...
		load()->func('file');
		$path = '/audio/ewei_shop/' . $_W['uniacid'];
		...
		$file = file_upload($uploadfile, 'audio');
		...
		$result['filename'] = $file['path'];
		$result['url'] = trim($_W['attachurl'] . $result['filename']);
		pdo_insert('core_attachment', array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'filename' => $uploadfile['name'], 'attachment' => $result['filename'], 'type' => 99, 'createtime' => TIMESTAMP));
		return $result;
	}
