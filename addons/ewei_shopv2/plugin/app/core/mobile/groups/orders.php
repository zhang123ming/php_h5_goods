...
class Orders_EweiShopV2Page extends AppMobilePage
{
	public function main()
	{
		...
		if (!empty($goods['followneed']) && !$follow && is_weixin()) {
			...
			show_json(0, array('message' => '$followtext'));
		}
		...
		if ($order && ($order['groupnum'] <= $ordernum)) {
			show_json(0, array('message' => '该团人数已达上限，请浏览其他商品或联系商家！'));
		}
		...
		if (!empty($teamid)) {
			...
			foreach ($orders as $key => $value) {
				if ($orders && ($value['success'] == -1)) {
					show_json(0, array('message' => '该活动已过期，请浏览其他商品或联系商家！'));
				}

				if ($orders && ($value['success'] == 1)) {
					show_json(0, array('message' => '该活动已结束，请浏览其他商品或联系商家！'));
				}
			}
			...
			if ($num == $goods['groupnum']) {
				show_json(0, array('message' => '该活动已成功组团，请浏览其他商品或联系商家！'));
			}
		}
		...
		$realprice = $price+$goods['freight'];

		$result = array(
			'member'             => $member,
			'showTab'            => (0 < count($carrier_list)) && !($isverify) ,
			'showAddress'        => !($isverify),
			'isverify'           => $isverify,
	...
	public function get_list()
	{
		...
		foreach ($orders as $key => $value) {
			...
			if(!empty($value['optionid'])){
				$option = pdo_fetch('select *  from ' . tablename('ewei_shop_groups_option') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $value['optionid']));
				$orders[$key]['optionname'] = $option['name'];
			}
			if (!$verifytotal) {
				$verifytotal = 0;
			}
	...
	public function confirm()
	{
		...
		if($goods['op_more']){
			$is_more=1;
			$options = pdo_fetchall('select * from ' . tablename('ewei_shop_groups_option') . "where goodsid = :goodsid and uniacid = :uniacid and gid=:gid", array(':goodsid' => $goodid, ':uniacid' => $uniacid, ':gid' => $goods['gid']));
			$specs = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_spec') . "where goodsid = :goodsid and uniacid = :uniacid", array(':goodsid' => $goods['gid'], ':uniacid' => $uniacid));
			$specsname = '';
			if(!empty($_GPC['opid'])){
				$option = pdo_fetch('select * from ' . tablename('ewei_shop_groups_option') . "where id = :id and uniacid = :uniacid", array(':id' => $_GPC['opid'], ':uniacid' => $uniacid));
			...
			if(!empty($_GPC['optionid'])){
				$option = pdo_fetch('select * from ' . tablename('ewei_shop_groups_option') . "where id = :id and uniacid = :uniacid", array(':id' => $_GPC['optionid'], ':uniacid' => $uniacid));
				$goods['groupsprice'] = $option['price'];
				$goods['singleprice'] = $option['single_price'];
			}
			if(!empty($specs)){
				foreach ($specs as $k => $v) {
					$specsname = $specsname.$v['title'].",";
				}
				$strend=trim($specsname,',');
			}
			$specs_name = $strend;
		}
		if ($goods['stock'] <= 0) {
		...
		if ($_GPC['ispost']&&$_GPC['submit']) {
			...
			if ($isverify) {
				if (empty($_GPC['realname']) || empty($_GPC['mobile'])) {
					show_json(0, array('message' => '联系人或联系电话不能为空！'));
				}
			}
			...
			if (!$order_insert) {
				show_json(0, array('message' => '生成订单失败！'));
			}
