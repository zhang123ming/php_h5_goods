...
class Pay_EweiShopV2Page extends AppMobilePage
{
	public function main()
	{
		...
		// 	app_error($og_array['flag'],$og_array['msg']);
		// 	exit();
		// }
		...
		if ($order['status'] == -1) {
			app_error(AppError::$OrderCannotPay);
		 	...
		 	if (($order['isprepay']&&$order['isprepaysuccess'])||empty($order['isprepay'])) {
		...
		if ($order['isprepay']&&empty($order['isprepaysuccess'])&&!empty(floatval($order['balance']))) {
			$order['price']=$order['balance'];
			$order['ordersn'] .="WK";
		}
...
//			$res = $this->model->wxpayhk($payinfo, 14);//境外商户v3接口支付
	...
	public function complete()
	{
		...
		if ($order['isprepay']&&!empty(floatval($order['balance']))) {
			$ordersn=$order['ordersn'] ."WK";
		}else{
		...
		if ($type == 'credit') {
			...
			if (!empty(floatval($order['balance']))&&$order['isprepay']) {
				$ordersn .= 'WK';
			}
	...
	public function check(){
		...
		if (!empty($og_array['flag'])) {
			app_error(1, $og_array['msg']);
		}

		app_json(array('result'=>"success "));
	}
}
