...
class Refund_EweiShopV2Page extends AppMobilePage
{
	protected function globalData()
	{
		...
		$order = pdo_fetch('select id,status,isCash,earnest,invoice,invoiceprice,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid,isprepay,isprepaysuccess,balance from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if (!empty($commissionSet['commissionsettletype'])) {
			$_err = '该订单不能申请退款!';
		}
		if ($order['status'] == 0) {
			$_err = '订单未付款，不能申请退款!';
		...
		if($order['isCash']==1){
			$order['refundprice']=$order['earnest'];
		}else{
			$order['refundprice'] = $order['price'] + $order['deductcredit2'];
		}	

		if($order['invoice']==1){
			$order['refundprice']=$order['invoiceprice'];
		}
	...
	public function buyerexpresssn(){
		global $_W;
		global $_GPC;		
		$inputvalue = $_GPC['inputvalue'];
		$orderid = $_GPC['orderid'];
		$uniacid = $_W['uniacid'];
		$refundno = $_GPC['refundno'];
		pdo_update('ewei_shop_order_refund',array("buyerexpresssn" => $inputvalue),array("orderid" => $orderid,"uniacid" => $uniacid,"refundno" => $refundno));
		app_json(1);	
	}
}
