...
class Create_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		if (!($packageid)) {
			...
			if (empty($id)) {
				...
				foreach ($goods as $k => $v) {
					...
					if (0 < $v['isprepay']) {
						...
						$goods[$k]['marketprice'] = $v['prepayprice'];
					}
					...
					if ($v['cash'] == 2) {
						if ($v['fixed_pay'] == 1) {
							...
							$dingjin = $dingjin + ($v['fixed_money'] * $v['total']);
						} elseif ($v['fixed_pay'] == 2) {
							$dingjin = $dingjin + floatval($v['fixed_money'] / 100 * $v['total'] * $v['marketprice']);
						}
					}
					...
					if (!empty($goods[$k]['seckillinfo']['maxbuy']) && (($goods[$k]['seckillinfo']['maxbuy'] - $goods[$k]['seckillinfo']['selfcount']) < $goods[$k]['total'])) {
						app_error('-1', '您已购买了' . $goods[$k]['seckillinfo']['selfcount'] . '最多购买' . $goods[$k]['seckillinfo']['maxbuy'] . '件');
					}
				...
				$sql = 'SELECT id as goodsid,cash,fixed_pay,fixed_money,type,title,weight,issendfree,taxation,isnodiscount,isfullback, ' . ' thumb,marketprice,storeids,isverify,deduct,' . ' manydeduct,`virtual`,maxbuy,limitbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,ispresell,presellprice,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,isprepay,prepayprice,' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale,preparegoodstime' . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
				...
				if ((0 < $data['isprepay'])) {
					$data['marketprice'] = $data['prepayprice'];
				}
				...
				$data['preparegoodstime'] = $data['preparegoodstime'] ? date('Y-m-d',strtotime('+ '.intval($data['preparegoodstime']).'day',time())):0;
				...
				if (!(empty($optionid))) {
					...
					if (!(empty($option))) {
						...
						if (0 < intval($data['isprepay'])) {
							$data['marketprice'] = $option['preprice'];
						}
			...
			$openidlevel = m('member')->getLevel($openid);//获取会员ID
		
			foreach ($goods as $key => $val) {
				//会员等级单次购买限制
				$goods[$key] = m('goods')->getGoodsLimit($val,$openidlevel['id']);
			}
			foreach ($goods as &$g) {
			...
			if (!($isverify) && !($isvirtual) && !($ismerch)) {
				...
				$store_id = $member['store_id'] ? $member['store_id'] : $_GPC['store_id'];
				if (!empty($store_id)) {
					$carrierCondition = " and id= '{$store_id}' ";
				}

				if (0 < $merch_id) {
					$carrier_list = pdo_fetchall('select * from ' . tablename('ewei_shop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) ' . $carrierCondition . ' order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
				} else {
					$carrier_list = pdo_fetchall('select * from ' . tablename('ewei_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) ' . $carrierCondition . ' order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			...
			foreach ($goods as &$g) {
				...
				if ($g['type'] == 4 && m('order')->getWholesaleprice($g['total'], $id)) { // 批发模式获取批发价
					$g['marketprice'] = m('order')->getWholesaleprice($g['total'], $id);
				}
			...
			foreach ($goods as $g) {
				...
				$goodsdata[] = array('goodsid' => $g['goodsid'], 'cash' => $g['cash'], 'fixed_pay' => $g['fixed_pay'], 'fixed_money' => $g['fixed_money'], 'total' => $g['total'], 'optionid' => $g['optionid'], 'marketprice' => $g['marketprice'], 'merchid' => $g['merchid'], 'cates' => $g['cates'], 'discounttype' => $g['discounttype'], 'isdiscountprice' => $g['isdiscountprice'], 'discountprice' => $g['discountprice'], 'isdiscountunitprice' => $g['isdiscountunitprice'], 'discountunitprice' => $g['discountunitprice'], 'preparegoodstime' => $g['preparegoodstime']);
			}
			if ($g["seckillinfo"] && $g["seckillinfo"]["status"] == 0) {
			} else {
				if ($g["isverify"] == 2) {
		...
		foreach ($goods_list as $k => $v) {
			...
			foreach ($v['goods'] as $g) {
				// 此处为返回到页面的具体某个商品的数组,具体到某一个商品的
				$allgoods[$k]['goods'][] = array('id' => $g['goodsid'], 'goodsid' => $g['goodsid'], 'cash' => $g['cash'], 'isCash' => $g['isCash'], 'fixed_pay' => $g['fixed_pay'], 'fixed_money' => $g['fixed_money'], 'title' => $g['title'], 'thumb' => tomedia($g['thumb']), 'optionid' => (int)$g['optionid'], 'optiontitle' => $g['optiontitle'], 'hasdiscount' => empty($g['isnodiscount']) && !(empty($g['dflag'])), 'discounttype' => $g['discounttype'], 'isdiscountprice' => $g['isdiscountprice'], 'discountprice' => $g['discountprice'], 'isdiscountunitprice' => $g['isdiscountunitprice'], 'discountunitprice' => $g['discountunitprice'], 'total' => $g['total'], 'price' => ($g['unitprice'] < $g['marketprice'] ? (double)$g['marketprice'] : (double)$g['unitprice']), 'marketprice' => (double)$g['marketprice'], 'merchid' => $g['merchid'], 'cates' => $g['cates'], 'unit' => $g['unit'], 'totalmaxbuy' => $g['totalmaxbuy'], 'minbuy' => $g['minbuy'], 'preparegoodstime' => $g['preparegoodstime']);
			}
		}

		$appsets = m('common')->getSysset(array('app'));
		$shopset = m('common')->getSysset('shop');
		$verifyset = m('common')->getSysset('verify');
		$isexpressopen = $shopset['isexpressopen'];

		$invoiceset = m('common')->getSysset('invoice');
		// var_dump($invoiceset['addinvoice']);die;


		if (!empty($verifyset['store']['salerTelHidden'])) {

			foreach ($carrier_list as $key => $value) {
				$carrier_list[$key]['mobile'] = substr_replace($value['mobile'], '****', 3, 4);
			}
		...
		if (!empty($verifyset['store']['unbindHidden'])) {
			...
			if ((empty($carrier_list) && !empty($member['store_id'])) || ($carrier_list && empty($member['store_id']))) {

				unset($carrier_list);
				$carrier_list[0]['mobile'] = $verifyset['store']['unbindTip'];
			}
		}
		...
		$shoptips = pdo_fetch('select * from ' . tablename('ewei_shop_shoptips') . ' where uniacid = ' . $_W['uniacid'] . ' and showtype = "3" and status = "1" limit 1');


		if (!empty($verifyset['store']['showdebug'])) {
			$verifyset['store']['takeTip'] .= "(ID:" . $member[id] . ",门店ID:" . $carrier_list[0][id] . ")";
		}
		if(!empty($_W['shopset']['shop']['isuserid']) && !empty($address)){
			if(strlen($address['userid'])==15){
				$address['userid'] = substr_replace($address['userid'],"*****",6,4);
			}
			if(strlen($address['userid'])==18){
				$address['userid'] = substr_replace($address['userid'],"*****",6,8);
			}
		}
		$setsdata = m('common')->getSysset('trade');
		$result = array(
			'hid_address' => $setsdata['set_address'],
			'hid_realname' => $setsdata['set_realname'],
			'hid_mobile' => $setsdata['set_mobile'],
			...
			'dispatch_price' => $dispatch_price + $seckill_dispatchprice,
			...
			'checkAddress' => (int)$appsets['app']['checkAddress'],

			'city_express_state' => (empty($dispatch_array['city_express_state']) ? 0 : $dispatch_array['city_express_state']),
			'dingjin' => $dingjin,
			'isCash' => $isCash,
			'isexpressopen' => $isexpressopen,
			'salerTelHidden' => $verifyset['store']['salerTelHidden'],
			'takeTip' => $verifyset['store']['takeTip'],
			'invoiceprice' => round($invoiceprice, 2),
			'addinvoice' => $invoiceset['addinvoice'],
			'commoninvoice' => $invoiceset['commoninvoice'],
			'taxationcount'      =>$taxationcount,
			'shoptips'			 =>$shoptips,
		);
		app_json($result);
	}
	...
	public function caculate()
	{
		...
		if (0 < $packageid) {
			...
			foreach ($goodsarr as $k=> &$g) {
				...
				if (0 < $_GPC['bargain_id']) {
					...
					$sql = 'SELECT id as goodsid,title,type, weight,total,issendfree,isnodiscount, thumb,marketprice,cash,isverify,goodssn,productsn,sales,istime,' . ' timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,ispresell,presellprice,isprepay,prepayprice,preselltimeend,manydeduct,`virtual`,' . ' discounts,deduct2,ednum,edmoney,edareas,diyformid,diyformtype,diymode,dispatchtype,dispatchid,dispatchprice,' . ' isdiscount,isdiscount_time,isdiscount_discounts ,virtualsend,merchid,merchsale,' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale,bargain,intervalfloor,intervalprice' . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
					...
					if ($data['type'] == 4 && m('order')->getWholesaleprice($goodstotal, $goodsid)) { // 批发模式获取批发价
						$data['marketprice'] = m('order')->getWholesaleprice($goodstotal, $goodsid);
						$goodsprice = $data['marketprice'];
						$goodsarr[$k]['marketprice'] = $data['marketprice'];
						$goodsarr[$k]['price'] = $data['marketprice'];
						$goodstype = 4;
					}
				}
				if (empty($data)) {
				...
				if (!(empty($optionid))) {
					$option = pdo_fetch('select id,title,marketprice,presellprice,preprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
		...
		$return_array['goodscount'] = $goodscount;  // 商品总价不含快递
		$return_array['goodsprice'] = $goodsprice;  // 商品价格
		$return_array['goodstype'] = $goodstype;  // 商品类型
		$return_array['goodsid'] = $goodsid;  // 商品id
	...
	public function submit()
	{
		...
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
		$viturl_merchid = $_GPC['storeid'];
		if (empty($_GPC['carriers']['address']) && !empty($_GPC['carriers'])) {
			$_GPC['carriers']['address'] = $_GPC['carriers']['carrier_address'];
		}
		...
		if ($_GPC['carrierid']) {
			pdo_update('ewei_shop_member', array('store_id' => $_GPC['carrierid']), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		}
		...
		foreach ($goods as $g) {
			...
			$sql = 'SELECT id as goodsid,title,type, weight,total,issendfree,isnodiscount, thumb,marketprice,cash,isverify,verifytype,' . ' goodssn,productsn,sales,istime,timestart,timeend,ispresell,presellprice,preselltimeend,isprepay,prepayprice,isstatustime,statustimestart,statustimeend,' . ' usermaxbuy,minbuy,maxbuy,limitbuy,unit,buylevels,buygroups,deleted,' . ' status,deduct,manydeduct,`virtual`,discounts,deduct2,ednum,edmoney,edareas,diyformtype,diyformid,diymode,' . ' dispatchtype,dispatchid,dispatchprice,merchid,merchsale,cates,' . ' isdiscount,isdiscount_time,isdiscount_discounts, virtualsend,' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
			$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $goodsid));
			...
			if ($data['isstatustime'] == 1) {
				if ($data['statustimestart'] > time() || time() > $data['statustimeend']) {

					$statusdata['status'] = 0;

					pdo_update('ewei_shop_goods', array('status' => $statusdata['status']), array('id' => $goodsid));
					app_error(AppError::$GoodsNotFound, $data['title'] . '<br/> 已下架!');
			...
			$data['ischeck'] = $g['ischeck'];

			$data['fixed_money'] = $g['fixed_money'];
			...
			if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
				...
				if (is_error($check_buy)) {
					app_error(-1, $check_buy['message']);
				}
			} else {
				if (0 < $data['minbuy']) {
					if ($goodstotal < $data['minbuy']) {
						app_error(AppError::$OrderCreateMinBuyLimit, $data['title'] . '\n' . $data['minbuy'] . $unit . '起售!');
					}
				}
			...
			if (!empty($optionid)) {
				$option = pdo_fetch('select id,title,presellprice,marketprice,preprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
				...
				if (!empty($option)) {
					...
					if (0 < intval($data['isprepay'])) {
						$data['marketprice'] = $option['preprice'] ? $option['preprice'] : $option['marketprice'];
					}
				...
				if ($packageid) {
				    //添加自提日期
//                    if(empty($_GPC['carriers']['changeTime'])){
//                        app_error(AppError::$OrderCanNotCreate,'自提时间未设置,不能下订单');
//                    }
		...
		$order = array();
		// 如果开启了is_opensynchro，获取的ismerch值就为1
		...
		if ($_GPC['ischeck'] == 1) {
			$order['type'] = trim($_GPC['invotype']);
			$order['invoice'] = $_GPC['ischeck'];
			$order['invoiceprice'] = $_GPC['invoiceprice'];
			$order['price'] = $_GPC['realpays'];
		} else {
			$order['price'] = $totalprice;
		}

		if ($isprepay) {
			$order['isprepay'] = $isprepay;
		}
		$order['parentid'] = 0;
		...
		if (!empty($_GPC['earnest']) && $_GPC['isCash']) {
			$order['isCash'] = $_GPC['isCash'];
			$order['earnest'] = $_GPC['earnest'];
		}
		...
		if ($_GPC['ischeck'] == 1) {
			$invoiceData['openid'] = $_GPC['openid'];
			$invoiceData['orderID'] = $ordersn;
			$invoiceData['uniacid'] = $_W['uniacid'];
			$invoiceData['amount'] = $_GPC['realpays'];
			$invoiceData['raisedType'] = $_GPC['raisedtype'];
			if ($_GPC['raisedtype'] == 1) {
				$invoiceData['number'] = $_GPC['bnumber'];
			}
			$invoiceData['raised'] = $_GPC['raised'];
			$invoiceData['type'] = $_GPC['invotype'];
			$invoiceData['content'] = $_GPC['content'];
			$invoiceData['createtime'] = time();

			pdo_insert('ewei_shop_invoice', $invoiceData);
		}

		setDanmu($_W['openid'], 0);
		if (!empty($goods[0]['bargain_id']) && p('bargain')) {
		...
		if ($multiple_order == 0) {
			...
			foreach ($allgoods as $goods) {
				...
				if ($goods['ischeck']) {
					$order_goods['dingjin'] = $goods['fixed_money'];
				}
		...
		if (empty($virtualid)) {
			$ret = m('order')->checkGoodsStock($orderid, 0);
			if ($data["seckillinfo"] && $data["seckillinfo"]["status"] == 0) {
			} else {
				if (!$ret) {
					pdo_update("ewei_shop_order", array("status" => -1), array("id" => $orderid));
					app_error('-1', "您当前下单商品库存不足,此订单已关闭.");
				}
			}
		...
		if (!empty($verifyset['store']['salernotice'])) {
			m('notice')->sendMessageToSaler($orderid);
		}
