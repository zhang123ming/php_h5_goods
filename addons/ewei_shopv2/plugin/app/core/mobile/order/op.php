...
class Op_EweiShopV2Page extends AppMobilePage
{
	...
	public function cancel()
	{
		...
		$order = pdo_fetch('select id,ordersn,openid,status,deductcredit,deductcredit2,deductprice,couponid,`virtual`,`virtual_info`  from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
	...
	public function finish()
	{
		...
		if (empty($set['commissionsettletype'])) {
 			p('commission')->payRebateManage($orderid,$_W['uniacid'],$order['openid']);
 		}
