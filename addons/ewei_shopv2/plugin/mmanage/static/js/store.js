define(['core'], function (core) {
    ...
    modal.initClick = function () {
        ...
        $(document).on('click', '.container .fui-list .fui-list-media,.fui-list .fui-list-inner', function () {
            ...
            if (item.css('display') != 'none') {
                ...
                if (goodsid && canJump) {
                    $.router.load(core.getUrl('mmanage/store/edit', {id: goodsid}), true)
                }
            }
        });
        ...
        $(document).on('click', '.btn-goods', function () {
            ...
            if (action == 'view') {
                $.router.load(core.getUrl('store/detail', {id: goodsid}), true);
                return
            } else if (action == 'edit') {
                $.router.load(core.getUrl('mmanage/store/edit', {id: goodsid}), true);
                return
            ...
            if (action == 'delete' || action == 'status' || action == 'restore') {
                FoxUI.confirm(confirm_text, function () {
                    modal.stop = true;
                    core.json("mmanage/store/" + action, obj, function (json) {
                        if (json.status == 1) {
        ...
        $(document).on('click','.changeStatus',function(){
            ...
            var obj={storeid,storeid,status:status};
            core.json('mmanage/store/state',obj,function(json){
               if(json.status){
                FoxUI.toast.show("更新成功");
               }else{
                    FoxUI.toast.show(json.result.message);
               }
            });
    ...
    modal.getList = function () {
        ...
        core.json('mmanage/store/getlist', obj, function (json) {
            if (json.status != 1) {
                return
            }
