...
class Index_EweiShopV2Page extends SnsMobilePage
{
	public function main()
	{
		...
		$category = pdo_fetchall('select id,`name`,thumb,isrecommand from ' . tablename('ewei_shop_bottledoctorarticle_category') . ' where uniacid=:uniacid  and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$recommands = pdo_fetchall('select sb.id,sb.title,sb.logo,sb.`desc`  from ' . tablename('ewei_shop_sns_board') . " as sb\r\n\t\t\t\t\t\tleft join " . tablename('ewei_shop_bottledoctorarticle_category') . " as sc on sc.id = sb.cid\r\n\t\t\t\t\t\twhere sb.uniacid=:uniacid  and sb.status=1 and sc.enabled = 1 order by sb.displayorder desc", array(':uniacid' => $uniacid));
