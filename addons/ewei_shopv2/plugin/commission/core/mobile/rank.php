...
class Rank_EweiShopV2Page extends CommissionMobileLoginPage
{
		...
		public function userrank()
	{
		...
		$commission_rank = $this->commission_rank;
		//当前用户
		$user = $this->model->getInfo($_W['openid']);
		$userlevel11 =  pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $user['id'], ':uniacid' => $_W['uniacid']));

		$user['usertotal']=$user['level1']+$userlevel11;

		//当前用户直推人数

		$params = array();
		$sql = 'select dm.id,dm.nickname,dm.avatar,dm.openid from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ORDER BY dm.agenttime desc';
		$list = pdo_fetchall($sql, $params);

		foreach ($list as &$row) {
			$info = $this->model->getInfo($row['openid']);

			$level11 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $row['id'], ':uniacid' => $_W['uniacid']));
			if (1 <= $level) {
				$row['level1'] = $info['level1']+$level11;
			}
		}
		...
		usort($list, function($a,$b){
		    ...
		    return ($a['level1'] < $b['level1']) ? 1 : -1;

		});
	
		$user['paiming'] = array_search($user['id'], array_column($list,'id'))+1;
		$commission_title = "直推人数";

		switch ($this->commission_rank['type']) {
		case '0':
			$user_commission_title='累计佣金';
			break;

		case '1':
			$user_commission_title='已提现佣金';
			break;

		case '2':
			$user_commission_title=$this->commission_rank['title'];
			break;
		}

		if (count($list)>$commission_rank['num']) {
			$list=array_slice($list,0,$commission_rank['num']);
		}
		
		
		include $this->template('commission/rank');
	}
