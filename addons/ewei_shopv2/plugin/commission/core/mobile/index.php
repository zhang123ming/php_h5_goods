...
class Index_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		$mlevel = m('member')->getLevel($_W['openid']);
		if($mlevel['level']>=91){
			$this->message('请前往代理中心！');
		}
		$cansettle = (1 <= $member['credit3']) && (floatval($this->set['withdraw']) <= $member['credit3']);
		...
		$l1 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and uniacid=:uniacid and status=:status and isagent = 1 limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid'],'status'=>0));
		if ((2 <= $this->set['level']) && (0 < count($member['level1_agentids']))) {
			...
			$l2 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid and status=:status and isagent = 1 limit 1', array(':uniacid' => $_W['uniacid'],'status'=>0));
		}
		...
		if ((3 <= $this->set['level']) && (0 < count($member['level2_agentids']))) {
			...
			$l3 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid and status=:status and isagent = 1 limit 1', array(':uniacid' => $_W['uniacid'],'status'=>0));
		}
		...
		$openselect = false;
		$member['applypay'] =  pdo_fetchcolumn('select sum(commission) from ' . tablename('ewei_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid and status=3 limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
