...
class Agent_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($searchfield) && !empty($keyword)) {
			if ($searchfield == 'member') {
				$condition .= ' and ( dm.realname like :keyword or dm.nickname like :keyword or dm.mobile like :keyword or dm.openid like :keyword)';
				...
				if ($searchfield == 'parent') {
					if ($keyword == '总店') {
						...
						$condition .= ' and ( p.mobile like :keyword or p.nickname like :keyword or p.realname like :keyword or dm.openid like :keyword)';
						$params[':keyword'] = '%' . $keyword . '%';
					}
				}
			}
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar,f.follow as followed, f.unfollowtime,dm.agentcount as agentcount from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agentcount desc';
		...
		$commissionspay = $_W['shopset']['commission']['commissionsettletype'];
		foreach ($list as &$row) {
			// $info = $this->model->getInfo($row['openid'], array('total', 'pay'));
			$info = $this->model->getInfo2($row['openid']);
			...
			if(!empty($row['level'])){
				$level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid and id=:id limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$row['level']));
				if($level['level']>91){
					$lcondition = ' and agent'.$level['level'] . '='.$row['id'];
					$row['levelcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid ' . $lcondition . ' and level>0 ',array(':uniacid'=>$_W['uniacid']));
				}
			}
			#2020.5.5 14.50 李国梁
			if($row['agentcount'] !== $info['agentcount']){
				$row['levelcount'] = $info['agentcount'];
			}else{
				$row['levelcount'] = $row['agentcount'];
			}
			#end
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_update('ewei_shop_member', array('isagent' => 0, 'status' => 0,'diycommissionfields'=>'','diycommissiondata'=>''), array('id' => $member['id']));
	...
	public function user()
	{
		...
		$member = $this->model->getInfo2($agentid);
		$mlevel = pdo_fetch('select id,level,levelname from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid and id=:id and enabled=1 limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$member['level']));
		// var_dump($mlevel);die;
		...
		if($mlevel['level']>91){
			...
			$level11 = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where isagent=1 and `status`=0  and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
		}
		...
		if ($hasagent) {
			...
			foreach ($list as &$row) {
				...
				if($mlevel['level']>91){
					$sqls = "select sum(o.price) from " . tablename('ewei_shop_order') . ' o left join ' . tablename('ewei_shop_commission_order_log') . ' l on l.uniacid=o.uniacid and l.orderid=o.id ' . ' where o.uniacid=:uniacid and o.status=3 and l.money>0 and find_in_set(:id,o.fids) and l.openid=:openid ';
					$oparams = array(':uniacid'=>$_W['uniacid'],':id'=>$row['id'],':openid'=>$row['openid']);
					$moneycount = pdo_fetchcolumn($sqls,$oparams);
					$row['orderprice'] = $moneycount?$moneycount:0;
				} else {
					$row['orderprice'] = pdo_fetchcolumn('select sum(price) from ' . tablename('ewei_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
				}
				$mlevelname = m('member')->getLevel($row['openid']);
				$row['mlevelname'] = $mlevelname['levelname'];
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,agentid,nickname,realname,mobile,status,openid FROM ' . tablename('ewei_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
	...
	public function performance()
	{
		...
		foreach ($list as $k => &$v) {
			...
			$gsql = "select sum(og.total) from " . tablename('ewei_shop_order_goods') . " og left join " . tablename('ewei_shop_order') . ' o on o.uniacid=og.uniacid and o.id=og.orderid ' . ' left join ' . tablename('ewei_shop_commission_order_log') . ' l on l.uniacid=og.uniacid and l.goodsid=og.goodsid and l.orderid=o.id ' . ' where og.uniacid=:uniacid and o.status>=1 and l.money>0 and l.issend>=0 and find_in_set(:id,o.fids) and og.goodsid=:goodsid and l.openid=:openid ';
			// $vparams = array(':uniacid'=>$_W['uniacid'],':id'=>$member['id'],':goodsid'=>$v['goodsid'],':openid'=>$member['openid']);
			$v['total'] = pdo_fetchcolumn($gsql,$vparams);
			if(empty($v['total'])){
				$v['total'] = 0;
			}
		...
		// echo "<pre>";
		// var_dump($list);die;
		include $this->template('commission/performance');
	}
}
