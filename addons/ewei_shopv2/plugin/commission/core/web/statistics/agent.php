...
class Agent_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		foreach ($list as &$row) {
			...
			// $row['commission_pay'] = $info['commission_pay']+$info['commissiona_pay']+$record['pay'];
	...
	public function selectfifteen()
	{
		global $_W;
		global $_GPC;
		$id = $_GPC['id'];
		$member = m('member')->getMember($id);
		$list = pdo_fetchall('select find_in_set(\''.$id.'\',fids) as find,openid,fids from '.tablename('ewei_shop_member').' where  find_in_set(\''.$id.'\',fids) and uniacid=:uniacid',array(':uniacid'=>$_W['uniacid']));
		$arr = '';
		foreach($list as $v){
			$v['fids'] = explode(',',$v['fids']);
			if((count($v['fids'])-$v['find'])+1>=16){
				$arr .= '\''.$v['openid'].'\''.',';
			}
		}
		if(strlen($arr)){
			$count = count(explode(',',rtrim($arr,',')));	
		}
		$total = pdo_fetchcolumn('select sum(price) from '.tablename('ewei_shop_order').' where openid in ('.rtrim($arr,',').') and uniacid = :uniacid ',array(':uniacid'=>$_W['uniacid']));
		$count = empty($count)?0:$count;
		$total = empty($total)?0:$total;
		include $this->template('commission/statistics/selectfifteen');
	}
}
