...
class DiypageModel extends PluginModel
{
							...
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										...
										$goods = pdo_fetchall('select id, title, subtitle, thumb, productprice, minprice, total, showlevels, showgroups, bargain, merchid, sales, salesreal,commission1_rate,nocommission from ' . tablename('ewei_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
									...
									if (!empty($goods) && is_array($goods)) {
										foreach ($goodsids as $goodsid) {
											foreach ($goods as $index => $good) {
												if ($good['id'] == $goodsid) {
													if($good['commission1_rate'] && $good['minprice'] && empty($good['nocommission'])){
														$good['commission'] = round($good['minprice']*$good['commission1_rate']/100,2);
													}
													$childid = mt_rand(100000000,999999999);
													$childid = 'C' . $childid;
													$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']),'commission'=>$good['commission']);
												}
											}
										}
									}
			...
			if ($mobile && !empty($page['data']['items']) && is_array($page['data']['items'])) {
				foreach ($page['data']['items'] as $itemid => &$item) {
					if ($item['id'] == 'goods') {
						if ($item['params']['goodsdata'] == '0') {
							...
							if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) {
								...
								if (!empty($group) && !empty($group['goodsids'])) {
									...
									$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice,marketprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice,commission1_rate,nocommission from ' . tablename('ewei_shop_goods') . ' where id in( ' . $goodsids . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $limit, array(':uniacid' => $_W['uniacid']));
									if (!empty($goods) && is_array($goods)) {
										foreach ($goods as $index => $good) {
											$showgoods = m('goods')->visit($good, $this->member);
											if(empty($good['nocommission'])){
												if(!empty($good['minprice'])){
													$good['commission'] = round($good['minprice']*$good['commission1_rate']/100,2);
												} else {
													$good['commission'] = round($good['marketprice']*$good['commission1_rate']/100,2);
												}
											} else {
												$good['commission'] = 0;
											}
											if (!empty($showgoods)) {
												$childid = rand(1000000000, 9999999999);
												$childid = 'C' . $childid;
												$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'sales' => $good['sales'] + $good['salesreal'],'commission'=>$good['commission']);
											}
										}
									}
								}
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (!empty($goods) && is_array($goods)) {
										...
										foreach ($goods as $index => $good) {
											...
											if (!empty($showgoods)) {
												$childid = rand(100000000, 999999999);
												$childid = 'C' . $childid;
												$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']),'commission'=>$good['commission']);
											}
										}
						...
						if (is_weixin()&&empty($shopset['isopenlogin'])) {
							unset($page['data']['items'][$itemid]);
						}
						else {
							$member = $this->member;
						...
						$item['params']['canwithdraw'] = number_format($member['credit3'], 2);
						$item['params']['upname'] = $commission['set']['texts']['up'];
						$item['params']['upmember'] = empty($member['up']) ? '总店' : $member['up']['nickname'];
						$item['params']['upid'] = empty($member['up']) ? '' : $member['up']['id'];
						$item['params']['allwithdraw'] = round($member['commission_total'], 2);

						// $getcommissions = cache_load('getcommissions');					
						// $daycommission = 0;
						// $daycommissiona = 0;
						// $monthcommission = 0;
						// $monthcommissiona = 0;
						// foreach ($getcommissions as $key => $monthcommissions) {
						// 	$daystart = strtotime(date('Y-m-d 00:00:00'));
						// 	$monthend = strtotime(date('Y-m-d H:i:s'));
						// 	$daycreatetime = strtotime($monthcommissions['createtime']);
						// 	if($daycreatetime>=$daystart &&  $daycreatetime < $monthend){
						// 		$daycommission += round($monthcommissions['commission'],2);
						// 		$daycommissiona += round($monthcommissions['commissiona'],2);	
						// 	}

						// 	$monthcommission += round($monthcommissions['commission'],2);
						// 	$monthcommissiona += round($monthcommissions['commissiona'],2);
								
						// }
						$item['params']['daycommissionvalue'] = $member['commission_day_total'];
						$item['params']['monthcommissionvalue'] = $member['commission_month_total'];
						$item['params']['allamountcommissionvalue'] = $member['commision_allamount_total'];
	...
	public function getPageType($type = NULL)
	{
		...
		$pagetype = array(
			...
			6  => array('name' => ''.$_W['shopset']['trade']['credittext'].'商城', 'pagetype' => 'plu', 'class' => 'info'),
	...
	public function calculate($str = NULL, $pagetype = false)
	{
		...
		if (strexists($str, 'r=commission')) {
			...
			if ($plugin_commissiom) {
				...
				$member = p('commission')->getInfo2($_W['openid']);

				if (strexists($str, 'r=commission.cashbe')) {
					$this->commission['commission_total'] = $member['commission_total'];
					$this->commission['credit3'] = $member['credit3'];//可提现佣金
				}

				if (strexists($str, 'r=commission.order')) {
					$this->commission['ordercount0'] = $member['commission_count'];
				}
	...
	public function judge($type = NULL, $str = NULL)
	{
		...
		if ($type == 'tipnum') {
			if (strexists($str, 'r=commission.cashbe')) {
				return $this->commission['credit3'];//可提现佣金
			}
	...
	public function url($str)
	{
		...
		$str = preg_replace_callback('/"linkurl"\\:"([^\\\'" ]+)"/', function($matches) use($mid) {
			$preg = $matches[1];
			if (!strexists($preg,'s100.kemanduo.cc')) {
				return '"linkurl":"' . $preg . '"';
			}
