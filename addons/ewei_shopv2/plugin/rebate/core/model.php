...
class RebateModel extends PluginModel
{
		...
		if (!empty($lotterylist)) {
			...
			if ($type == 5) {
				...
				foreach ($lotterylist as $key => $value) {
					...
					if ($join_info['money'] < $value['task_data']['payments_money']) {
						$join_info['money'] = $value['task_data']['payments_money'];
						$join_info['num'] = $value['task_data']['payments_num'];
						$join_info['lottery_id'] = $value['lottery_id'];
					}
				...
				if (!empty($join_info['lottery_id'])) {
					...
					while ($i <= $join_info['num']) {
						$join_data = array('uniacid' => $_W['uniacid'], 'join_user' => $openid, 'lottery_id' => $join_info['lottery_id'], 'lottery_num' => 1, 'lottery_tag' => '支付满' . $join_info['money'] . '元,赠' . $join_info['num'] . '次', 'addtime' => time());
						pdo_insert('ewei_shop_lottery_join', $join_data);
