...
class PosterProcessor extends PluginProcessor
{
	...
	public function respond($obj = NULL)
	{
		...
		$obj->member = $this->model->checkMember($message['from'],'',$qr['openid']);
	...
	private function responseSubscribe($obj)
	{
		...
		if (empty($log) && ($openid != $qr['openid'])) {
			...
			if (0 < $poster['subcredit']) {
				m('member')->setCredit($openid, 'credit1', $poster['subcredit'], array(0, '扫码关注'.$_W['shopset']['trade']['credittext'].'+' . $poster['subcredit']));
			}
			...
			if (empty($reward_totle['reccredit_totle']) || (intval($reward_real['reccredit_totle']) <= intval($reward_totle['reccredit_totle']))) {
				if (0 < $poster['reccredit']) {
					m('member')->setCredit($qr['openid'], 'credit1', $poster['reccredit'], array(0, '推荐扫码关注'.$_W['shopset']['trade']['credittext'].'+' . $poster['reccredit']));
				}
			}
	...
	private function commission($poster, $member, $qrmember)
	{
		...
		$time = time();
		$p = p('commission');
		if ($qrmember[id]==$member[id]) {
			return false;
		}
		if ($p) {
			$cset = $p->getSet();
			$p->checkAgent($openid);
			$list = pdo_fetchall('select id,merchid from '.tablename('ewei_shop_merch_member').' where openid=:openid',array(':openid'=>$qrmember['openid']));
			if(count($list)>0){
				foreach($list as $v){
			...
			if (!empty($cset)) {
				if ($member['isagent'] != 1) {
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) {
						...
						$data= p('commission')->getAgentId($qrmember['id'],$member);
