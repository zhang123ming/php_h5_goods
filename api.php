...
class WeEngine {
	...
	public function start() {
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
			...
			ob_end_clean();
			$this->transferCustomerService($postStr);
			exit();
		}
		WeUtility::logging('waring', 'Request Failed');
		exit('Request Failed');
	...
	private function booking($message) {
		global $_W;
		//请求防伪出货 发送代理消息开始
		// $fromKey = explode('_', $message['eventkey']);
		// if ($fromKey[0]=='apply'||$fromKey[1]=='apply') {
		// 	$applyMsg = ihttp_request('http://yunyue.guangzhoubaidu.com/index/wechat/sendmessage.html?message='.$message['eventkey'],$message);
		// 	$applyMsg = json_decode($applyMsg['content'],true);
		// }
		// 结束
...
EOF;
	...
	转发消息到客服
	**/
	private function transferCustomerService($message){
		$arr = xml2array($message);
		unset($arr['Content']);
		unset($arr['MsgId']);
		list($arr['ToUserName'],$arr['FromUserName'])=[$arr['FromUserName'],$arr['ToUserName']];
		$arr['MsgType']='transfer_customer_service';
		exit(array2xml($arr));
	}
