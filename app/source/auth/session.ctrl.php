...
$dos = array('openid', 'userinfo', 'touch','mobile');
...
if ($do == 'openid') {
	...
	$encrypt_data = $_GPC['data'];
	$iv = $_GPC['iv'];
	if (empty($_SESSION['session_key']) || empty($encrypt_data) || empty($iv)) {
		$account_api->result(1, '请先登录');
	}
	
	$mobileinfo = $account_api->pkcs7Encode($encrypt_data, $iv);

	$member_update = array(
		"mobile"=>$mobileinfo['purePhoneNumber']
	);
	if ($_W['member']['uid']) {
		pdo_update('mc_members',$member_update,array('uid'=>$_W['member']['uid']));
		$account_api->result(0,'');
	}else{
		$account_api->result(1,'更新失败');
	}
