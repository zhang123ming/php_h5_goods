...
if($do == 'post') {
	...
	$modules = pdo_fetchall('SELECT type, mid, title FROM ' .tablename('modules'). ' WHERE type <> :type  order by title asc' ,array(':type' => 'system'),'title');
...
if($do == 'list') {
	...
	$modules = pdo_fetchall('SELECT * FROM ' . tablename('modules') . ' WHERE type <> :type order by title asc', array(':type' => 'system'), 'title');
	template('website/wenda/wenda');
}
