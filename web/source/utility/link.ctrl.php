...
if ($do == 'news') {
	...
	$searchs = " and nickname like '%".$search."%' " ;
	$pindex = max(1, intval($_GPC['page']));
	$sql = "SELECT * FROM ". tablename('hulu_like_user')." where uniacid = :uniacid ".$searchs." ORDER BY uid DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize;
	$result['list'] = pdo_fetchall($sql, array( ':uniacid' => $_W['uniacid']));
	if (!empty($result['list'])) {
		$sql = "SELECT COUNT(*) FROM ". tablename('hulu_like_user'). " where uniacid = :uniacid ".$searchs;
		$total = pdo_fetchcolumn($sql, array(':news' => 'news', ':uniacid' => $_W['uniacid']));
		$result['pager'] = pagination($total, $pindex, $psize, '', array('before' => '2', 'after' => '3', 'ajaxcallback'=>'null'));
	}
}
...
if ($do == 'article') {
	...
	$search = $_GPC['search'] ? $_GPC['search'] : '';
	if($search){
		$searchs = " and active_id = {$search} " ;
	}else{
		$searchs = '';
	}
	$pindex = max(1, intval($_GPC['page']));
	$sql = "SELECT * FROM ". tablename('hulu_like_active')." where uniacid = :uniacid ".$searchs." ORDER BY active_id DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize;
	$result['list'] = pdo_fetchall($sql, array( ':uniacid' => $_W['uniacid']));
	if (!empty($result['list'])) {
		$sql = "SELECT COUNT(*) FROM ". tablename('hulu_like_active'). " where uniacid = :uniacid ".$searchs;
		$total = pdo_fetchcolumn($sql, array(':news' => 'news', ':uniacid' => $_W['uniacid']));
		$result['pager'] = pagination($total, $pindex, $psize, '', array('before' => '2', 'after' => '3', 'ajaxcallback'=>'null'));
	}
}
