...
if(!$usergroup['domain']){
	itoast('抱歉，您不能使用此功能，请联系管理员开通',url('system/welcome',error));
}
...
$settings = $setting ? iunserializer($setting['copyright']) : array();
	if (checksubmit('submit')) {
		...
		itoast('更新设置成功！', url('user/set/copyright'));
	}
...
if(checksubmit('submit')){
	...
	if (!empty($_W['page']['copyright']['sitehost'])) {
		if (trim($_GPC['yuming']) == $_W['page']['copyright']['sitehost']) {
			itoast('请填写其他域名，不要填本平台域名！', referer(), 'error');
		}
	}
	...
	if(empty($setting)){
		...
		itoast('域名设置成功！', url('user/set/yuming'));
}
...
$_W['page']['title'] = '高级工具 - 版权设置';
$usergroup = pdo_get('users_group',array('id'=>$_W['user']['groupid']));
if(!$usergroup['domain']){itoast('抱歉，您不能使用此功能，请联系管理员开通',url('system/welcome',error));}
...
if(checksubmit('submit')){
	...
	if(empty($setting)){
		...
		itoast('后台皮肤设置成功！', url('user/set/pifu'));
}
template('user/pifu');
}