...
if($do == 'base') {
	...
	if($_W['ispost'] && $_W['isajax']) {
		...
		switch ($type) {
			...
			case 'jointype':
				...
				break;
			case 'istest':
				$istest = intval($_GPC['request_data']);
				$result = pdo_update('account', array('istest' => $istest), array('uniacid' => $uniacid));
				break;
			case 'highest_visit':
			...
			break;
			case 'testtime':
					$testtime = strtotime($_GPC['testtime']);
					$result = pdo_update('account', array('testtime' => $testtime), array('uniacid' => $uniacid));
		}
		if(!in_array($type, array('qrcodeimgsrc', 'headimgsrc', 'name', 'endtime', 'jointype', 'highest_visit','istest','testtime'))) {
	...
	if (empty($account['testtime'])) {
		unset($account['testtime']);
	}else{
		$account['testtime'] = date('Y-m-d', $account['testtime']);//测试提示时间
	}
