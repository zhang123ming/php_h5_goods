...
if ($do == 'manage') {
        ...
        if ($op == 'delete') {
            ...
            $psize      = 20;
            $condition  = $_W['isfounder'] ? ' WHERE (uniacid = :uniacid or uniacid=0) ' : ' WHERE uniacid = :uniacid ';
            $params     = array(
                ':uniacid' => $_W['uniacid']
            );
            $query_name = array(
...
function saveBindSet($bind, $delete = false)
    {
		...
		if($bind['uniacid']){
			$file = $path . $bind['uniacid']. '.php';
		}else{
			$file = $path . $bind['domain']. '.php';
		}
