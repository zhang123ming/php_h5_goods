...
if ($do == 'get_setting') {
	...
	if(!is_array($pay_setting) || empty($pay_setting)) {
		$pay_setting = array(
			...
			'wechat_qukuailian' => array('switch' => false, 'mchid' => '1', 'signkey' => '1'),
...
if ($do == 'save_setting') {
	...
	if ($type == 'qukuailian'){
		$qukuailian = $_GPC['qukuailian'];
		$param = array(
			'switch' => $qukuailian['switch'] == 'false'? false : true,
			'merid' => $qukuailian['switch'] == 'false'?'':$qukuailian['merid'],
			'signkey' => $qukuailian['switch'] == 'false'?'':$qukuailian['signkey']
		);
		if($param['switch'] && (empty($param['merid']) || empty($param['signkey']))) {
			itoast('请输入完整的区块链支付接口信息.', referer(), 'error');
		}
	}
...
if ($do == 'display') {
	...
	if(empty($pay_setting['qukuailian'])){
		$pay_setting['qukuailian'] = array('switch' => false);
	}
		...
		if (empty($pay_setting['wechat_qukuailian'])) {
			$pay_setting['wechat_qukuailian'] = array('switch' => false, 'mchid' => '', 'signkey' => '');
		}
