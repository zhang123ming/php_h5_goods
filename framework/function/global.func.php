...
function sendsms($mobile,$content=array(),$mobanid='',$msg=''){
	...
	if($msg['type']==2){
		...
		require IA_ROOT.'/framework/library/aliyunsms/SignatureHelper.php';
		if (empty($msg['appkey'])) {
			...
			$msg['secret'] = $sms_default['aliyun_appsecret'];
			$msg['appkey'] = $sms_default['aliyun_appkey'];
			$msg['qianming'] = '短信通';
			$mobanid = 'SMS_106600025';
		}
		$params = json_encode($content);
		$result = sendAliMsg($mobile,$params,$mobanid,$msg['qianming'],$msg['appkey'],$msg['secret']);
		$msg = $result->Code;
		if($msg!='OK'){
