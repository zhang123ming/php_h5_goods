<?php defined('IN_IA') or exit('Access Denied');?><div id="ajax">
...
<div class="tab-pane active" role="tabpanel">
	<table class="table table-hover">
			...
			<th style="width: 30%; text-align: right">
				<div class="input-group input-group-sm">
					<input type="text" id="search" name="search" class="form-control">
						...
						<span class="input-group-btn">
							<button id="dosearch" class="btn btn-default" type="button"><i class="fa fa-search"></i></button>
							<input type="hidden" id="url" value="../web/index.php?c=utility&a=link&do=news&callback=newsLinkComplete&dotype=search">
						</span>
				</div>
			</th>
		</tr>
		...
		<?php  if(is_array($result['list'])) { foreach($result['list'] as $list) { ?>
		<tr>
			<td><img style="width:80px;height:auto;" src="<?php  echo $list['avatar'];?>" alt=""></td>
			<td><?php  echo $list['nickname'];?></td>
			<td><button class="btn btn-default select" onclick="clicklink('../app/index.php?i=<?php  echo $_W['uniacid'];?>&c=entry&vid=<?php  echo $list['uid'];?>&do=view&m=hulu_like')">选取</button></td>
		</tr>
		<?php  } } ?>
		</tbody>
	</table>
...
<div class="tab-pane active" role="tabpanel">
	<div id="articlelist">
		<table class="table table-hover">
			...
			<tr>
				...
				<th style="width: 30%; text-align: right">
					...
					<input type="text" id="search" name="search" class="form-control">
						...
						<span class="input-group-btn">
							<button id="dosearch" class="btn btn-default" type="button"><i class="fa fa-search"></i></button>
							<input type="hidden" id="url" value="../web/index.php?c=utility&a=link&do=article&callback=articleLinkComplete&dotype=search">
						</span>
				</div>
				</th>
			</tr>
			...
			<?php  if(is_array($result['list'])) { foreach($result['list'] as $list) { ?>
			<tr>
				<td><a href="#"><?php  echo $list['active_id'];?></a></td>
				<td><a href="#"><?php  echo $list['active_title'];?></a></td>
				<td><button class="btn btn-default select" onclick="clicklink('../app/index.php?i=<?php  echo $_W['uniacid'];?>&c=entry&active_id=<?php  echo $list['active_id'];?>&do=active_view&m=hulu_like')">选取</button></td>
			</tr>
			<?php  } } ?>
			</tbody>
		</table>
...
<script>
	$("#dosearch").click(function(){
		...
		var search = $(this).parent().parent().find("#search").val();
		$.ajax({
	        url: url,
	        type: 'post',
	        dataType: 'text',
