...
<form <?php if( ce('goods' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
	<input type="hidden" id="docopy" name="docopy" value="<?php  echo $_GPC['copy'];?>" />
    ...
    <div class="tabs-container tab-goods">
        <div class="tabs-left">
            <ul class="nav nav-tabs" id="myTab">
                ...
                <?php  } ?>
                <?php  if(!empty($abonus_set['open'])) { ?>
                <li <?php  if($_GPC['tab']=='abonus') { ?>class="active"<?php  } ?>><a href="#tab_abonus" id="a_abonus">区域分红</a></li>
                <?php  } ?>

				<?php  if($merchid == 0 ) { ?>
				<li <?php  if($_GPC['tab']=='verify') { ?>class="active"<?php  } ?> id="tab_nav_verify" <?php  if($item['type']==3||$item['type']==5||($item['type']==2&&!empty($item['virtualsend']))) { ?>style="display:none;"<?php  } ?> style="display: none;"><a href="#tab_verify">线下核销</a></li>

				<?php  } ?>

                <?php  if(p('diyform')) { ?>
                <li <?php  if($_GPC['tab']=='diyform') { ?>class="active"<?php  } ?> style="display: none;"><a href="#tab_diyform">自定义表单</a></li>
                <?php  } ?>

                <?php  if($merchid == 0) { ?>
                <li <?php  if($_GPC['tab']=='detaildiy') { ?>class="active"<?php  } ?> style="display: none;"><a href="#tab_detaildiy">店铺信息</a></li>
                <?php  } ?>

                <?php  if(p('diypage')) { ?>
                <li <?php  if($_GPC['tab']=='diypage') { ?>class="active"<?php  } ?> style="display: none;"><a href="#tab_diypage">自定义模板</a></li>
                <?php  } ?>
                <?php  if(p('ccard')) { ?>
            ...
            <div class="tab-content  ">
                ...
                <div class="tab-pane  <?php  if($_GPC['tab']=='option') { ?>active<?php  } ?>" id="tab_option"> <div class="panel-body111"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('goods/tab/option', TEMPLATE_INCLUDEPATH)) : (include template('goods/tab/option', TEMPLATE_INCLUDEPATH));?></div></div>
                ...
                <?php  } ?>
                <?php  if(p('abonus')&&!empty($abonus_set['open'])) { ?>
                	<div class="tab-pane  <?php  if($_GPC['tab']=='abonus') { ?>active<?php  } ?>" id="tab_abonus"> <div class="panel-body"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('goods/tab/abonus', TEMPLATE_INCLUDEPATH)) : (include template('goods/tab/abonus', TEMPLATE_INCLUDEPATH));?></div></div>
                <?php  } ?>
                <?php  if($merchid == 0) { ?>
...
<script type="text/javascript">
	...
	$(function () {
		...
		$(':radio[name=isprepay]').click(function () {
			window.isprepay = $("input[name='isprepay']:checked").val();

			if(window.isprepay==0){
				$(".prepay_info").hide();
			}else{
				$(".prepay_info").show();
			}
		...
		$(":checkbox[name='protocolshow']").click(function () {
		...
		$(':radio[name=protocolshow]').click(function () {
			window.protocolshow = $("input[name='protocolshow']:checked").val();

			if(window.protocolshow=='1'){
				$('.protocolcontent').show();
			} else {
				$('.protocolcontent').hide();
			}
	...
	function optionArray()
	{
		...
		var option_preprice = new Array();
		$('.option_preprice').each(function (index,item) {
			option_preprice.push($(item).val());
		});
		var option_period = new Array();
		$('.option_period').each(function (index,item) {
			option_period.push($(item).val());
		});
		...
		var options = {
			...
			option_preprice : option_preprice,
	...
	function type_change(type) {
		...
		if (type == 6) {
			$("#mallproperty").hide();
			$("#zhaoshangproperty").show();
		}else{
			$("#zhaoshangproperty").hide();
			$("#mallproperty").show();
		}
