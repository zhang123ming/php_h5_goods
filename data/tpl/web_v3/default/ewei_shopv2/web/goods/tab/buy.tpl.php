...
<div class="region-goods-details row">
	...
	<div class="form-group discounts-info">
        <div class="col-sm-12 alert alert-primary">
            <div class="text-danger alert alert-primary">
                优先读取会员单次购买限制,若该没有设置会员等级单次购买限制则读取普通单次购买限制
                <br>
                <span class="text-danger">根据会员等级设置单次最小购买/单次最大购买:</span>
            </div>
            <?php  if(is_array($levellimit)) { foreach($levellimit as $level) { ?>
	        <div class="form-group">
	            ...
	            <div class="col-sm-6 col-xs-12">
	                <div class="input-group">
	                    ...
	                    <input type="number" name="limitbuy[<?php  echo $level['minlimit'];?>]" class="form-control" value="<?php  echo $item['limitbuy'][$level['minlimit']];?>" data-min="0" data-max="10" />
	                    <div class="input-group-addon">件 单次最多购买</div>
	                    <input type="number" name="limitbuy[<?php  echo $level['maxlimit'];?>]" class="form-control" value="<?php  echo $item['limitbuy'][$level['maxlimit']];?>" />
	...
	<div class="form-group">
		<label class="col-sm-2 control-label">限制按月购买</label>

		<div class="col-sm-6 col-xs-12">
			<?php if( ce('goods' ,$item) ) { ?>
			<div class="input-group">
				<label for="usermonthbuy0" class="radio-inline"><input type="radio" name="usermonthbuy" value="1" <?php  if(!empty($item['usermonthbuy'])) { ?>checked<?php  } ?>/> 是</label>
				<label for="usermonthbuy1" class="radio-inline"><input type="radio" name="usermonthbuy" value="0" <?php  if(empty($item['usermonthbuy'])) { ?>checked<?php  } ?>/> 否</label>
			</div>
			<span class="help-block">用户购买过的此商品按月限制,此选项和最多购买配合使用</span>
			<?php  } else { ?>
			<div class='form-control-static'>
				<?php  if(empty($item['usermonthbuy'])) { ?>是<?php  } else { ?>否<?php  } ?>
			</div>
			<?php  } ?>
