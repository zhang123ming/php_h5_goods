    ...
    <div class="region-goods-right col-sm-10">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9 col-xs-12">
                ...
                <!-- <label class="radio-inline"><input type="radio" name="type" value="6" <?php  if(!empty($item['id'])) { ?>disabled<?php  } ?>  <?php  if($item['type'] == 6) { ?>checked="true"<?php  } ?>  onclick="type_change(6);" /> 招商商品</label> -->
...
<div class="region-goods-details row">
    ...
    <div class="region-goods-right col-sm-10">
        ...
        <div class="form-group dispatch_info" <?php  if(!empty($item['type']) && $item['type'] > 1) { ?>style="display: none;"<?php  } ?>>
            <label class="col-sm-2 control-label">预付设置</label>
            <div class="col-sm-9 col-xs-12">
                <?php if( ce('goods' ,$item) ) { ?>
                <label class="radio-inline"><input type="radio" name="isprepay" value="0"  <?php  if(empty($item['isprepay'])) { ?>checked="true"<?php  } ?>/> 否</label>
                <label class="radio-inline"><input type="radio" name="isprepay" value="1" <?php  if($item['isprepay'] == 1) { ?>checked="true"<?php  } ?>   /> 是</label>
                <span class="help-block">提示：是否开启商品预付设置</span>
                <?php  } else { ?>
                <div class='form-control-static'><?php  if(empty($item['isprepay'])) { ?>否<?php  } else { ?>是<?php  } ?></div>
                <?php  } ?>
            </div>
        </div>
        
        <div class="form-group prepay_info" <?php  if(!$item['prepayprice']) { ?> style="display: none;"<?php  } ?>>
            <label class="col-sm-2 control-label must">预付价格</label>
            <div class="col-sm-9 col-xs-12">
                <?php if( ce('goods' ,$item) ) { ?>
                <div class="input-group">
                    <input type="text" name="prepayprice" id="prepayprice" class="form-control" value="<?php  echo $item['prepayprice'];?>" />
                    <span class="input-group-addon">元</span>
                </div>
                <span class='help-block'>尽量填写完整，有助于于商品销售的数据分析</span>
                <?php  } else { ?>
                <div class='form-control-static'>预付：<?php  echo $item['prepayprice'];?> 元</div>
                <?php  } ?>
            </div>
        </div>
        ...
        <div class="form-group">
    <label class="col-sm-2 control-label" style="margin-right:10px;display: none;" >是否为升级礼包</label>
    <label for="isupgradebag" class="checkbox-inline">
        <input type="checkbox" name="isupgradebag" value="<?php  if($item['isupgradebag']) { ?><?php  echo $item['isupgradebag'];?><?php  } else { ?>0<?php  } ?>" id="upgradebag" <?php  if($item['isupgradebag'] == 1) { ?>checked="true"<?php  } ?> />
    </label>
    </div>
    <div class="form-group" id="isupgradebagchild" <?php  if($item['isupgradebag'] != 1) { ?>style="display: none;"<?php  } ?>>
        <label class="col-sm-2 control-label">选择礼包等级</label>
        <div class="col-sm-8 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <select   name="upgradebag[memberlevel]" style='width:auto;padding: 0 2%;margin-right:7px;' >
                <?php  if(empty($levelnumber)) { ?>
                <option value="default" style="text-align:center;" >请设置会员等级</option>
                <?php  } else { ?>
                <?php  if(is_array($levelnumber)) { foreach($levelnumber as $k=>$v) { ?>
                <option value="<?php  echo $v['level'];?>" style="text-align:center;" <?php  if($item['upgradebag']['memberlevel'] == $v['level']) { ?>selected<?php  } ?> ><?php  echo $v['levelname'];?></option>
                <?php  } } ?>
                <?php  } ?>
            </select>
            <span style="margin-right:20px;">会员等级</span>
            <select   name="upgradebag[commissionlevel]" style='width:auto;padding: 0 2%;margin-right:7px;' >
                <?php  if(empty($commissionnumber)) { ?>
                <option value="default" style="text-align:center;" >请设置分销商等级</option>
                <?php  } else { ?>
                <?php  if(is_array($commissionnumber)) { foreach($commissionnumber as $k=>$v) { ?>
                <option value="<?php  echo $v['level'];?>" style="text-align:center;" <?php  if($item['upgradebag']['commissionlevel'] == $v['level']) { ?>selected<?php  } ?> ><?php  echo $v['levelname'];?></option>
                <?php  } ?>
                <?php  } } ?>
            </select>
            <span>分销商等级</span>
        ...
        <div class="form-group price" <?php  if($item['type']==4) { ?>  style="display: none"<?php  } ?>>
            ...
            <div class="col-sm-9 col-xs-12">
                ...
                <div class="input-group">
                    ...
                    <span class="input-group-addon">元 </span>
                    <span class="input-group-addon">毛利指数</span>
                    <input type="text" name="grossindex" id="grossindex" class="form-control" value="<?php  echo $item['grossindex'];?>" />
                    <span class="input-group-addon">元</span> -->
...
<div class="region-goods-details row" id="zhaoshangproperty" style="<?php  if($item['type']!=6) { ?>display: none;<?php  } ?>"  >
    <div class="region-goods-left col-sm-2">
        招商属性
    </div>
    <div class="region-goods-right col-sm-10">
        <div class="form-group">
            <label class="col-sm-2 control-label">招商政策</label>
            <div class="col-sm-8">
                <div class="">
                    <?php if( ce('goods' ,$item) ) { ?>
                    <?php  echo tpl_ueditor('shopinpolicy',$item['shopinpolicy'],array('height'=>'300'))?>
                    <?php  } else { ?>
                    <textarea id='shopinpolicy' style='display:none'><?php  echo $item['shopinpolicy'];?></textarea>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <div class="">
                    <?php if( ce('goods' ,$item) ) { ?>
                    <?php  echo tpl_ueditor('shopintroduce',$item['shopintroduce'],array('height'=>'300'))?>
                    <?php  } else { ?>
                    <textarea id='shopintroduce' style='display:none'><?php  echo $item['shopintroduce'];?></textarea>
                    <a href='javascript:preview_html("#detail")' class="btn btn-default">查看内容</a>
                    <?php  } ?>
                </div>
            </div>
...
<div class="region-goods-details row" id="mallproperty"  style="<?php  if($item['type']==6) { ?>display: none;<?php  } ?>">
    <div class="region-goods-left col-sm-2">
        商品属性
    </div>
    <div class="region-goods-right col-sm-10">
        ...
        <div class="form-group" style="display: none;">
            ...
            <div class="col-sm-9 col-xs-12">
                <?php if( ce('goods' ,$item) ) { ?>
                <label class="col-lg-2" style="float: left;" <?php  if($item['isverify'] == 2 || $item['type'] == 2 || $item['type'] == 3) { ?>style="display:none;"<?php  } ?>>
                    <input class="checkFixedMoney" type="checkbox" name="cash" value="2" <?php  if($item['cash'] =='2') { ?>checked="checked"<?php  } ?>/> 支持
                    <input type="hidden" id="fixedPayMethod" name="fixed_pay" value=""/>
                    <input type="hidden" id="fixedMoney" name="fixed_money" value=""/>
                </label>
                <!--按照固定金额或比例支付-->
                <div class="fixedMoney_Div">
                    <div class="col-lg-4">
                        <div class="input-group">
                          <span class="input-group-addon">
                            <input name="checkeState" class="fixedMoney_rdo01" <?php  if($item['fixed_pay']==1) { ?> checked="checked"<?php  } ?> value="1" type="radio">固定订金
                        </span>
                        <input type="text" <?php  if($item['fixed_pay']==1) { ?> value="<?php  echo $item['fixed_money'];?>" <?php  } ?> class="form-control fixed_money1">
                        <span class="input-group-addon">元</span>
                    </div>
                </div>
                <!-- <div class="col-lg-6">
                    <div class="input-group">
                        <span class="input-group-addon">
                          <input name="checkeState" class="fixedMoney_rdo02" <?php  if($item['fixed_pay']==2) { ?> checked="checked"<?php  } ?> value="2" type="radio">价格比例
                      </span>
                      <input type="text" <?php  if($item['fixed_pay']==2) { ?> value="<?php  echo $item['fixed_money'];?>"<?php  } ?> class="form-control fixed_money2">
...
<div class="form-group" style="display: none;">
    <label class="col-sm-2 control-label">单品税费</label>
    <div class="col-sm-8">
        <?php if( ce('goods' ,$item) ) { ?>
        <div class='input-group fixsingle-input-group'>
            <input type="text" name="taxation" value="<?php  echo $item['taxation'];?>" class="form-control"/>
            <span class="input-group-addon">元</span>
        </div>
        <span class="help-block">如单品价格1000，税费100</span>
        <span class="help-block">海外版小程序的税费统计，不纳入付款金额，只用于显示</span>
        <?php  } ?>
    </div>
</div>
...
<div class="entity" <?php  if(($item['type'] == 2 || $item['type'] == 3 || $item['type'] == 10)) { ?>style="display: none;"<?php  } ?>>
    ...
    <!-- 备货时间 -->
    <div class="form-group">
        <label class="col-sm-2 control-label">预计发货时间</label>
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <div class="input-group">
                <span class="input-group-addon">下单后</span>
                <input type="number" name="preparegoodstime" class="form-control valid" value="<?php  echo $item['preparegoodstime'];?>">
                <span class="input-group-addon">天</span>
            </div>
            <?php  } ?>
            <div class="help-block">此处设置备货时间,即是用户下单后可以看到的预计发货时间</div>
        </div>
    </div>
    <!-- 备货时间 -->
...
<script language="javascript">
    ...
    $('#upgradebag').click(function(){
        if($(this).val()=='0'){
            $(this).val('1');
            $("#isupgradebagchild").css('display','block');
        }else{
            $(this).val('0');
             $("#isupgradebagchild").css('display','none');
        }
    });
    ...
    function setFixedMoneyStyle(){
        if($(".checkFixedMoney").is(":checked")){
            $(".fixedMoney_Div").show();
        }else {
            $(".fixedMoney_Div").hide();
        }
...
//验证用户输入订金价格的合法性
function checkDingJin(dingjin){
    if(!dingjin){return};
    var regStr = "(^[1-9]([0-9]+)?(\\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\\.[0-9]([0-9])?$)";
    var reg = new RegExp(regStr);
    if(!reg.exec(dingjin)){
        return alert("固定金额价格有误，请重新输入！");
    }
};

//给隐藏域表单1：设置订金方式、2：预付订金额度
function setDingJinItem(){
    var checkFixedMoney = $(".checkFixedMoney").is(":checked");
    var checkState1 = $(".fixedMoney_rdo01").is(":checked");
    var checkState2 = $(".fixedMoney_rdo02").is(":checked");
    if(checkFixedMoney){
        if(checkState1){
            var fixed_money = $(".fixed_money1").val();
            $("#fixedPayMethod").val(1);
            $("#fixedMoney").val(fixed_money);
        }else if(checkState2){
            var fixed_money = $(".fixed_money2").val();
            $("#fixedPayMethod").val(2);
            $("#fixedMoney").val(fixed_money);
        }
    }
};
...
//更改订金支付方式 1
$(".fixed_money1").on("blur",function(){
    var fixed_money = $(".fixed_money1").val();
    checkDingJin(fixed_money);
    setDingJinItem();
});
//更改订金支付方式 2
$(".fixed_money2").on("blur",function(){
    var fixed_money = $(".fixed_money2").val();
    checkDingJin(fixed_money);
    setDingJinItem();
});
...
$(function(){
    $(".checkFixedMoney").on("click",function(){
        setFixedMoneyStyle();
    })
});
</script>
