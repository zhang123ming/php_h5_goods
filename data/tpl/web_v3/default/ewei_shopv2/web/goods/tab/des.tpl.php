...
<div class="region-goods-details row">
    ...
    <div class="region-goods-details row">
        <div class="region-goods-left col-sm-2">
            <div class="col-sm-10">
                <div>
                    <h4 style="line-height: 50px;font-weight: bold;">协议</h4>
                    <div class="col-sm-2 pull-right"  >
                        <?php if( ce('goods' ,$item) ) { ?>
                        <input type="checkbox" class="js-switch small" name="protocolshow" value="1" <?php  if($item['protocolshow']==1) { ?>checked<?php  } ?> />
                        <?php  } else { ?>
                        <?php  if($item['protocolshow']==1) { ?>
                        <span class='text-success'>开启</span>
                        <?php  } else { ?>
                        <span class='text-default'>关闭</span>
                        <?php  } ?>
                ...
                <span style="font-weight: normal;font-size: 12px;display: block;">开启后购买商品会显示</span>
            </div>
        </div>
        <div class="region-goods-right col-sm-10 protocolcontent" <?php  if(empty($item['protocolshow'])) { ?>style="display: none;"<?php  } ?>>
            <?php if( ce('goods' ,$item) ) { ?>
            <?php  echo tpl_ueditor('protocolcontent',$item['protocolcontent'],array('height'=>'300'))?>
            <?php  } else { ?>
            <textarea id='protocolcontent' style='display:none;'><?php  echo $item['protocolcontent'];?></textarea>
            <a href='javascript:preview_html("#protocolcontent")' class="btn btn-default">查看内容</a>
            <?php  } ?>
        </div>
