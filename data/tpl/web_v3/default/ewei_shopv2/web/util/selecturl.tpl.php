    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
        	<ul class="nav nav-tabs" id="selectUrlTab">
				...
				<?php  } ?>
				<?php  if(!empty($groups)) { ?>
					<li class=""><a href="#sut_group">商品分组</a></li>
				<?php  } ?>
				<?php  if(p('article')) { ?>
			...
			<div class="tab-content ">
				...
				<div class="tab-pane active" id="sut_shop">
					<?php  if(is_array($allUrls)) { foreach($allUrls as $item) { ?>
						...
						<?php  if(is_array($item['list'])) { foreach($item['list'] as $child) { ?>
							<nav data-href="<?php echo $platform=='wxapp'?$child['url_wxapp']:$child['url']?>" class="btn btn-default btn-sm" title="商城首页"><?php  echo $child['name'];?></nav>
						<?php  } } ?>
					<?php  } } ?>
				</div>
					...
					<?php  if(is_array($categorys)) { foreach($categorys as $category) { ?>
						 <?php  if(empty($category['parentid'])) { ?>
							<div class="line">
								...
								<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  if($platform=='wxapp') { ?>/pages/goods/index/index?cate=<?php  echo $category['id'];?><?php  } else { ?><?php  echo mobileUrl('goods',array('cate'=>$category['id']), $full)?><?php  } ?>">选择</nav>
							 ...
							 <?php  if(is_array($categorys)) { foreach($categorys as $category2) { ?>
							 	<?php  if($category2['parentid']==$category['id']) { ?>
								 	<div class="line">
										...
										<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  if($platform=='wxapp') { ?>/pages/goods/index/index?cate=<?php  echo $category2['id'];?><?php  } else { ?><?php  echo mobileUrl('goods',array('cate'=>$category2['id']), $full)?><?php  } ?>">选择</nav>
									...
									<?php  if(is_array($categorys)) { foreach($categorys as $category3) { ?>
										 <?php  if($category3['parentid']==$category2['id']) { ?>
											 <div class="line">
												...
												<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  if($platform=='wxapp') { ?>/pages/goods/index/index?cate=<?php  echo $category3['id'];?><?php  } else { ?><?php  echo mobileUrl('goods',array('cate'=>$category3['id']), $full)?><?php  } ?>">选择</nav>
				...
				<div class="tab-pane" id="sut_group">
					<?php  if(is_array($groups)) { foreach($groups as $group) { ?>
						<div class="line">
							<div class="icon icon-1"></div>
							<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  if($platform=='wxapp') { ?><?php  } else { ?><?php  echo mobileUrl('goods',array('group'=>$group['id']), $full)?><?php  } ?>">选择</nav>
