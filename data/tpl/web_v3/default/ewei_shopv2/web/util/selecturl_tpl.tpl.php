	...
	<?php  } else { ?>
		<?php  if(is_array($list)) { foreach($list as $item) { ?>
			<div class="line">
				...
				<nav data-href="<?php  if($platform=='wxapp') { ?>/pages/article/detail?id=<?php  echo $item['id'];?><?php  } else { ?><?php  echo mobileUrl('article', array('aid'=>$item['id']), $full)?><?php  } ?>" class="btn btn-default btn-sm" title="选择">选择</nav>
...
<?php  } else if($type=='creditshop') { ?>
	...
	<?php  } else { ?>
		...
		<div class="line good">
			...
			<div class="text">
				<p class="name"><?php  echo $item['title'];?></p>
				<p class="price">最低价: <?php  echo $item['minprice'];?> 原价: <?php  echo $item['price'];?>元 现价: <?php  echo $item['credit'];?><?php  echo $_W['shopset']['trade']['credittext'];?>+<?php  echo $item['money'];?>元</p>
			</div>
		</div>
		<?php  } } ?>
	<?php  } ?>
