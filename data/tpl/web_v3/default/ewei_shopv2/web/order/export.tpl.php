...
<div class="page-header">
    当前位置：<span class="text-primary">订单自定义导出</span>
</div>
<style type='text/css'>
...
<div class="page-content">
    <form action="./index.php" method="post" class="form-horizontal" role="form" id="form2">
        <input type="hidden" name="c" value="site" />
        <input type="hidden" name="a" value="entry" />
        <input type="hidden" name="m" value="ewei_shopv2" />
        <input type="hidden" name="do" value="web" />
        <input type="hidden" name="r" value="order.export.match" />
        <div class="panel panel-default">
            
            <div class='panel-heading'>
                标题对应
            </div>
            <div class="panel-body" id='add_match'>


                <?php  if(is_array($default_columns)) { foreach($default_columns as $row) { ?>
                 <div class="form-group">
                    <label class="col-lg control-label must"><?php echo !empty($row['subtitle'])?$row['subtitle']:$row['title']?> 更改为：</label>
                    <div class="col-sm-9 col-xs-12">
                         <input type="text" name="post[<?php  echo $row['field'];?>]" class="match-item form-control" value="<?php  echo $ordermatchs[$row['field']];?>"/>
                    </div>
                </div>
                <?php  } } ?>
        ...
        <div class='panel-body'>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-7 col-lg-9 col-xs-12">
                    <button type="submit" name="export" value="1" class="btn btn-primary">保存设置</button>
                </div>
            </div>
        </div>
