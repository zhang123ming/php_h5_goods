...
<script type="text/html" id="tpl_edit_danmu">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10">
            ...
            <label class="radio-inline"><input type="radio" name="datatype" value="3" class="diy-bind" data-bind-child="params" data-bind="datatype" data-bind-init="true" <%if params.datatype=='3'%>checked<%/if%>>混合数据</label>
            <%if params.datatype=='2'%>
                <div class="help-block">提示：自定义数据时下单时间可填"5秒前"、"5秒"或"5" 后两者系统将自动转换为前者</div>
            <%/if%>
            <%if params.datatype=='3'%>
            <div class="help-block">提示：混合数据包含了下订单,成为会员,充值记录等等轮播展示</div>
            <%/if%>
        </div>
    </div>
