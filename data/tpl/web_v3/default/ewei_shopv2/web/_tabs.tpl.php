<?php defined('IN_IA') or exit('Access Denied');?><div class="subnav-scene">
	...
	<?php  } else { ?>
		<a href="<?php  echo webUrl($sysmenus['submenu']['route'])?>"><?php  echo preg_replace('/积分/', $_W['shopset']['trade']['credittext'], $sysmenus['submenu']['subtitle']); ?></a>
	<?php  } ?>
</div>
...
<?php  if(!empty($sysmenus['submenu']['items'])) { ?>
	<?php  if(is_array($sysmenus['submenu']['items'])) { foreach($sysmenus['submenu']['items'] as $submenu) { ?>
		<?php  if(!empty($submenu['items'])) { ?>
			<div class='menu-header <?php  if($submenu['active']) { ?>active data-active<?php  } ?>'><div class="menu-icon fa fa-caret-<?php  if($submenu['active']) { ?>down<?php  } else { ?>right<?php  } ?>"></div><?php  echo preg_replace('/积分/', $_W['shopset']['trade']['credittext'], $submenu['title']); ?></div>
			<ul <?php  if($submenu['active']) { ?>style="display: block"<?php  } ?>>
				<?php  if(is_array($submenu['items'])) { foreach($submenu['items'] as $threemenu) { ?>
					<li <?php  if($threemenu['active']) { ?>class="active"<?php  } ?>><a href="<?php  echo $threemenu['url'];?>" style="cursor: pointer;" data-route="<?php  echo $threemenu['route'];?>"><?php  echo preg_replace('/积分/', $_W['shopset']['trade']['credittext'], $threemenu['title']); ?></a>
				<?php  } } ?>
			</ul>
		<?php  } else { ?>
