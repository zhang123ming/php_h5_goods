...
<div class="region-goods-details row">
    ...
    <div class="region-goods-right col-sm-10">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <?php if( ce('goods' ,$item) ) { ?>
                <label class="checkbox-inline" <?php  if(empty($com_set['commissionMode'])) { ?> style="display: none;" <?php  } ?>>
                    <input type="checkbox" id="commissionagent" value="1" name="commissionagent" <?php  if($item['commissionagent']==1) { ?>checked='checked'<?php  } ?> />是否开启代理返利
                </label><br/>
                <!-- <label class="checkbox-inline" style="margin:10px 0px;">
                    <input type="checkbox" id="commissionself" value="1" name="commissionself" <?php  if($item['commissionself']==1) { ?>checked='checked'<?php  } ?> />是否开启自购返利
                </label><br/> -->
    ...
    <div class="form-group">
        <label class="col-sm-2 control-label">区代</label>
        <div class="col-sm-4 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <div class="input-group">
                <input type="text" name="level1_rate" id="level1_rate" class="form-control" value="<?php  echo $item['level1_rate'];?>" />
                <div class="input-group-addon">%</div>
            </div>
            <?php  } else { ?>
            <div class='form-control-static'>比例: <?php  echo $item['level1_rate'];?>% </div>
            <?php  } ?>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label">市代</label>
        <div class="col-sm-4 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <div class="input-group">
                <input type="text" name="level2_rate" id="level2_rate" class="form-control" value="<?php  echo $item['level2_rate'];?>" />
                <div class="input-group-addon">% </div>
            </div>
            <?php  } else { ?>
            <div class='form-control-static'>比例: <?php  echo $item['level2_rate'];?>%</div>
            <?php  } ?>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label">省代</label>
        <div class="col-sm-4 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <div class="input-group">
                <input type="text" name="level3_rate" id="level3_rate" class="form-control" value="<?php  echo $item['level3_rate'];?>" />
                <div class="input-group-addon">%</div>
            </div>
            <?php  } else { ?>
            <div class='form-control-static'>比例: <?php  echo $item['level3_rate'];?>%</div>
            <?php  } ?>
        </div>
    </div>
    <div id="commission_1" <?php  if($commission_type!=1) { ?> style="display:none;" <?php  } ?>>
     ...
     <div class="form-group" id="has_commission_agent_group" <?php  if(empty($item['commissionagent'])) { ?>style="display:none;"<?php  } ?>>
        <label class="col-sm-2 control-label">是否开启独立代理返利</label>
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <label class="checkbox-inline">
                <input type="checkbox" id="has_commission_agent"  value="1" name="has_commission_agent" <?php  if($item['has_commission_agent']==1) { ?>checked<?php  } ?> /> 是
            </label>
            <span class="help-block">如果开启独立则，代理返利按照以下参数进行计算</span>
            <?php  } else { ?>
            <div class='form-control-static'><?php  if($item['has_commission_agent']==1) { ?>是<?php  } else { ?>否<?php  } ?></div>
            <?php  } ?>
        </div>
    </div>
    <div id="commission_agent_div" <?php  if($item['has_commission_agent']!=1) { ?> style="display:none;" <?php  } ?>> 
        <div class='alert alert-danger'>
            以下为代理返利，会员等级91以上的会员才会享有，不设置则为零
        </div>
        <?php  if(is_array($levels)) { foreach($levels as $lv) { ?>
            <?php  if($lv['level']>90) { ?>
            <div class="form-group">
                <label class="col-sm-2 control-label"><?php  echo $lv['levelname'];?>返利</label>
                <div class="col-sm-4 col-xs-12">
                    <?php if( ce('goods' ,$item) ) { ?>
                    <div class="input-group">
                        <input type="text" name="commission_agent[commission][<?php  echo $lv['level'];?>]" class="form-control" value="<?php  echo $commission_agent['commission'][$lv['level']];?>" />
                        <div class="input-group-addon">% 同级收益</div>
                        <input type="text" name="commission_agent[samegrade][<?php  echo $lv['level'];?>]" class="form-control" value="<?php  echo $commission_agent['samegrade'][$lv['level']];?>" />
                        <div class="input-group-addon">%</div>
                    </div>
                    <?php  } ?>
                </div>
...
<script>
    $(function () {
        ...
        $("#has_commission_agent").click(function() {
            var obj = $(this);
            if (obj.get(0).checked) {
                $("#commission_agent_div").show();
            } else {
                $("#commission_agent_div").hide();
            }
        });
        $('#commissionagent').click(function(){
              var obj = $(this);
            if (obj.get(0).checked) {
                $("#has_commission_agent_group").show();
            } else {
                $("#has_commission_agent_group").hide();
            }
        });
    });
