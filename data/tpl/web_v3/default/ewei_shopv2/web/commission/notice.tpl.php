...
<div class="page-content">
    <form id="setform"  <?php if(cv('commission.notice.edit')) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate">
        ...
        <div class='alert alert-primary' id="advanced_alert">
            使用高级模式 , 将全部启用自定义的模板内容进行推送 ! <span class="text-danger"><a href="<?php  echo webUrl('sysset/tmessage')?>">模板库(点击进入)</a> <?php  if($opensms) { ?><span class="text-danger"><a href="<?php  echo webUrl('sysset/sms/temp')?>">短信模板库(点击进入)</a></span><?php  } ?>
        </div>
        <div class='alert alert-primary' id="normal_alert">
            默认为全部开启，用户在会员中心可自行设置是否开启, 模板消息自动替换变量 <?php  if($opensms) { ?><span class="text-danger"><a href="<?php  echo webUrl('sysset/sms/temp')?>">短信模板库(点击进入)</a></span><?php  } ?>
        </div>
        <?php  } ?>
        <div id="normal">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    ...
                    <div class="help-block">模板变量 [下级昵称] [订单编号] [订单金额] [商品详情] [时间] [下线层级] [分销收益] [团队收益]</div>
                    <div class="help-block">注意: [分销收益] [团队收益] ，不代表上级用户会立即获得，为可能获得的佣金金额</div>
                    <?php  } else { ?>
        ...
        <div id="advanced">
            ...
            <table class="table table-responsive">
                ...
                <tr>
                    ...
                    <td>
                        <label class="radio-inline"><input type="radio" value="0" name="data[commission_order_finish_advanced_notice]" <?php  if(empty($data['tm']['commission_order_finish_advanced_notice'])) { ?>checked=""<?php  } ?>> <?php echo empty($data['texts']['c1'])?'一级':$data['texts']['c1']?>[默认]
    </label>
...
<!-- yi fu yuan ma wang 54mI5p2D5omA5pyJ-->