<?php defined('IN_IA') or exit('Access Denied');?><div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12" >
        ...
        <label class="radio-inline">是否需要审核</label>
        <label class="radio-inline"><input type="radio"  name="data[ischeck1]" value="0" <?php  if($data['ischeck1'] == 0) { ?> checked="checked"<?php  } ?> /> 是</label>
        <label class="radio-inline"><input type="radio"  name="data[ischeck1]" value="1" <?php  if($data['ischeck1'] == 1) { ?> checked="checked"<?php  } ?> /> 否</label>
        <br>
...
<div class="form-group">
    <label class="col-lg control-label">提现次数</label>
    <div class="col-sm-9 col-xs-12 fixmore-input-group">
        <?php if(cv('commission.set.edit')) { ?>
        <input type="text" name="data[cashnum]" class="form-control" value="<?php echo empty($data['cashnum'])?0:$data['cashnum']?>"  />
        <span class="help-block">每个月可提现的次数，为0时没有限制</span>
        <?php  } else { ?>
        <?php echo empty($data['cashnum'])?1:$data['cashnum']?>
        <?php  } ?>
    </div>
</div>
<div class="form-group">
    <label class="col-lg control-label">提现时间</label>
    <div class="col-sm-9 col-xs-12 fixmore-input-group">
        <?php if(cv('commission.set.edit')) { ?>
        <input type="text" name="data[successtime]" class="form-control" value="<?php echo empty($data['successtime'])?0:$data['successtime']?>"  />
        <span class="help-block">订单完成几天后才能提现，默认订单完成可提现</span>
        <?php  } else { ?>
        <?php echo empty($data['successtime'])?0:$data['successtime']?>
        <?php  } ?>
    </div>
</div>
...
<div class="form-group">
    <label class="col-lg control-label">佣金结算方式</label>
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('commission.set.edit')) { ?>        
        <label class="radio-inline"><input type="radio"  name="data[commissionsettletype]" value="0" <?php  if(isset($data['commissionsettletype'])) { ?> checked="checked"<?php  } ?> /> 订单完成后结算</label>
        <label class="radio-inline"><input type="radio"  name="data[commissionsettletype]" id="commissionsettletype" value="1" <?php  if($data['commissionsettletype'] ==1) { ?> checked="checked"<?php  } ?>/> 付款后结算</label>
        <label class="radio-inline" ><input type="radio"  name="data[commissionsettletype]" id="commissionsettletype" value="2" <?php  if($data['commissionsettletype'] ==2) { ?> checked="checked"<?php  } ?>/> 发货后结算</label>
        <span class="help-block">默认方式: 订单完成后才可以进行提现，佣金即可进行提现<br>付款后秒到: 订单完成付款，产生的佣金可立马提现<br/>发货后结算：订单发货后，产生的佣金可立马提现</span>
        <?php  } ?>
    </div>
</div>
<div class="form-group showtype">
    <label class="col-lg control-label">整数倍提现</label>
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('commission.set.edit')) { ?>  
        <div class='input-group fixmore-input-group'>
            <span class='input-group-addon'><input type="radio"  name="data[ismultiple]" value="1" <?php  if($data['ismultiple'] == 1) { ?> checked="checked"<?php  } ?> />是</span>
            <input type="text" name="data[withdrawtype]" class="form-control" value="<?php  echo $data['withdrawtype'];?>" placeholder="请输入提现奇数" />
            <span class='input-group-addon'><input type="radio"  name="data[ismultiple]" value="0" <?php  if($data['ismultiple'] == 0) { ?> checked="checked"<?php  } ?> />否</span>
        </div>
        <span class='help-block'>整数倍提现：按所填基数的倍数提现</span>
        <?php  } else { ?>
            <?php  echo $data['withdrawtype'];?>
        <?php  } ?>
    </div>
</div>
<!-- yi fu yuan ma wang -->