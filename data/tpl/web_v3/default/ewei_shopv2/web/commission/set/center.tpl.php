...
<div id="qrcode" style="height: 620px"  <?php  if(empty($data['qrcode'])) { ?>style="display: none"<?php  } ?>>
    ...
    <div class="form-group">
        <label class="col-lg control-label">显示以下教学内容</label>
        <div class="col-sm-9 col-xs-12">
            <?php if(cv('commission.set.edit')) { ?>
            <label class="radio-inline"><input type="radio"  name="data[teaching]" value="1" <?php  if($data['teaching'] ==1) { ?> checked="checked"<?php  } ?> /> 隐藏</label>
            <label class="radio-inline"><input type="radio"  name="data[teaching]" value="0" <?php  if(empty($data['teaching'])) { ?> checked="checked"<?php  } ?> /> 显示</label>
            <?php  } else { ?>
            <?php  if(empty($data['teaching'])) { ?>显示<?php  } else { ?>隐藏<?php  } ?>
            <?php  } ?>
        </div>
    </div>    
