    ...
    <div class="col-sm-9 col-xs-12">
        ...
        <div class='input-group'>
            ...
            <div class='input-group-btn'>
                <button class="btn btn-default" type="button" <?php  if($merchUser) { ?> disabled <?php  } ?>onclick="chooseAgent()">选择上级分销商</button>
                <button class="btn btn-danger" type="button" <?php  if($merchUser) { ?> disabled <?php  } ?> onclick="clearAgent()">清除选择</button>
            </div>
        </div>
            ...
            <div class="modal-dialog" style='width: 920px;'>
                <div class="modal-content">
                    ...
                    <div class="modal-body" >
                        <div class="row">
                            <div class="input-group">
                                <input type="text" class="form-control" name="keyword" value="" id="search-kwd-notice" placeholder="请输入分销商昵称/姓名/手机号/ID" />
...
<div class="form-group">
    <label class="col-lg control-label">上级代理</label>
    <div class="col-sm-9 col-xs-12">
        <?php  if(is_array($agent)) { foreach($agent as $info) { ?>
        <div class='aagent-item aagent-province-item btn btn-default'  style='margin-bottom:5px;margin-right:5px'>
            <a href="<?php  echo webUrl('member/list/detail',array('id' => $info['id']));?>" title="会员详情"><?php  echo $info['levelname'];?>：<?php  if(!empty($info['nickname'])) { ?><?php  echo $info['nickname'];?><?php  } else { ?>未更新<?php  } ?></a>
        </div>
        <?php  } } ?>
    </div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('commission.agent.edit')) { ?>
        <select name='adata[agentlevel]' <?php  if($merchUser&&$merchUser['commission_level']<$member['commission_level']) { ?> disabled <?php  } ?> class='form-control'>
            ...
            <option value='<?php  echo $level['id'];?>' <?php  if($merchUser&&$merchUser['commission_level']<$member['commission_level']) { ?> hidden <?php  } ?> <?php  if($member['agentlevel']==$level['id']) { ?>selected<?php  } ?>><?php  echo $level['levelname'];?></option>
            <?php  } } ?>
        </select>
        <?php  } else { ?>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        <?php if(cv('commission.agent.edit')) { ?>
        <label class="radio-inline" ><input type="radio" class="agentpower" name="agentpower" value="0" <?php  if($member['agentpower']['agentpower']==0) { ?>checked<?php  } ?>>关闭</label>
        <label class="radio-inline"><input type="radio" class="agentpower" name="agentpower" value="1" <?php  if($member['agentpower']['agentpower']==1) { ?>checked<?php  } ?>>开启</label>
        <span class="help-block">开启时拥有审核下线分销等级的权限(不显示默认等级)</span>
        <div class='form-control-static' id="leveloption" style="display: <?php  if($member['agentpower']['agentpower']==0) { ?>none <?php  } else { ?> block <?php  } ?>">
   
            <div class='form-control-static'>
                <label class="checkbox-inline" >等级:</label>
                <?php  if(!empty($comlevel)) { ?>
                    <?php  if(is_array($comlevel)) { foreach($comlevel as $clevel) { ?>
                    <label class="checkbox-inline" ><input type="checkbox" <?php if(cv(in_array($clevel['id'],$member['agentpower']['agentlevelid']))) { ?>checked="checked"<?php  } ?> name="aglevel" id="<?php  echo $clevel['id'];?>" ><?php  echo $clevel['levelname'];?></label>
                    <?php  } } ?>
                <?php  } else { ?>
                    <?php  if(is_array($levels)) { foreach($levels as $ulevel) { ?>
                    <label class="checkbox-inline" ><input type="checkbox" <?php if(cv(in_array($ulevel['id'],$member['agentpower']['userlevelid']))) { ?>checked="checked"<?php  } ?> name="uslevel" id="<?php  echo $ulevel['id'];?>" ><?php  echo $ulevel['levelname'];?></label>
                    <?php  } } ?>
                <?php  } ?>
            </div>
            <input type="hidden" name="uslevel" value="">
            <input type="hidden" name="aglevel" value="">
        </div>
        <?php  } else { ?>
        <input type="hidden" name="agentpower" class="form-control" value="<?php  echo $member['agentpower'];?>"  />
        <div class='form-control-static'><?php  if($member['agentpower']==1) { ?>开启<?php  } else { ?>关闭<?php  } ?></div>
        <?php  } ?>
    </div>
</div>
<?php  } ?>
...
<script language='javascript'>
   ...
   $(function () {
        $(".agentpower").unbind('click').click(function () {
            var val = $(this).val();
            if(val==1){
                $("#leveloption").css({'display':'inline-block'});
            }else{
                $("#leveloption").css({'display':'none'});
            }
        });
        ...
        $("input[type='checkbox']").click(function () {
            ...
            $("input[type='hidden'][name='"+name+"']").val(ulevel);
        });
    })
